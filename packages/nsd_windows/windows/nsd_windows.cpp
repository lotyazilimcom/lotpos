...
namespace nsd_windows {
	...
	NsdWindows::NsdWindows(
		...
		this->dispatchHwnd = dispatchHwnd;
	...
	std::optional<LRESULT> NsdWindows::HandleWindowProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
	{
		if (this->dispatchMessage == 0 || message != this->dispatchMessage) {
			return std::nullopt;
		}

		DrainPlatformInvocations();
		return 0;
	}

	void NsdWindows::InvokeMethodOnPlatformThread(std::string method, flutter::EncodableMap args)
	{
		if (this->dispatchHwnd == nullptr || this->dispatchMessage == 0) {
			methodChannel->InvokeMethod(method, CreateMethodResult(args));
			return;
		}

		{
			std::lock_guard<std::mutex> lock(dispatchMutex);
			dispatchQueue.push([this, method = std::move(method), args = std::move(args)]() mutable {
				methodChannel->InvokeMethod(method, CreateMethodResult(args));
				});
		}

		PostMessage(this->dispatchHwnd, this->dispatchMessage, 0, 0);
	}

	void NsdWindows::DrainPlatformInvocations()
	{
		std::queue<std::function<void()>> localQueue;
		{
			std::lock_guard<std::mutex> lock(dispatchMutex);
			std::swap(localQueue, dispatchQueue);
		}
	...
	void NsdWindows::OnServiceDiscovered(const std::string handle, const DWORD status, PDNS_RECORD records)
	{
		...
		if (serviceInfo.status == ServiceInfo::STATUS_FOUND) {
			...
			if (it == services.end()) {
				...
				InvokeMethodOnPlatformThread("onServiceDiscovered", {
						{ "handle", handle },
			...
			if (it != services.end()) {
				services.erase(it);
				InvokeMethodOnPlatformThread("onServiceLost", {
						{ "handle", handle },
						{ "service.name", serviceInfo.name.value() },
						{ "service.type", serviceInfo.type.value() },
					});
	...
	void NsdWindows::OnServiceResolved(const std::string handle, const DWORD status, PDNS_SERVICE_INSTANCE pInstance)
	{
		...
		if (status != ERROR_SUCCESS) {
			InvokeMethodOnPlatformThread("onResolveFailed", {
					{ "handle", handle },
		...
		InvokeMethodOnPlatformThread("onResolveSuccessful", {
				{ "handle", handle },
	...
	void NsdWindows::OnServiceRegistered(const std::string handle, const DWORD status, PDNS_SERVICE_INSTANCE pInstance)
	{
		...
		if (status != ERROR_SUCCESS) {
			DnsServiceFreeInstance(pInstance);
			InvokeMethodOnPlatformThread("onRegistrationFailed", {
					{ "handle", handle },
					{ "error.cause", ToErrorCode(ErrorCause::INTERNAL_ERROR) },
					{ "error.message", GetErrorMessage(status) },
				});
			InvokeMethodOnPlatformThread("onRegistrationFailed", { { "handle", handle } });
			return;
		}
		...
		InvokeMethodOnPlatformThread("onRegistrationSuccessful", {
				{ "handle", handle },
				{ "service.type", serviceType },
				{ "service.name", serviceName },
				{ "service.port", servicePort },
	...
	void NsdWindows::OnServiceUnregistered(const std::string handle, const DWORD status, PDNS_SERVICE_INSTANCE pInstance)
	{
		...
		if (status != ERROR_SUCCESS) {
			InvokeMethodOnPlatformThread("onUnregistrationFailed", {
					{ "handle", handle },
					{ "error.cause", ToErrorCode(ErrorCause::INTERNAL_ERROR) },
					{ "error.message", GetErrorMessage(status) },
				});
		...
		InvokeMethodOnPlatformThread("onUnregistrationSuccessful", { { "handle", handle } });
	}
