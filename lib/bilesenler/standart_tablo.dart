...
class StandartTablo extends StatefulWidget {
  ...
  /// - "son sayfa" atlaması yoktur (deep pagination maliyeti önlenir).
...
class _StandartTabloState extends State<StandartTablo> {
  ...
  List<int> get _rowsPerPageOptions => widget.cursorPagination
      ...
      : const <int>[10, 25, 50, 100, 250, 500];
  ...
  @override
  void didUpdateWidget(covariant StandartTablo oldWidget) {
    super.didUpdateWidget(oldWidget);

    // Cursor moduna geçişte (veya geri dönüşte) dropdown value güvenli aralıkta olmalı.
    // Aksi halde DropdownButton "value not in items" hatası verebilir.
  ...
  Widget _buildPaginationFooter() {
    ...
    final int dataCount = widget.source.rows.length;
    final int startRecord = dataCount > 0
    ...
    if (dataCount == 0) {
      ...
      endRecord = startRecord + dataCount - 1;
    } else {
      endRecord = (_currentPage * effectiveRowsPerPage).clamp(
        0,
        widget.totalRecords,
      );
    }
