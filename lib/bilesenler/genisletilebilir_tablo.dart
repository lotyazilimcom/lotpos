...
class GenisletilebilirTablo<T> extends StatefulWidget {
  ...
  ///
  /// "Limitsiz" tanımı:
  /// - Her istek mutlaka LIMIT'li olmalıdır (sayfa başı kayıt sayısı).
  /// - Limitsiz olan şey: sayfa sayısıdır (next/prev ile sınırsız gezinebilirsin).
  /// - Cursor modunda UI güvenlik üst sınırı: sayfa başı en fazla 250 kayıt.
  ///
  /// Not:
  /// - totalRecords=0 olsa bile footer görünür.
...
class _GenisletilebilirTabloState<T> extends State<GenisletilebilirTablo<T>> {
  static const int _cursorMinRowsPerPage = 10;
  static const int _cursorMaxRowsPerPage = 250;
  ...
  int _rowsPerPage = 25;
  int _currentPage = 1;
  List<int> get _rowsPerPageOptions => widget.cursorPagination
      ? const <int>[_cursorMinRowsPerPage, 25, 50, 100, _cursorMaxRowsPerPage]
      : const <int>[10, 25, 50, 100, 250, 500, 1000];
  int? _expandedIndex;
  ...
  void initState() {
    ...
    // ESC key handling: clear search text when ESC is pressed
  ...
  void didUpdateWidget(GenisletilebilirTablo<T> oldWidget) {
    ...
    // Parent requested pagination reset (keep rowsPerPage; reset page/expanded).
    final bool resetRequested =
        widget.paginationResetKey != oldWidget.paginationResetKey;
    final bool cursorModeChanged =
        widget.cursorPagination != oldWidget.cursorPagination;
    if (resetRequested || cursorModeChanged) {
      setState(() {
        _currentPage = 1;
        _expandedIndex = null;
        _focusedRowIndex = null;
  ...
  Widget build(BuildContext context) {
    return TapRegion(
      ...
      child: Focus(
        ...
        autofocus:
            widget.autofocusTable, // CRITICAL: Auto-focus when widget is built
                                ...
                                delegate: SliverChildBuilderDelegate((
                                  ...
                                  final isExpanded =
                                      ...
                                      (widget.expandedIndices?.contains(
                                          ...
                                          false);
                                  final isSelected =
                                      widget.isRowSelected?.call(item, index) ??
                                      false;
                                    ...
                                    header: Material(
                                      ...
                                      child: InkWell(
                                        ...
                                        onTap: () {
                                          ...
                                          if (widget.deselectOnRepeatedRowTap &&
                                              _focusedRowIndex == index) {
                                            _clearAllSelections();
                                            return;
                                          }
  ...
  Widget _buildPaginationFooter() {
    ...
    // Legacy (COUNT-based) footer: hide when totalRecords unknown/0.
    if (!isCursorMode && widget.totalRecords == 0) {
      return const SizedBox.shrink();
    }

    final int startRecord = widget.data.isNotEmpty
    ...
    if (widget.data.isEmpty) {
      ...
      endRecord = startRecord + widget.data.length - 1;
    } else {
      endRecord = (_currentPage * effectiveRowsPerPage);
    ...
    // Cursor mode: toplam sayıyı bilmeyiz, ama kullanıcıya "sonsuz" hissi verelim.
    // Mevcut çeviri anahtarını bozmamak için {total} alanına "end+" formatı yazıyoruz.
    ...
    return Container(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final paginationButtons = isCursorMode
              ? Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    _buildPageButton(
                    ...
                    const SizedBox(width: 8),
                    _buildPageButton(text: '$_currentPage', isActive: true),
                    const SizedBox(width: 8),
                    _buildPageButton(
                      icon: Icons.keyboard_arrow_right,
                      onTap: widget.hasNextPage
                          ? () => _changePage(_currentPage + 1)
                          : null,
                    ),
                  ],
