...
class _BankalarSayfasiState extends State<BankalarSayfasi> {
  ...
  Future<void> _fetchBankalar({bool showLoading = true}) async {
    ...
    try {
      ...
      final List<String> indexTables = needsTxTables
          ? const <String>['banks', 'bank_transactions']
          : const <String>['banks'];
      ...
      if (txType.isNotEmpty) {
        if (txType == 'Satış Yapıldı' || txType == 'Satis Yapildi') {
          filterParts.add(
            '(integration_ref:SALE-* OR integration_ref:RETAIL-*)',
          );
        } else if (txType == 'Alış Yapıldı' || txType == 'Alis Yapildi') {
          filterParts.add('integration_ref:PURCHASE-*');
        } else {
      ...
      if (usePrimarySearch) {
        ...
        final fallback = await BankalarVeritabaniServisi().bankalariGetir(
          sayfa: _currentPage,
      ...
      final List<BankaModel> display = depolar;
  ...
  Future<void> _loadMoreDetailTransactions(
    ...
    if (_detailTransactionsLoading[banka.id] == true) return;

    final existing =
        _detailTransactionsCache[banka.id] ??
        List<Map<String, dynamic>>.from(initialTransactions);
    if (existing.isEmpty) return;

    final int? lastId = existing.last['id'] as int?;
    if (lastId == null) {
      setState(() => _detailTransactionsHasMore[banka.id] = false);
      return;
    }

    setState(() {
      _detailTransactionsLoading[banka.id] = true;
      _detailTransactionsCache[banka.id] = existing;
      _detailTransactionsHasMore[banka.id] =
          _detailTransactionsHasMore[banka.id] ??
          (initialTransactions.length == _detailTransactionsPageSize);
    });

    try {
      final more = await BankalarVeritabaniServisi().bankaIslemleriniGetir(
        banka.id,
        aramaTerimi: _searchQuery,
        kullanici: _selectedUser,
        baslangicTarihi: _startDate,
      ...
      setState(() {
        _detailTransactionsCache[banka.id] = [...existing, ...more];
        _detailTransactionsHasMore[banka.id] =
            more.length == _detailTransactionsPageSize;
        _detailTransactionsLoading[banka.id] = false;
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _detailTransactionsLoading[banka.id] = false;
        _detailTransactionsHasMore[banka.id] = false;
      });
      debugPrint('Banka detay yükle hatası: $e');
    }
  }
    ...
    try {
      ...
      for (var i = 0; i < dataToProcess.length; i++) {
        ...
        final isExpanded =
            ...
            _expandedMobileIds.contains(banka.id) ||
          ...
          txTable = DetailTable(
            ...
            data: transactions.map((t) {
              ...
              if (lowRef.startsWith('retail-')) {
                ilgiliHesap = _getPerakendeRelatedName(yerAdi);
              }
        ...
        return Container(
          ...
          child: FutureBuilder<List<Map<String, dynamic>>>(
            ...
            builder: (context, snapshot) {
              ...
              final cached = _detailTransactionsCache[banka.id];
    ...
    return Builder(
      builder: (context) {
        ...
        final String relatedLocationName = isPerakendeSatis
            ? _getPerakendeRelatedName(locationName)
            : locationName;
  ...
  Widget _buildMobileView(List<BankaModel> filteredBankalar) {
    final int safeRowsPerPage = _rowsPerPage <= 0 ? 25 : _rowsPerPage;
    final int startRecordIndex = (_currentPage - 1) * safeRowsPerPage;
    final int showingStart = filteredBankalar.isEmpty
        ? 0
        : startRecordIndex + 1;
    final int endRecord = filteredBankalar.isEmpty
        ? 0
        : startRecordIndex + filteredBankalar.length;
    final String totalLabel = filteredBankalar.isEmpty
        ? '0'
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          body = Column(
            children: transactions.map((tx) {
              ...
              if (lowRef.startsWith('retail-')) {
                relatedName = _getPerakendeRelatedName(relatedName);
                relatedCode = '';
              }
  ...
  String _getSourceSuffix(
    ...
    if (integrationRef != null) {
      ...
      if (lowRef.startsWith('retail-')) {
        return _getPerakendeSuffix(locationName);
      }
  ...
  String _getPerakendeRelatedName(String locationName) {
    final String trimmed = locationName.trim();
    if (trimmed.toLowerCase().startsWith('perakende satış yapıldı')) {
      return trimmed;
    }
    return 'Perakende Satış Yapıldı (Banka)';
  }

  String _getPerakendeSuffix(String locationName) {
    final String label = _getPerakendeRelatedName(locationName);
    final match = RegExp(r'\\([^\\)]+\\)\\s*$').firstMatch(label);
    return match?.group(0) ?? '(Banka)';
  }
}
