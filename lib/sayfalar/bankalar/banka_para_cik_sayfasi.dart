...
class BankaParaCikSayfasi extends StatefulWidget {
  final int? islemId;
  final Map<String, dynamic>? initialData;
  final BankaModel banka;

  const BankaParaCikSayfasi({
    super.key,
    required this.banka,
    this.islemId,
    this.initialData,
  ...
  @override
  State<BankaParaCikSayfasi> createState() => _BankaParaCikSayfasiState();
}

class _BankaParaCikSayfasiState extends State<BankaParaCikSayfasi> {
  ...
  final List<Map<String, String>> _sourceOptions = [
    {
      ...
      'key': 'banks.transaction.type.current_account',
    },
    {'value': 'cash', 'key': 'banks.transaction.type.cash'},
    {'value': 'bank', 'key': 'banks.transaction.type.bank'},
    {'value': 'credit_card', 'key': 'banks.transaction.type.credit_card'},
    {'value': 'personnel', 'key': 'banks.transaction.type.personnel'},
    {'value': 'income', 'key': 'banks.transaction.type.income'},
    {'value': 'other', 'key': 'banks.transaction.type.other'},
  ];
  ...
  void _showBankaSearchDialog() {
    showDialog(
      ...
      builder: (context) => _BankaSearchDialog(
        excludeBankaId: widget.banka.id,
        onSelect: (banka) {
          setState(() {
  ...
  Future<void> _selectDate() async {
    final DateTime? picked = await showDialog<DateTime>(
      ...
      builder: (context) => TekTarihSeciciDialog(
        ...
        title: tr('banks.transaction.date'),
      ),
    );
    ...
    try {
      ...
      if (widget.islemId != null) {
        ...
        if (!isVirman) {
          ...
          final ref = 'VIRMAN-${DateTime.now().microsecondsSinceEpoch}';

          // 1) Bu banka -> Ödeme (Çıkış)
          await BankalarVeritabaniServisi().bankaIslemEkle(
            bankaId: widget.banka.id,
            tutar: amount,
            islemTuru: 'Ödeme',
          ...
          if (_selectedSource == 'cash') {
            await KasalarVeritabaniServisi().kasaIslemEkle(
              ...
              aciklama: _descriptionController.text,
              tarih: _selectedDate,
              cariTuru: tr('cashregisters.transaction.type.bank'),
              cariKodu: widget.banka.kod,
              cariAdi: widget.banka.ad,
              kullanici: currentUser,
              cariEntegrasyonYap: false,
              entegrasyonRef: ref,
            );
            ...
            await BankalarVeritabaniServisi().bankaIslemEkle(
              ...
              aciklama: _descriptionController.text,
              tarih: _selectedDate,
              cariTuru: tr('banks.transaction.type.bank'),
              cariKodu: widget.banka.kod,
              cariAdi: widget.banka.ad,
              kullanici: currentUser,
              cariEntegrasyonYap: false,
          ...
          } else if (_selectedSource == 'credit_card') {
            await KrediKartlariVeritabaniServisi().krediKartiIslemEkle(
              ...
              aciklama: _descriptionController.text,
              tarih: _selectedDate,
              cariTuru: tr('creditcards.transaction.type.bank'),
              cariKodu: widget.banka.kod,
              cariAdi: widget.banka.ad,
              kullanici: currentUser,
              cariEntegrasyonYap: false,
          ...
          child: Scaffold(
            ...
            appBar: AppBar(
              ...
              title: Text(
                tr(
                  widget.islemId != null
                      ? 'banks.transaction.withdraw.edit_title'
                      : 'banks.transaction.withdraw.title',
                ),
        ...
        builder: (context, constraints) {
          ...
          final balanceBlock = Column(
            ...
            children: [
              ...
              const SizedBox(height: 4),
              Text(
                '${FormatYardimcisi.sayiFormatlaOndalikli(widget.banka.bakiye, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.fiyatOndalik)} ${widget.banka.paraBirimi}',
                style: TextStyle(
                  fontSize: compact ? 16 : 18,
                  fontWeight: FontWeight.w800,
                  color: widget.banka.bakiye >= 0
        ...
        return Column(
          children: [
            ...
            _buildDropdown<String>(
              ...
              label: tr('banks.transaction.destination'),
            ...
            _buildRow(isWide, [
              _buildAutocompleteField(
                ...
                label: tr('banks.transaction.account_code'),
                isRequired: true,
                color: requiredColor,
              ...
              _buildTextField(
                ...
                label: tr('banks.transaction.account_name'),
                isRequired: true,
                color: requiredColor,
                readOnly: true,
            ...
            _buildRow(isWide, [
              _buildTextField(
                ...
                label: tr('banks.transaction.amount'),
                isNumeric: true,
                isRequired: true,
                color: requiredColor,
              ...
              _buildDateField(
                ...
                label: tr('banks.transaction.date'),
                onTap: _selectDate,
                isRequired: true,
                color: requiredColor,
              ),
            ...
            AkilliAciklamaInput(
              ...
              label: tr('banks.transaction.description'),
              category: 'bank_withdrawal_description',
              color: optionalColor,
              defaultItems: [
                tr('smart_select.bank_withdraw.desc.1'),
                tr('smart_select.bank_withdraw.desc.2'),
                tr('smart_select.bank_withdraw.desc.3'),
                tr('smart_select.bank_withdraw.desc.4'),
                tr('smart_select.bank_withdraw.desc.5'),
              ],
            ),
          ],
        );
...
class _BankaSearchDialog extends StatefulWidget {
  ...
  final int? excludeBankaId;
  const _BankaSearchDialog({required this.onSelect, this.excludeBankaId});
        ...
        setState(() {
          _items = results
              .where(
                (b) =>
                    b.aktifMi &&
                    (widget.excludeBankaId == null ||
                        b.id != widget.excludeBankaId),
              )
              .toList();
          _isLoading = false;
        });
