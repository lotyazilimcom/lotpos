...
class _GiderlerSayfasiState extends State<GiderlerSayfasi> {
  ...
  Future<void> _fetchGiderler({bool showLoading = true}) async {
    ...
    try {
      ...
      final String indexSortBy = switch (_sortBy) {
        ...
        'baslik' => 'root_name.keyword:$dir',
        'tutar' => 'root_amount:$dir',
        'kategori' => 'root_category.keyword:$dir',
        'tarih' => 'root_date:$dir',
        'aktif_mi' => 'root_aktif_mi:$dir',
        'aciklama' => 'root_description.keyword:$dir',
        _ => 'root_id:$dir',
      };

      final bool needsItems = _searchQuery.trim().isNotEmpty;
      final List<String> indexTables = needsItems
          ? const <String>['expenses', 'expense_items']
          : const <String>['expenses'];

      final filterParts = <String>[];
      if (aktifMi != null) {
        filterParts.add('root_aktif_mi:${aktifMi ? 'true' : 'false'}');
      }
      final kategori = (_selectedCategory ?? '').trim();
      if (kategori.isNotEmpty) {
        filterParts.add('root_category:"${osQuote(kategori)}"');
      }
      final odeme = (_selectedPaymentStatus ?? '').trim();
      if (odeme.isNotEmpty) {
        filterParts.add('root_payment_status:"${osQuote(odeme)}"');
      }
      ...
      if (primary.indexEnabled) {
        ...
        final fallback = await GiderlerVeritabaniServisi().giderleriGetir(
          sayfa: _currentPage,
        ...
        giderler = hasMore
            ...
            : List<GiderModel>.from(fallback);
        if (giderler.isNotEmpty) {
          nextCursor = 'id:${giderler.last.id}';
        }
      }

      if (!mounted || sorguNo != _aktifSorguNo) return;
      final List<GiderModel> display = giderler;
  ...
  Widget _buildMobileView(List<GiderModel> giderler) {
    ...
    final int showingStart = giderler.isEmpty ? 0 : startRecordIndex + 1;
    final int endRecord = giderler.isEmpty
        ? 0
        : startRecordIndex + giderler.length;
    final String totalLabel = giderler.isEmpty
        ? '0'
