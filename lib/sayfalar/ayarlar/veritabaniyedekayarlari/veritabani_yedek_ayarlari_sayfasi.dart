...
import '../../../servisler/db_api/api_connection.dart';
import '../../../servisler/db_api/veritabani_api_client.dart';
...
class _VeritabaniYedekAyarlariSayfasiState
  ...
  Future<void> _loadClusterIdBestEffort() async {
    ...
    try {
      ...
      final pool = await VeritabaniHavuzu().havuzAl(database: cfg.database);
      final id = await ClusterKimligiServisi().okuVeyaOlusturWithExecutor(
        executor: pool,
        allowCreate: true,
      );
      if (!mounted) return;
  ...
  Future<bool> _guardClusterIdUyumsuzlugu({
    ...
    final hasDirectCloud =
        cloudHost != null && cloudUser != null && cloudPass != null;

    if (!VeritabaniYapilandirma.cloudApiCredentialsReady && !hasDirectCloud) {
      return true;
    }
    ...
    String? cloudId;
    String cloudDisplayHost = '';
    if (VeritabaniYapilandirma.cloudApiCredentialsReady) {
      final rawUrl = VeritabaniYapilandirma.cloudApiWriteBaseUrl;
      final rawToken = VeritabaniYapilandirma.cloudApiToken;
      final uri = Uri.tryParse((rawUrl ?? '').trim());
      final token = (rawToken ?? '').trim();
      if (uri == null || token.isEmpty) return true;
      cloudDisplayHost = uri.host.isNotEmpty ? uri.host : uri.toString();

      final conn = ApiConnection(
      ...
      try {
        cloudId = await ClusterKimligiServisi().okuVeyaOlusturWithExecutor(
          executor: conn,
          allowCreate: true,
        );
      } finally {
      ...
      cloudDisplayHost = cloud.host;
      cloudId = await ClusterKimligiServisi().okuVeyaOlustur(
        endpoint: cloud,
        sslMode: cloudSsl,
