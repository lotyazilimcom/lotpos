...
class _SirketAyarlariSayfasiState extends State<SirketAyarlariSayfasi> {
  ...
  Future<void> _verileriYukle() async {
    try {
      ...
      final int? lastId = _suAnkiSayfa > 1
          ? _pageCursors[_suAnkiSayfa - 1]
          : null;

      final veri = await service.sirketleriGetir(
        sayfa: _suAnkiSayfa,
        // [2026] COUNT(*) olmadan "hasNext" tespiti için +1 satır çek.
        sayfaBasinaKayit: _sayfaBasinaKayit + 1,
        aramaTerimi: _aramaTerimi,
        lastId: lastId,
      );

      final bool hasMore = veri.length > _sayfaBasinaKayit;
      final List<SirketAyarlariModel> display = hasMore
          ? veri.sublist(0, _sayfaBasinaKayit)
          : veri;
      ...
      if (mounted) {
        setState(() {
          ...
          final int? cursorId = display.isNotEmpty ? display.last.id : null;
          if (cursorId != null) {
            _pageCursors[_suAnkiSayfa] = cursorId;
          }
        });
  ...
  void _sayfaDegisti(int sayfa, int sayfaBasinaKayit) {
    setState(() {
      final bool rowsChanged = sayfaBasinaKayit != _sayfaBasinaKayit;
  ...
  Widget _buildActionButton({
    ...
    final VoidCallback? effectiveOnTap = printDisabled ? null : onTap;
    final Color effectiveColor = printDisabled ? Colors.grey.shade100 : color;
    final Color effectiveTextColor = printDisabled
  ...
  Widget _buildMobileSquareActionButton({
    ...
    final VoidCallback? effectiveOnTap = printDisabled ? null : onTap;
    return Tooltip(
      message: YazdirmaErisimKontrolu.tooltip(tooltip),
      child: Material(
        color: printDisabled ? Colors.grey.shade100 : color,
        borderRadius: BorderRadius.circular(8),
        child: InkWell(
          mouseCursor: printDisabled
              ? SystemMouseCursors.basic
          ...
          child: Container(
            ...
            child: Icon(
              icon,
              color: printDisabled ? Colors.grey.shade400 : iconColor,
              size: 20,
            ),
          ),
        ),
  ...
  Widget _buildMobileView() {
    ...
    final int effectivePage = _suAnkiSayfa;
    final int showingStart = _sirketler.isEmpty
        ? 0
        : ((effectivePage - 1) * _sayfaBasinaKayit + 1);
    final int showingEnd = _sirketler.isEmpty
        ? 0
        : (showingStart + _sirketler.length - 1);
    final String totalLabel = _sirketler.isEmpty
        ? '0'
        : (_hasNextPage ? '$showingEnd+' : '$showingEnd');
