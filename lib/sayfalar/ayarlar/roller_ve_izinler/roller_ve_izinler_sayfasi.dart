...
class _RollerVeIzinlerSayfasiState extends State<RollerVeIzinlerSayfasi> {
  ...
  Future<void> _verileriYukle() async {
    try {
      ...
      final String? lastId = _suAnkiSayfa > 1
      ...
      final List<RolModel> display = hasMore
      ...
      if (mounted) {
        setState(() {
          ...
          if (display.isNotEmpty) {
            _pageCursors[_suAnkiSayfa] = display.last.id;
          }
        });
  ...
  Widget _buildMobileView() {
    ...
    final int showingStart = _roller.isEmpty
        ? 0
        : ((effectivePage - 1) * _sayfaBasinaKayit + 1);
    final int showingEnd = _roller.isEmpty
        ? 0
        : (showingStart + _roller.length - 1);
    final String totalLabel = _roller.isEmpty
        ? '0'
