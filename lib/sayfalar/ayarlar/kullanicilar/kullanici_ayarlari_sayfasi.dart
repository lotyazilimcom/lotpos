...
class _KullaniciAyarlarSayfasiState extends State<KullaniciAyarlarSayfasi> {
  ...
  Future<void> _fetchKullanicilar({bool showLoading = true}) async {
    ...
    try {
      ...
      final String? lastId = _currentPage > 1
      ...
      final bool hasMore = veri.length > _rowsPerPage;
      final List<KullaniciModel> display = hasMore
          ? veri.sublist(0, _rowsPerPage)
          : veri;
      ...
      if (mounted) {
        ...
        setState(() {
          _isLoading = false;
          _cachedKullanicilar = display;
          _autoExpandedIndices = indices;
          _hasNextPage = hasMore;
          if (display.isNotEmpty) {
            _pageCursors[_currentPage] = display.last.id;
          }
        });
      }
    ...
    try {
      ...
      for (var i = 0; i < dataToProcess.length; i++) {
        ...
        final bool isExpanded =
            ...
            _expandedMobileIds.contains(kullanici.id);
    ...
    return Padding(
      ...
      child: ResponsiveFilterRow(
        items: [
          ...
          ResponsiveFilterItem(child: _buildRoleFilter(width: double.infinity)),
          ResponsiveFilterItem(
  ...
  Widget _buildMobileView() {
    ...
    final int effectivePage = _currentPage;
    final int showingStart = _cachedKullanicilar.isEmpty
        ? 0
        : ((effectivePage - 1) * safeRowsPerPage + 1);
    final int endRecord = _cachedKullanicilar.isEmpty
        ? 0
        : (showingStart + _cachedKullanicilar.length - 1);
    final String totalLabel = _cachedKullanicilar.isEmpty
        ? '0'
