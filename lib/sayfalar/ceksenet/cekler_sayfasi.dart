...
import 'cek_al_dialog.dart';
import 'cek_ver_dialog.dart';
import 'cek_tahsil_sayfasi.dart'; // Add import
import 'cek_ciro_sayfasi.dart'; // Add import
...
class CeklerSayfasi extends StatefulWidget {
  final String? initialSearchQuery;
  const CeklerSayfasi({super.key, this.initialSearchQuery});

  @override
  State<CeklerSayfasi> createState() => _CeklerSayfasiState();
}

class _CeklerSayfasiState extends State<CeklerSayfasi> {
  final TextEditingController _searchController = TextEditingController();
  final TextEditingController _startDateController = TextEditingController();
  final TextEditingController _endDateController = TextEditingController();
  final FocusNode _searchFocusNode = FocusNode();
  String _searchQuery = '';
  List<CekModel> _cachedCekler = [];
  ...
  int _rowsPerPage = 25;
  int _currentPage = 1;
  // [2026] Cursor pagination:
  // - Index mode: OpenSearch cursor token (base64url json)
  // - DB fallback: "id:<lastId>" string
  ...
  // [2026] Expanded/detail cursor pagination (limitsiz geçmiş gezinme)
  ...
  void _onGlobalSync() {
    _fetchCekler(showLoading: false);
  }
  ...
  Future<void> _fetchCekler({bool showLoading = true}) async {
    ...
    try {
      ...
      final String indexSortBy = switch (_sortBy) {
        ...
        // Amount column historically maps to a legacy key; keep behaviour but sort correctly.
        'adres' => 'root_amount:$dir',
        'duzenlenme_tarihi' => 'root_issue_date:$dir',
        'keside_tarihi' => 'root_due_date:$dir',
        _ => 'root_id:$dir',
      };
      ...
      final List<String> indexTables =
          (_searchQuery.trim().isNotEmpty || hasTxFilters)
          ? const <String>['cheques', 'cheque_transactions']
          : const <String>['cheques'];

      final filterParts = <String>[];
      final bank = (_selectedBank ?? '').trim();
      if (bank.isNotEmpty) {
        filterParts.add('root_bank:"${osQuote(bank)}"');
      }

      // Transaction-level filters should not accidentally match root docs.
      if (hasTxFilters) {
        filterParts.add('table:cheque_transactions');
      }
      ...
      if (usePrimarySearch) {
        ...
        depolar = await CeklerVeritabaniServisi().cekleriGetir(
          sayfa: _currentPage,
          ...
          aktifMi: null, // Filtre kaldırıldı
          banka: _selectedBank,
          kullanici: _selectedUser,
          baslangicTarihi: _startDate,
          bitisTarihi: _endDate,
        ...
        hasMore = depolar.length > _rowsPerPage;
      }

      if (!mounted || sorguNo != _aktifSorguNo) return;

      final List<CekModel> display = hasMore
          ? depolar.sublist(0, _rowsPerPage)
          : depolar;

      final statsFuture = CeklerVeritabaniServisi()
          .cekFiltreIstatistikleriniGetir(
      ...
      unawaited(
        statsFuture
            ...
            .catchError((e) {
              debugPrint('Çek filtre istatistikleri güncellenemedi: $e');
            }),
      );
    } catch (e) {
  ...
  Future<void> _loadMoreDetailTransactions(
    ...
    if (_detailTransactionsLoading[cek.id] == true) return;

    final existing =
        _detailTransactionsCache[cek.id] ??
        List<Map<String, dynamic>>.from(initialTransactions);
    if (existing.isEmpty) {
      setState(() => _detailTransactionsHasMore[cek.id] = false);
      return;
    }

    final dynamic lastIdRaw = existing.last['id'];
    final int? lastId = lastIdRaw is int
        ? lastIdRaw
        : int.tryParse(lastIdRaw?.toString() ?? '');
    ...
    setState(() {
      _detailTransactionsLoading[cek.id] = true;
      _detailTransactionsCache[cek.id] = existing;
      _detailTransactionsHasMore[cek.id] =
          _detailTransactionsHasMore[cek.id] ??
          (initialTransactions.length == _detailTransactionsPageSize);
    });

    try {
      final more = await CeklerVeritabaniServisi().cekIslemleriniGetir(
        cek.id,
        aramaTerimi: _searchQuery,
        baslangicTarihi: _startDate,
        bitisTarihi: _endDate,
      ...
      setState(() {
        _detailTransactionsCache[cek.id] = [...existing, ...more];
        _detailTransactionsHasMore[cek.id] =
            more.length == _detailTransactionsPageSize;
        _detailTransactionsLoading[cek.id] = false;
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _detailTransactionsLoading[cek.id] = false;
        _detailTransactionsHasMore[cek.id] = false;
      });
      debugPrint('Çek detay yükleme hatası: $e');
    }
  }
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          _buildConfigCheckbox(
                            ...
                            localVisibility,
                            'code',
                            tr('checks.table.code'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                            'name',
                            tr('checks.table.name'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                            'amount',
                            tr('checks.table.amount'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                            'currency',
                            tr('checks.table.currency'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                            'bank',
                            tr('checks.table.bank'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                            'issue_date',
                            tr('checks.table.issue_date'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                            'due_date',
                            tr('checks.table.due_date'),
                          ),
                        ],
                      ),
                      ...
                      Wrap(
                        ...
                        children: [
                          _buildConfigCheckbox(
                            ...
                            localVisibility,
                            'dt_transaction',
                            tr('checks.detail.transaction'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                            'dt_date',
                            tr('checks.detail.date'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                            'dt_contact',
                            tr('checks.detail.contact'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                            'dt_amount',
                            tr('checks.detail.amount'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                            'dt_description',
                            tr('checks.detail.description'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                            'dt_user',
                            tr('checks.detail.user'),
                          ),
                        ],
                      ),
                    ],
  ...
  List<CekModel> _filterCekler(List<CekModel> depolar) {
    if (_searchQuery.isEmpty) return depolar;

    final q = _normalizeTurkish(_searchQuery);
    return depolar.where((depo) {
      final codeMatch = _normalizeTurkish(depo.cekNo).contains(q);
  ...
  Future<void> _showAddDialog() async {
    final result = await showDialog<CekModel>(
      context: context,
      builder: (context) => const CekAlDialog(),
    );

    if (result != null) {
      try {
        await CeklerVeritabaniServisi().cekEkle(result);
        await _fetchCekler();
  ...
  Future<void> _showEditDialog(CekModel cek) async {
    final result = await showDialog<CekModel>(
      context: context,
      builder: (context) => CekAlDialog(cek: cek),
    );

    if (result != null) {
      try {
        await CeklerVeritabaniServisi().cekGuncelle(result);
        await _fetchCekler();
  ...
  Future<void> _deleteSelectedCekler() async {
    ...
    if (onay == true) {
      try {
        for (final id in _selectedIds) {
          await CeklerVeritabaniServisi().cekSil(id);
        }
  ...
  Future<void> _showCollectDialog(CekModel cek) async {
    final result = await Navigator.push(
      context,
      PageRouteBuilder(
        pageBuilder: (context, animation, secondaryAnimation) =>
            CekTahsilSayfasi(cek: cek),
  ...
  Future<void> _showEndorseDialog(CekModel cek) async {
    final result = await Navigator.push(
      context,
      PageRouteBuilder(
        pageBuilder: (context, animation, secondaryAnimation) =>
            CekCiroSayfasi(cek: cek),
        transitionsBuilder: (context, animation, secondaryAnimation, child) {
          const begin = Offset(1.0, 0.0);
          const end = Offset.zero;
          const curve = Curves.easeInOut;
  ...
  void _deleteCek(CekModel depo) async {
    final bool? onay = await showDialog<bool>(
      context: context,
      builder: (context) => OnayDialog(
        baslik: tr('common.confirmation'),
    ...
    if (onay == true) {
      try {
        await CeklerVeritabaniServisi().cekSil(depo.id);
        await _fetchCekler();
  ...
  Future<void> _showGiveCheckDialog() async {
    final result = await showDialog<CekModel>(
      context: context,
      builder: (context) => const CekVerDialog(),
    );
  ...
  Widget build(BuildContext context) {
    ...
    // Filtreleme mantığı
    List<CekModel> filteredCekler = _cachedCekler;
        ...
        child: CallbackShortcuts(
          bindings: {
            ...
            const SingleActivator(LogicalKeyboardKey.f2): () {
              ...
              final cek = _cachedCekler.firstWhere(
                (c) => c.id == _selectedRowId,
                orElse: () => _cachedCekler.first,
              );
              if (cek.id == _selectedRowId) {
                _showEditDialog(cek);
              }
            },
            ...
            const SingleActivator(LogicalKeyboardKey.f8): () {
              if (_selectedIds.isEmpty) return;
              _deleteSelectedCekler();
            },
            // F4: Çek Ver
            const SingleActivator(LogicalKeyboardKey.f4): _showGiveCheckDialog,
            // F9: Tahsil Et
            ...
            const SingleActivator(LogicalKeyboardKey.delete): () {
              ...
              if (_selectedDetailTransactionId != null &&
                  _selectedDetailCek != null) {
                _handleDetailDelete(
                  _selectedDetailCek!.id,
  ...
  void _onSelectAll(bool? value) {
    setState(() {
      if (value == true) {
        _selectedIds.addAll(_filterCekler(_cachedCekler).map((e) => e.id));
      } else {
        _selectedIds.clear();
      }
    });
  ...
  void _handleDetailDelete(int cekId, int transactionId) {
    showDialog(
      context: context,
      ...
      builder: (context) => OnayDialog(
        ...
        isDestructive: true,
        onOnay: () async {
          await CeklerVeritabaniServisi().cekIsleminiSil(transactionId);
          SayfaSenkronizasyonServisi().veriDegisti('cari');
          SayfaSenkronizasyonServisi().veriDegisti('kasalar');
          SayfaSenkronizasyonServisi().veriDegisti('bankalar');
          SayfaSenkronizasyonServisi().veriDegisti('krediKartlari');
          _detailFutures.remove(cekId);
          await _fetchCekler();
          if (mounted) {
            setState(() {
              _selectedDetailTransactionId = null;
  ...
  void _onSelectAllDetails(int cekId, bool? value) {
    setState(() {
      if (value == true) {
        _selectedDetailIds[cekId] = (_visibleTransactionIds[cekId] ?? [])
            .toSet();
      } else {
        _selectedDetailIds[cekId]?.clear();
      }
    });
  }

  void _onSelectDetailRow(int cekId, int transactionId, bool? value) {
    setState(() {
      if (_selectedDetailIds[cekId] == null) {
        _selectedDetailIds[cekId] = {};
      }
      if (value == true) {
        _selectedDetailIds[cekId]!.add(transactionId);
      } else {
        _selectedDetailIds[cekId]!.remove(transactionId);
      }
    });
  }
  ...
  Future<void> _handlePrint() async {
    ...
    try {
      ...
      final dataToProcess = mainRowIdsToProcess.isNotEmpty
          ? _cachedCekler
                .where((c) => mainRowIdsToProcess.contains(c.id))
                .toList()
          : _cachedCekler;
      ...
      for (var i = 0; i < dataToProcess.length; i++) {
        ...
        if (isExpanded || hasPrintSelectedDetails) {
          transactions = await CeklerVeritabaniServisi().cekIslemleriniGetir(
            depo.id,
            aramaTerimi: _searchQuery,
            baslangicTarihi: _startDate,
            bitisTarihi: _endDate,
        ...
        final String typeStr = depo.tur == 'Alınan Çek'
            ? '(${tr('checks.type.received_short')})'
            : '(${tr('checks.type.given_short')})';
        ...
        if (transactions.isNotEmpty) {
          txTable = DetailTable(
            title: tr('checks.detail.movements'),
            headers: [
              tr('checks.detail.type'), // İşlem
              tr('checks.detail.date'), // Tarih
              tr('checks.detail.contact'), // İlgili Cari/Kişi
              tr('checks.detail.amount'), // Tutar
              tr('checks.detail.description'), // Açıklama
              tr('checks.detail.user'), // Kullanıcı
            ],
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) => GenisletilebilirPrintPreviewScreen(
            ...
            hideFeaturesCheckbox:
                true, // Özellikler göster checkbox'ı pasif olsun
            initialShowDetails:
                _keepDetailsOpen ||
              ...
              Expanded(
                child: Text(
                  _selectedTransactionType == null
                      ...
                      : '${IslemCeviriYardimcisi.cevir(IslemTuruRenkleri.getProfessionalLabel(_selectedTransactionType!, context: 'check'))} (${_filterStats['islem_turleri']?[_selectedTransactionType] ?? 0})',
                  style: TextStyle(
                    fontSize: 14,
                    fontWeight: FontWeight.w500,
                    color: _isTransactionFilterExpanded
  ...
  Widget _buildTransactionOption(String? value, String label) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        color: isSelected ? const Color(0xFFF0F7FF) : Colors.transparent,
        child: Text(
          '${value == null ? label : IslemCeviriYardimcisi.cevir(IslemTuruRenkleri.getProfessionalLabel(value, context: 'check'))} ($count)',
          style: TextStyle(
            fontSize: 14,
            fontWeight: isSelected ? FontWeight.bold : FontWeight.w500,
            color: isSelected ? const Color(0xFF2C3E50) : Colors.black87,
  ...
  Widget _buildDesktopView(List<CekModel> depolar, BoxConstraints constraints) {
    ...
    return GenisletilebilirTablo<CekModel>(
      ...
      columns: [
        ...
        if (_columnVisibility['currency'] == true)
          GenisletilebilirTabloKolon(
            label: tr('checks.table.currency'),
            width: 100,
            alignment: Alignment.centerLeft,
            allowSorting: false,
            flex: 20,
        ...
        if (_columnVisibility['issue_date'] == true)
          GenisletilebilirTabloKolon(
            label: tr('checks.table.issue_date'),
        ...
        if (_columnVisibility['due_date'] == true)
          GenisletilebilirTabloKolon(
            label: tr('checks.table.due_date'),
            width: colBranchNameWidth,
            alignment: Alignment.centerLeft,
            allowSorting: true,
          ),
        GenisletilebilirTabloKolon(
          label: tr('checks.table.actions'),
          width: colActionsWidth,
          alignment: Alignment.centerLeft,
        ),
      ],
      expandOnRowTap: false,
      isRowSelected: (cek, index) => _selectedRowId == cek.id,
      onRowTap: (cek) {
        setState(() => _selectedRowId = cek.id);
      },
          ...
          children: [
            _buildCell(
              ...
              child: SizedBox(
                ...
                height: 20,
                child: Checkbox(
                  value: _selectedIds.contains(cek.id),
                  onChanged: (val) => _onSelectRow(val, cek.id),
                  shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(4),
                  ),
                  side: const BorderSide(color: Color(0xFFD1D1D1), width: 1),
              ...
              _buildCell(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    HighlightText(
                      text: () {
                        final String typeText = cek.tur == 'Alınan Çek'
                            ? '(${tr('checks.type.received_short')})'
                            : '(${tr('checks.type.given_short')})';
                        if (cek.tahsilat == 'Tahsil Edildi' ||
                            cek.tahsilat == 'Ödendi' ||
                            cek.tahsilat == 'Ciro Edildi' ||
                            cek.tahsilat == 'Karşılıksız') {
                          return '$typeText (${IslemCeviriYardimcisi.cevirDurum(cek.tahsilat)})';
                        }
                        return typeText;
                      }(),
                      query: _searchQuery,
                      style: TextStyle(
                        fontSize: 10,
                        color: cek.tahsilat == 'Karşılıksız'
            ...
            if (_columnVisibility['bank'] == true)
              _buildCell(
                width: 150,
                child: HighlightText(
                  text: cek.banka.isNotEmpty ? cek.banka : '-',
                  query: _searchQuery,
                  style: const TextStyle(color: Colors.black87, fontSize: 14),
                ),
              ),
            ...
            // İşlemler
            _buildCell(width: colActionsWidth, child: _buildPopupMenu(cek)),
          ],
        );
      },
      detailBuilder: (context, cek) {
        return _buildDetailView(cek);
      },
    );
  }
  ...
  Widget _buildDetailView(CekModel cek) {
    final selectedIds = _selectedDetailIds[cek.id] ?? {};
    final visibleIds = _visibleTransactionIds[cek.id] ?? [];
    final allSelected =
        visibleIds.isNotEmpty && selectedIds.length == visibleIds.length;

    // Get first character for icon
    final firstChar = cek.cariAdi.isNotEmpty
        ? cek.cariAdi[0].toUpperCase()
        : 'Ç';

    // Calculate Alınan/Verilen totals from all checks (for demo, using current check)
    final double alinanToplam = cek.tur == 'Alınan Çek' ? cek.tutar : 0.0;
    final double verilenToplam = cek.tur == 'Verilen Çek' ? cek.tutar : 0.0;
        ...
        children: [
          ...
          Row(
            children: [
              SizedBox(
                ...
                child: Checkbox(
                  value: allSelected,
                  onChanged: (val) => _onSelectAllDetails(cek.id, val),
                  shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(4),
                  ),
                  side: const BorderSide(color: Color(0xFFD1D1D1), width: 1),
            ...
            child: Row(
              children: [
                ...
                if (_columnVisibility['dt_transaction'] == true)
                  Expanded(
                    flex: 5, // Type column
                    child: _buildDetailHeader(tr('checks.detail.transaction')),
                  ),
                if (_columnVisibility['dt_date'] == true) ...[
                  const SizedBox(width: 32),
                  Expanded(
                    flex: 7, // Date column
                    child: _buildDetailHeader(tr('checks.detail.date')),
                  ),
                ],
                if (_columnVisibility['dt_contact'] == true)
                  Expanded(
                    flex: 7, // Contact column
                    child: _buildDetailHeader(tr('checks.detail.contact')),
                  ),
                ...
                if (_columnVisibility['dt_description'] == true) ...[
                  ...
                  Expanded(
                    ...
                    child: _buildDetailHeader(tr('checks.detail.description')),
                  ),
                ],
                if (_columnVisibility['dt_user'] == true) ...[
                  ...
                  Expanded(
                    flex: 3, // User column
                    child: _buildDetailHeader(tr('checks.detail.user')),
                  ),
                ],
          ...
          FutureBuilder<List<Map<String, dynamic>>>(
            future: _detailFutures.putIfAbsent(
              cek.id,
              () => CeklerVeritabaniServisi().cekIslemleriniGetir(
                cek.id,
                aramaTerimi: _searchQuery,
                baslangicTarihi: _startDate,
                bitisTarihi: _endDate,
            ...
            builder: (context, snapshot) {
              ...
              final cached = _detailTransactionsCache[cek.id];
              ...
              WidgetsBinding.instance.addPostFrameCallback((_) {
                if (mounted) {
                  _visibleTransactionIds[cek.id] = transactions
                      .map((t) => t['id'] as int)
                      .toList();
                }
              });
                    ...
                    return Column(
                      children: [
                        _buildDetailRowCells(
                          ...
                          isSelected: val,
                          onChanged: (val) =>
                              _onSelectDetailRow(cek.id, tx['id'], val),
                          onTap: () {
                            ...
                            setState(() {
                              ...
                              _selectedDetailCek = cek;
                              _selectedRowId = null;
                            });
                          },
                          transactionType: tx['type']?.toString() ?? '',
                          tahsilat: cek.tahsilat,

                          cariKod: cek.cariKod,
                          cariAdi: tx['source_dest']?.toString() ?? cek.cariAdi,
                          duzTarih: () {
                            final date = tx['date'];
                ...
                child: Row(
                  children: [
                                ...
                                child: Row(
                                  children: [
                                    Container(
                                      ...
                                      child: Text(
                                        tr(
                                          'purchase.complete.location.customer',
                                        ),
                                        style: TextStyle(
                                          fontSize: 10,
                    ...
                    SizedBox(
                      ...
                      child: Align(
                        alignment: Alignment.centerRight,
                        child: _buildTransactionPopupMenu(tx, cek),
                      ),
                    ),
                  ],
                ),
  ...
  Widget _buildTransactionPopupMenu(Map<String, dynamic> tx, CekModel cek) {
    final int id = tx['id'];
    ...
    return Theme(
      ...
      child: PopupMenuButton<String>(
        ...
        onSelected: (value) {
          if (value == 'edit') {
            _handleDetailEdit(tx, cek);
          } else if (value == 'delete') {
            _handleDetailDelete(cek.id, id);
          }
        },
      ),
    );
  ...
  Future<void> _handleDetailEdit(Map<String, dynamic> tx, CekModel cek) async {
    // Veritabanından gelen kolon isimleri 'type' ve 'description'dır.
    final String type = (tx['type'] ?? '').toString().toLowerCase();
    final String desc = (tx['description'] ?? '').toString().toLowerCase();

    debugPrint('Editing Transaction - Type: $type, Desc: $desc');

    bool? result;

    // İşlem tipine veya açıklamaya göre doğru sayfayı aç
    // Tahsilat için: 'tahsil', 'ödeme'/'odeme', 'giriş' veya verilen çek çıkışı kontrolü
    if (type.contains('tahsil') ||
        type.contains('ödeme') ||
        type.contains('odeme') ||
        type.contains('giriş') ||
        type.contains('giris') ||
        (type.contains('çıkış') && cek.tur == 'Verilen Çek') ||
        (type.contains('cikis') && cek.tur == 'Verilen Çek') ||
        desc.contains('tahsil') ||
        desc.contains('ödeme') ||
        desc.contains('odeme')) {
      result = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => CekTahsilSayfasi(cek: cek, transaction: tx),
        ),
      );
    }
    // Ciro için: 'ciro' veya alınan çek çıkışı kontrolü
    else if (type.contains('ciro') ||
        (type.contains('çıkış') && cek.tur == 'Alınan Çek') ||
        (type.contains('cikis') && cek.tur == 'Alınan Çek') ||
        desc.contains('ciro')) {
      result = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => CekCiroSayfasi(cek: cek, transaction: tx),
        ),
      );
    } else {
      // Eğer Giriş/Çıkış veya bilinmeyen bir türse uyarı ver
  ...
  Widget _buildPopupMenu(CekModel depo) {
    return Theme(
      data: Theme.of(context).copyWith(
        dividerTheme: const DividerThemeData(
          color: Color(0xFFEEEEEE),
      ...
      child: PopupMenuButton<String>(
        ...
        itemBuilder: (context) => [
          ...
          PopupMenuItem<String>(
            value: 'endorse',
            enabled: depo.tur != 'Verilen Çek', // Disable for issued checks
            height: 44,
            padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 12),
            child: Row(
              children: [
  ...
  Widget _buildMobileView(List<CekModel> filteredCekler) {
    final int safeRowsPerPage = _rowsPerPage <= 0 ? 25 : _rowsPerPage;
    final int startRecordIndex = (_currentPage - 1) * safeRowsPerPage;
    final int showingStart = filteredCekler.isEmpty ? 0 : startRecordIndex + 1;
    final int endRecord = filteredCekler.isEmpty
        ? 0
        : startRecordIndex + filteredCekler.length;
    final String totalLabel = filteredCekler.isEmpty
        ? '0'
  ...
  Widget _buildDepoCard(CekModel depo) {
  ...
  Widget _buildMobileDetails(CekModel depo) {
...
class _WarehouseFilterOverlay extends StatefulWidget {
  final CekModel? selectedWarehouse;
  final ValueChanged<CekModel?> onSelect;
...
class _WarehouseFilterOverlayState extends State<_WarehouseFilterOverlay> {
  ...
  Future<void> _fetchCekler() async {
    if (!mounted) return;
    setState(() => _isLoading = true);

    try {
      final results = await CeklerVeritabaniServisi().cekleriGetir(
