...
class _SenetlerSayfasiState extends State<SenetlerSayfasi> {
  ...
  Future<void> _fetchSenetler({bool showLoading = true}) async {
    ...
    try {
      ...
      final List<String> indexTables =
          ...
          ? const <String>['promissory_notes', 'note_transactions']
          : const <String>['promissory_notes'];
      ...
      if (hasTxFilters) {
        filterParts.add('table:note_transactions');
      }
      ...
      if (usePrimarySearch) {
        ...
        depolar = await SenetlerVeritabaniServisi().senetleriGetir(
          sayfa: _currentPage,
      ...
      final List<SenetModel> display = hasMore
  ...
  Future<void> _loadMoreDetailTransactions(
    ...
    if (_detailTransactionsLoading[senet.id] == true) return;

    final existing =
        _detailTransactionsCache[senet.id] ??
        List<Map<String, dynamic>>.from(initialTransactions);
    if (existing.isEmpty) {
      setState(() => _detailTransactionsHasMore[senet.id] = false);
      return;
    }
    ...
    setState(() {
      _detailTransactionsLoading[senet.id] = true;
      _detailTransactionsCache[senet.id] = existing;
      _detailTransactionsHasMore[senet.id] =
          _detailTransactionsHasMore[senet.id] ??
          (initialTransactions.length == _detailTransactionsPageSize);
    });

    try {
      final more = await SenetlerVeritabaniServisi().senetIslemleriniGetir(
        senet.id,
        aramaTerimi: _searchQuery,
        baslangicTarihi: _startDate,
        bitisTarihi: _endDate,
      ...
      setState(() {
        _detailTransactionsCache[senet.id] = [...existing, ...more];
        _detailTransactionsHasMore[senet.id] =
            more.length == _detailTransactionsPageSize;
        _detailTransactionsLoading[senet.id] = false;
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _detailTransactionsLoading[senet.id] = false;
        _detailTransactionsHasMore[senet.id] = false;
      });
      debugPrint('Senet detay yükleme hatası: $e');
    }
  }
        ...
        children: [
          ...
          FutureBuilder<List<Map<String, dynamic>>>(
            ...
            builder: (context, snapshot) {
              ...
              final cached = _detailTransactionsCache[senet.id];
                    ...
                    return Column(
                      children: [
                        _buildDetailRowCells(
                          ...
                          cariAdi:
                              tx['source_dest']?.toString() ?? senet.cariAdi,
                          duzTarih: () {
                            final date = tx['date'];
