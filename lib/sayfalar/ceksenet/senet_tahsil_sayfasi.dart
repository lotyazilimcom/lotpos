...
class SenetTahsilSayfasi extends StatefulWidget {
  final SenetModel senet;
  final Map<String, dynamic>? transaction;

  const SenetTahsilSayfasi({super.key, required this.senet, this.transaction});

  @override
  State<SenetTahsilSayfasi> createState() => _SenetTahsilSayfasiState();
}

class _SenetTahsilSayfasiState extends State<SenetTahsilSayfasi> {
  ...
  Future<void> _handleCollect() async {
    ...
    try {
      ...
      if (widget.transaction != null) {
        await SenetlerVeritabaniServisi().senetIsleminiGuncelle(
          islemId: widget.transaction!['id'],
          tutar: widget.senet.tutar,
          aciklama: _descriptionController.text,
          tarih: _selectedDate,
          kullanici: currentUser,
        );
      } else {
        await SenetlerVeritabaniServisi().senetTahsilEt(
          senetId: widget.senet.id,
      ...
      child: FocusTraversalGroup(
        ...
        child: Scaffold(
          ...
          appBar: AppBar(
            ...
            title: Text(
              widget.transaction != null
                  ? (widget.senet.tur.contains('Alınan')
                        ? tr('notes.actions.edit_collection_title')
                        : tr('notes.actions.edit_payment_title'))
                  : tr('notes.collect.title'),
                    ...
                    child: ConstrainedBox(
                      ...
                      child: Form(
                        ...
                        child: Column(
                          ...
                          children: [
                            _buildNoteInfoHeader(theme, compact: isCompact),
                            SizedBox(height: sectionGap),
                            _buildSection(
                              theme,
                              title: tr('notes.collect.location_info'),
  ...
  Widget _buildNoteInfoHeader(ThemeData theme, {bool compact = false}) {
    return Container(
          ...
          Row(
            children: [
              ...
              Container(
                ...
                child: Text(
                  '${FormatYardimcisi.sayiFormatlaOndalikli(widget.senet.tutar, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.fiyatOndalik)} ${widget.senet.paraBirimi}',
                  style: TextStyle(
                    color: Colors.green.shade700,
          ...
          LayoutBuilder(
            builder: (context, constraints) {
              if (constraints.maxWidth > 500) {
                  ...
                  children: [
                      ...
                      child: Column(
                        children: [
                          ...
                          _buildInfoRow(
                            ...
                            widget.senet.cariKod,
                          ),
                          _buildInfoRow(
                            tr('notes.form.issue_date.label'),
                            widget.senet.duzenlenmeTarihi,
                          ),
                          _buildInfoRow(
                            tr('notes.form.note_no.label'),
                            widget.senet.senetNo,
                          ),
                          _buildInfoRow(
                            tr('notes.form.description.label'),
                            widget.senet.aciklama,
                          ),
                        ],
                      ),
                    ),
                    ...
                    Expanded(
                      child: Column(
                        children: [
                          _buildInfoRow(
                            tr('notes.table.collection'),
                            widget.senet.tahsilat.isEmpty
                                ? tr('notes.collect.not_collected')
                                : widget.senet.tahsilat,
                          ),
                          _buildInfoRow(
                            tr('notes.form.customer_name.label'),
                            widget.senet.cariAdi,
                          ),
                          _buildInfoRow(
                            tr('notes.form.due_date.label'),
                            widget.senet.kesideTarihi,
                          ),
                          _buildInfoRow(
                            tr('notes.form.bank.label'),
                            widget.senet.banka,
                          ),
                        ],
                      ),
                ...
                return Column(
                  children: [
                    _buildInfoRow(tr('notes.table.type'), widget.senet.tur),
                    _buildInfoRow(
                      tr('notes.table.collection'),
                      widget.senet.tahsilat.isEmpty
                          ? tr('notes.collect.not_collected')
        ...
        return Column(
          children: [
            ...
            _buildDropdown<String>(
              ...
              label: tr('notes.collect.location'),
            ...
            _buildRow(isWide, [
              _buildAutocompleteField(
                ...
                label: tr('notes.collect.account_code'),
                isRequired: true,
                color: requiredColor,
              ...
              _buildTextField(
                ...
                label: tr('notes.collect.account_name'),
                isRequired: true,
                color: requiredColor,
                readOnly: true,
            ...
            _buildRow(isWide, [
              AkilliAciklamaInput(
                ...
                label: tr('notes.collect.description'),
                category: 'note_collection_description',
                color: optionalColor,
                defaultItems: [
                  tr('smart_select.note_receive.desc.1'),
                  tr('smart_select.note_receive.desc.2'),
                  tr('smart_select.note_receive.desc.3'),
                  tr('smart_select.note_receive.desc.4'),
                  tr('smart_select.note_receive.desc.5'),
                ],
              ),
              _buildDateField(
                controller: _dateController,
                label: tr('notes.collect.date'),
                onTap: _selectDate,
                isRequired: true,
                color: requiredColor,
              ),
            ...
            builder: (context, constraints) {
              ...
              final saveButton = ElevatedButton(
                ...
                child: _isLoading
                    ...
                    : Text(
                        tr(
                          widget.senet.tur == 'Verilen Senet'
    ...
    return Dialog(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            _buildDialogHeader(
              tr('notes.collect.select_cash_register'),
              tr('common.search'),
            ),
