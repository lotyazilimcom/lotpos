...
import 'kasa_ekle_dialog.dart';
import 'kasa_para_gir_sayfasi.dart';
import 'kasa_para_cik_sayfasi.dart';
...
import '../urunler_ve_depolar/urunler/urun_karti_sayfasi.dart';
...
class KasalarSayfasi extends StatefulWidget {
  final String? initialSearchQuery;
  const KasalarSayfasi({super.key, this.initialSearchQuery});

  @override
  State<KasalarSayfasi> createState() => _KasalarSayfasiState();
}

class _KasalarSayfasiState extends State<KasalarSayfasi> {
  final TextEditingController _searchController = TextEditingController();
  final FocusNode _searchFocusNode = FocusNode();
  String _searchQuery = '';
  List<KasaModel> _cachedKasalar = [];
  ...
  final Map<int, String> _pageCursors =
      {}; // [2026] Index cursor (PIT) / DB fallback: "id:<n>"
  ...
  // Tracks visible transaction IDs per account for "Select All" and keyboard nav
  ...
  static const int _detailTransactionsPageSize = 50;
  ...
  void initState() {
    ...
    _loadSettings();
    // Arama çalışması için mevcut kasaların search_tags'ını güncelle
  ...
  Future<void> _fetchKasalar({bool showLoading = true}) async {
    ...
    try {
      ...
      final String indexSortBy = switch (_sortBy) {
        ...
        'adres' => 'root_address.keyword:$dir',
        'sorumlu' => 'root_responsible.keyword:$dir',
        'telefon' => 'root_phone.keyword:$dir',
        'aktif_mi' => 'root_aktif_mi:$dir',
        _ => 'root_id:$dir',
      };

      final bool needsTxTables =
          _searchQuery.trim().isNotEmpty ||
          (_selectedUser ?? '').trim().isNotEmpty ||
          _startDate != null ||
          _endDate != null ||
          _selectedTransactionType != null;
      final List<String> indexTables = needsTxTables
          ? const <String>['cash_registers', 'cash_register_transactions']
          : const <String>['cash_registers'];
      ...
      if (_selectedDefault != null) {
        filterParts.add(
          'root_is_default:${_selectedDefault == true ? 'true' : 'false'}',
        );
      }
      if (_selectedWarehouse != null) {
        filterParts.add('root_id:${_selectedWarehouse!.id}');
      }
      final user = (_selectedUser ?? '').trim();
      if (user.isNotEmpty) {
        filterParts.add('user_name:"${osQuote(user)}"');
      }
      final txType = (_selectedTransactionType ?? '').trim();
      if (txType.isNotEmpty) {
        filterParts.add('type:"${osQuote(txType)}"');
      }
      ...
      if (usePrimarySearch) {
        ...
        final fallback = await KasalarVeritabaniServisi().kasalariGetir(
          sayfa: _currentPage,
        ...
        hasMore = fallback.length > _rowsPerPage;
        depolar = hasMore
            ? fallback.sublist(0, _rowsPerPage)
            : List<KasaModel>.from(fallback);
        if (depolar.isNotEmpty) {
          nextCursor = 'id:${depolar.last.id}';
        }
      }

      if (!mounted || sorguNo != _aktifSorguNo) return;

      final List<KasaModel> display = depolar;

      final statsFuture = KasalarVeritabaniServisi()
          .kasaFiltreIstatistikleriniGetir(
      ...
      unawaited(
        statsFuture
            ...
            .catchError((e) {
              debugPrint('Kasa filtre istatistikleri güncellenemedi: $e');
            }),
      );
    } catch (e) {
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          _buildConfigCheckbox(
                            ...
                            localVisibility,
                            'code',
                            tr('cashregisters.table.code'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                            'name',
                            tr('cashregisters.table.name'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                          ...
                          _buildConfigCheckbox(
                            ...
                            localVisibility,
                            'currency',
                            tr('cashregisters.table.currency'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                            'status',
                            tr('cashregisters.table.status'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                            'default',
                            tr('cashregisters.table.default'),
                          ),
                        ],
                      ),
  ...
  Future<void> _loadSettings() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _keepDetailsOpen =
              prefs.getBool('kasalar_keep_details_open') ?? false;
  ...
  Future<void> _toggleKeepDetailsOpen() async {
    ...
    await prefs.setBool('kasalar_keep_details_open', _keepDetailsOpen);
  }
  ...
  List<KasaModel> _filterKasalar(List<KasaModel> depolar) {
  ...
  Future<void> _showAddDialog() async {
    ...
    try {
      final settings = await GenelAyarlarVeriKaynagi().ayarlariGetir();
      if (settings.otoKasaKodu) {
        initialCode = await KasalarVeritabaniServisi().siradakiKasaKodunuGetir(
          alfanumerik: settings.otoKasaKoduAlfanumerik,
        );
      }
    } catch (e) {
    ...
    if (result != null) {
      try {
        await KasalarVeritabaniServisi().kasaEkle(result);
        SayfaSenkronizasyonServisi().veriDegisti('kasa');
        await _fetchKasalar();
  ...
  Future<void> _showEditDialog(KasaModel kasa) async {
    ...
    if (result != null) {
      try {
        await KasalarVeritabaniServisi().kasaGuncelle(result);
        SayfaSenkronizasyonServisi().veriDegisti('kasa');
        await _fetchKasalar();
  ...
  Future<void> _showDepositDialog(KasaModel kasa) async {
    final result = await Navigator.push<bool>(
      context,
      PageRouteBuilder(
        pageBuilder: (context, animation, secondaryAnimation) =>
            KasaParaGirSayfasi(kasa: kasa),
  ...
  Future<void> _showWithdrawDialog(KasaModel kasa) async {
    final result = await Navigator.push<bool>(
      context,
      PageRouteBuilder(
        pageBuilder: (context, animation, secondaryAnimation) =>
            KasaParaCikSayfasi(kasa: kasa),
        transitionsBuilder: (context, animation, secondaryAnimation, child) {
          const begin = Offset(1.0, 0.0);
          const end = Offset.zero;
          const curve = Curves.easeInOut;
  ...
  void _deleteKasa(KasaModel depo) async {
    ...
    if (onay == true) {
      try {
        await KasalarVeritabaniServisi().kasaSil(depo.id);
        SayfaSenkronizasyonServisi().veriDegisti('kasa');
        await _fetchKasalar();
  ...
  Widget build(BuildContext context) {
    ...
    // Filtreleme mantığı
    List<KasaModel> filteredKasalar = _cachedKasalar;
        ...
        child: CallbackShortcuts(
          bindings: {
            ...
            const SingleActivator(LogicalKeyboardKey.f2): () {
              ...
              final kasa = _cachedKasalar.firstWhere(
                (k) => k.id == _selectedRowId,
                orElse: () => _cachedKasalar.first,
              );
              if (kasa.id == _selectedRowId) {
                _showEditDialog(kasa);
              }
            },
            ...
            const SingleActivator(LogicalKeyboardKey.f6): () {
              ...
              if (kasa.id == _selectedRowId) {
                _kasaDurumDegistir(kasa, !kasa.aktifMi);
              }
            },
            // F7: Yazdır
              ...
              LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  if (forceMobile || constraints.maxWidth < 800) {
                    return _buildMobileView(filteredKasalar);
                  } else {
                    return _buildDesktopView(filteredKasalar, constraints);
                  }
                },
              ),
              if (_isLoading)
  ...
  Future<void> _loadMoreDetailTransactions(
    ...
    if (_detailTransactionsLoading[kasa.id] == true) return;

    final existing =
        _detailTransactionsCache[kasa.id] ??
        List<Map<String, dynamic>>.from(initialTransactions);
    if (existing.isEmpty) return;

    final int? lastId = existing.last['id'] as int?;
    if (lastId == null) {
      setState(() => _detailTransactionsHasMore[kasa.id] = false);
      return;
    }

    setState(() {
      _detailTransactionsLoading[kasa.id] = true;
      _detailTransactionsCache[kasa.id] = existing;
      _detailTransactionsHasMore[kasa.id] =
          _detailTransactionsHasMore[kasa.id] ??
          (initialTransactions.length == _detailTransactionsPageSize);
    });

    try {
      final more = await KasalarVeritabaniServisi().kasaIslemleriniGetir(
        kasa.id,
        aramaTerimi: _searchQuery,
        baslangicTarihi: _startDate,
        bitisTarihi: _endDate,
      ...
      setState(() {
        _detailTransactionsCache[kasa.id] = [...existing, ...more];
        _detailTransactionsHasMore[kasa.id] =
            more.length == _detailTransactionsPageSize;
        _detailTransactionsLoading[kasa.id] = false;
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _detailTransactionsLoading[kasa.id] = false;
        _detailTransactionsHasMore[kasa.id] = false;
      });
      debugPrint('Kasa detay yükle hatası: $e');
    }
  }
  ...
  Future<void> _kasaDurumDegistir(KasaModel depo, bool aktifMi) async {
    try {
      final yeniDepo = depo.copyWith(aktifMi: aktifMi);
      await KasalarVeritabaniServisi().kasaGuncelle(yeniDepo);
      await _fetchKasalar(showLoading: false);
  ...
  Future<void> _kasaVarsayilanDegistir(KasaModel depo, bool varsayilan) async {
    try {
      await KasalarVeritabaniServisi().kasaVarsayilanDegistir(
        depo.id,
        varsayilan,
      );
      await _fetchKasalar(showLoading: false);
      ...
      MesajYardimcisi.basariGoster(
        context,
        varsayilan
            ? tr('common.success_default_made') // "Varsayılan yapıldı"
            : tr('common.success_default_removed'), // "Varsayılan kaldırıldı"
      );
    } catch (e) {
      if (!mounted) return;
      MesajYardimcisi.hataGoster(context, '${tr('common.error')}: $e');
    ...
    try {
      ...
      for (var i = 0; i < dataToProcess.length; i++) {
        ...
        final isExpanded =
            ...
            _expandedMobileIds.contains(depo.id) ||
        ...
        if (isExpanded || hasPrintSelectedDetails) {
          transactions = await KasalarVeritabaniServisi().kasaIslemleriniGetir(
            depo.id,
            aramaTerimi: _searchQuery,
            baslangicTarihi: _startDate,
            bitisTarihi: _endDate,
        ...
        final mainRow = [
          ...
          depo.varsayilan ? tr('common.yes') : tr('common.no'),
        ];
        ...
        if (isExpanded || hasSelectedDetails) {
          ...
          details = {
            tr('cashregisters.table.info1'): depo.bilgi1.isNotEmpty
                ? depo.bilgi1
                : '-',
            tr('cashregisters.table.info2'): depo.bilgi2.isNotEmpty
                ? depo.bilgi2
                : '-',
            tr(
              'products.transaction.type.input',
            ): '${FormatYardimcisi.sayiFormatlaOndalikli(totalGirdi, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.fiyatOndalik)} ${depo.paraBirimi}',
            tr(
              'products.transaction.type.output',
            ): '${FormatYardimcisi.sayiFormatlaOndalikli(totalCikti, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.fiyatOndalik)} ${depo.paraBirimi}',
            tr(
              'common.total',
            ): '${FormatYardimcisi.sayiFormatlaOndalikli(toplam, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.fiyatOndalik)} ${depo.paraBirimi}',
          };
        }
        ...
        if (transactions.isNotEmpty) {
          txTable = DetailTable(
            title: tr('cashregisters.detail.timeline'),
            headers: [
              tr('cashregisters.detail.transaction'),
              tr('cashregisters.detail.date'),
            ...
            data: transactions.map((t) {
              ...
              if (lowRef.startsWith('retail-')) {
                ilgiliHesap = 'Perakende Satış Yapıldı (Nakit)';
              }
              ...
              return <String>[
                ...
                formattedDate,
                ilgiliHesap,
                '${FormatYardimcisi.sayiFormatlaOndalikli(amount, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.fiyatOndalik)} ${depo.paraBirimi}',
                description,
                user,
              ];
            }).toList(),
    ...
    return Padding(
      ...
      child: ResponsiveFilterRow(
        items: [
          ...
          ResponsiveFilterItem(
            child: _buildDefaultFilter(width: double.infinity),
          ),
          ResponsiveFilterItem(
  ...
  Widget _buildDesktopView(
    ...
    final colBalanceWidth = _calculateColumnWidth(
      tr('cashregisters.table.balance'),
      sortable: true,
    );
    ...
    return GenisletilebilirTablo<KasaModel>(
      ...
      onPageChanged: (page, rowsPerPage) {
        setState(() {
          final bool rowsChanged = rowsPerPage != _rowsPerPage;
      ...
      columns: [
        ...
        if (_columnVisibility['code'] == true)
          GenisletilebilirTabloKolon(
            label: tr('cashregisters.table.code'),
            width: colCodeWidth,
            flex: 20,
            alignment: Alignment.centerLeft,
            allowSorting: true,
        ...
        if (_columnVisibility['name'] == true)
          GenisletilebilirTabloKolon(
            label: tr('cashregisters.table.name'),
            width: colNameWidth,
            flex: 25,
            alignment: Alignment.centerLeft,
            allowSorting: true,
        ...
        if (_columnVisibility['balance'] == true)
          GenisletilebilirTabloKolon(
            label: tr('cashregisters.table.balance'),
        ...
        if (_columnVisibility['currency'] == true)
          GenisletilebilirTabloKolon(
            label: tr('cashregisters.table.currency'),
        ...
        if (_columnVisibility['status'] == true)
          GenisletilebilirTabloKolon(
            label: tr('cashregisters.table.status'),
        ...
        if (_columnVisibility['default'] == true)
          GenisletilebilirTabloKolon(
            label: tr('cashregisters.table.default'),
            width: colDefaultWidth,
            alignment: Alignment.center,
            allowSorting: true,
          ),
        GenisletilebilirTabloKolon(
          label: tr('cashregisters.table.actions'),
      ...
      data: depolar,
      isRowSelected: (kasa, index) => _selectedRowId == kasa.id,
      expandOnRowTap: false,
      onRowTap: (kasa) {
        setState(() {
          _selectedRowId = kasa.id;
        });
      },
      ...
      rowBuilder: (context, kasa, index, isExpanded, toggleExpand) {
        ...
        return Row(
          children: [
            ...
            // İşlemler
            _buildCell(width: colActionsWidth, child: _buildPopupMenu(kasa)),
          ],
        );
      },
      detailBuilder: (context, kasa) {
        final selectedIds = _selectedDetailIds[kasa.id] ?? {};
        final visibleIds = _visibleTransactionIds[kasa.id] ?? [];
        final allSelected =
            visibleIds.isNotEmpty && selectedIds.length == visibleIds.length;

        // Kasa ilk harfi
        final String firstChar = kasa.ad.isNotEmpty
            ? kasa.ad[0].toUpperCase()
            : '?';
        ...
        return Container(
          ...
          child: FutureBuilder<List<Map<String, dynamic>>>(
            future: _detailFutures.putIfAbsent(
              kasa.id,
              () => KasalarVeritabaniServisi().kasaIslemleriniGetir(
                kasa.id,
                aramaTerimi: _searchQuery,
                baslangicTarihi: _startDate,
                bitisTarihi: _endDate,
            ...
            builder: (context, snapshot) {
              ...
              final cached = _detailTransactionsCache[kasa.id];
              ...
              WidgetsBinding.instance.addPostFrameCallback((_) {
                if (mounted) {
                  _visibleTransactionIds[kasa.id] = transactions
                      .map((t) => t['id'] as int)
                      .toList();
                }
              });
              ...
              for (var t in transactions) {
                ...
                final bool isIncoming =
                    t['islem'] == 'Tahsilat' || t['isIncoming'] == true;
                // Note: logic refined based on available data, using simple check for demo
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      SizedBox(
                                        ...
                                        child: Text(
                                          FormatYardimcisi.sayiFormatlaOndalikli(
                                            kasa.bakiye, // Use actual balance from model
                                            binlik: _genelAyarlar.binlikAyiraci,
                                            ondalik:
                                                _genelAyarlar.ondalikAyiraci,
                                            decimalDigits:
                          ...
                          children: [
                            Expanded(
                              child: _buildFeatureItemWithIcon(
                                tr('cashregisters.table.info1'),
                                kasa.bilgi1.isNotEmpty ? kasa.bilgi1 : '-',
                            ...
                            Expanded(
                              child: _buildFeatureItemWithIcon(
                                tr('cashregisters.table.info2'),
                                kasa.bilgi2.isNotEmpty ? kasa.bilgi2 : '-',
                                icon: Icons.info_outline,
                              ),
                            ),
                          ],
                          ...
                          children: [
                            SizedBox(
                              ...
                              child: Checkbox(
                                value: allSelected,
                                onChanged: (val) =>
                                    _onSelectAllDetails(kasa.id, val),
                                shape: RoundedRectangleBorder(
                                  borderRadius: BorderRadius.circular(4),
                                ),
                            ...
                            Text(
                              tr('cashregisters.detail.timeline'),
                              style: TextStyle(
                                fontSize: 13,
                                fontWeight: FontWeight.bold,
                  ...
                  Column(
                    children: [
                      ...transactions.asMap().entries.map((entry) {
                        ...
                        bool displayIsIncoming = tx['isIncoming'] == true;
                        ...
                        if (integrationRef != null) {
                          ...
                          } else if (lowRef == 'opening_stock' ||
                              desc.contains('açılış') ||
                              desc.contains('acilis')) {
                            displayName = 'Açılış Stoğu';
                          } else if (lowRef.contains('production') ||
                              desc.contains('üretim') ||
                              desc.contains('uretim')) {
                            displayName = 'Üretim';
                        ...
                        return Column(
                          children: [
                            ...
                            _buildDetailRowCells(
                              ...
                              isFocused: isFocused,
                              onChanged: (val) =>
                                  _onSelectDetailRow(kasa.id, tx['id'], val),
                              onTap: () {
  ...
  Future<void> _showEditTransactionDialog(int id, KasaModel kasa) async {
    final transaction = await KasalarVeritabaniServisi().kasaIslemGetir(id);
    if (transaction == null) {
      return;
    }

    // isIncoming depends on 'islem' which holds the type (Tahsilat/Ödeme/etc) or 'isIncoming' boolean logic.
    // The service returns 'type' in 'islem' and 'isIncoming' boolean.
    // However, for editing we need to call the correct page.
  ...
  Future<void> _openRetailSaleUrunKarti(String integrationRef) async {
    try {
      final urun = await UrunlerVeritabaniServisi().urunGetirByIntegrationRef(
        integrationRef,
      );
      if (!mounted) return;
  ...
  Widget _buildTransactionPopupMenu(
      ...
      return Center(
        child: Tooltip(
          ...
          child: InkWell(
            mouseCursor: WidgetStateMouseCursor.clickable,
            onTap: () async {
              await _openRetailSaleUrunKarti(integrationRef);
            },
            borderRadius: BorderRadius.circular(6),
            child: Container(
    ...
    if (integrationRef.isNotEmpty) {
      if (integrationRef.startsWith('CARI-') ||
        ...
        // Transfer ise diğer tarafı bulmamız lazım ama basitçe ilgili modüle atalım
        // Kasa sayfasındayız, eğer kaynak Banka ise Banka'ya gitmeli
        final String kaynakAdi = (tx['kaynak_adi'] ?? '')
            .toString()
            .toLowerCase();
        if (kaynakAdi.contains('banka')) {
    ...
    // Eğer hedef sayfa belirlendiyse ve bu sayfa KENDİ sayfamız (Kasa) değilse link göster
    // Kasa sayfası indexi = 13
    if (targetIndex != null && targetIndex != 13) {
      return Center(
        child: Tooltip(
          message: tr('common.go_to_related_page'),
          child: InkWell(
    ...
    return Theme(
      ...
      child: PopupMenuButton<String>(
        ...
        onSelected: (value) {
          if (value == 'edit') {
            _showEditTransactionDialog(id, kasa);
            ...
            showDialog(
              ...
              builder: (context) => OnayDialog(
                ...
                isDestructive: true,
                onOnay: () async {
                  await KasalarVeritabaniServisi().kasaIslemSil(id);
                  _fetchKasalar(); // Refresh main table as balance changed
                },
              ),
            );
          }
  ...
  void _handleDetailDelete(int id, KasaModel kasa) {
    showDialog(
      context: context,
      barrierDismissible: true,
      barrierColor: Colors.black.withValues(alpha: 0.35),
      ...
      child: PopupMenuButton<String>(
          ...
          PopupMenuItem<String>(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 12),
                Text(
                  tr('cashregisters.actions.deposit'),
                  style: const TextStyle(
                    color: Color(0xFF28A745),
                    fontWeight: FontWeight.w600,
                    fontSize: 14,
          ...
          PopupMenuItem<String>(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 12),
                Text(
                  tr('cashregisters.actions.withdraw'),
                  style: const TextStyle(
                    color: Color(0xFFDC3545),
                    fontWeight: FontWeight.w600,
                    fontSize: 14,
          ...
          PopupMenuItem<String>(
            ...
            child: Row(
              children: [
                ...
                Text(
                  depo.varsayilan
                      ? tr('cashregisters.actions.unmake_default')
                      : tr('cashregisters.actions.make_default'),
                  style: const TextStyle(
                    color: Color(0xFF2C3E50),
                    fontWeight: FontWeight.w600,
                    fontSize: 14,
        ...
        onSelected: (value) {
          if (value == 'deposit') {
            ...
            _kasaDurumDegistir(depo, false);
          } else if (value == 'activate') {
            _kasaDurumDegistir(depo, true);
          } else if (value == 'make_default') {
            _kasaVarsayilanDegistir(depo, true);
          } else if (value == 'unmake_default') {
            _kasaVarsayilanDegistir(depo, false);
          } else if (value == 'delete') {
            _deleteKasa(depo);
          }
        },
  ...
  Widget _buildMobileTopActionRow() {
    ...
    final String addLabel = isNarrow ? 'Ekle' : tr('cashregisters.add');
  ...
  Widget _buildMobileView(List<KasaModel> filteredKasalar) {
    final int safeRowsPerPage = _rowsPerPage <= 0 ? 25 : _rowsPerPage;
    final int startRecordIndex = (_currentPage - 1) * safeRowsPerPage;
    final int showingStart = filteredKasalar.isEmpty ? 0 : startRecordIndex + 1;
    final int endRecord = filteredKasalar.isEmpty
        ? 0
        : startRecordIndex + filteredKasalar.length;
    final String totalLabel = filteredKasalar.isEmpty
        ? '0'
            ...
            Expanded(
              child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  return _buildDepoCard(filteredKasalar[index]);
                },
              ),
            ),
            if (!isKeyboardVisible)
  ...
  Widget _buildMobileDetails(KasaModel depo) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        final initialTransactions = snapshot.data ?? [];
        final cached = _detailTransactionsCache[depo.id];
        final transactions = cached ?? initialTransactions;
        final bool isLoadingMore = _detailTransactionsLoading[depo.id] == true;
        final bool hasMore =
            _detailTransactionsHasMore[depo.id] ??
            (initialTransactions.length == _detailTransactionsPageSize);
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          body = Column(
            children: transactions.map((tx) {
              // Determine in/out direction reliably (service may only mark "Tahsilat" as incoming)
              final String rawType = tx['islem']?.toString() ?? '';
              final String lowType = rawType.toLowerCase();
              ...
              // Build a professional + localized title similar to desktop
              final String? integrationRef = tx['integration_ref']?.toString();
              ...
              if (integrationRef != null) {
                ...
                if (lowRef.startsWith('sale-') ||
                  ...
                  title = 'Alış Yapıldı';
                    ...
                    descLower.contains('açılış') ||
                    descLower.contains('acilis')) {
                  title = 'Açılış Stoğu';
                } else if (lowRef.contains('production') ||
                    descLower.contains('üretim') ||
                    descLower.contains('uretim')) {
                  title = 'Üretim';
                } else if (lowRef.contains('transfer') ||
                    descLower.contains('devir')) {
                  title = 'Devir';
                } else if (lowRef.startsWith('cheque') ||
                    lowRef.startsWith('cek-')) {
                  isCheckNote = true;
              ...
              // Personel Ödemesi label (UX parity with desktop)
              ...
              return _buildMobileTransactionRow(
                ...
                date: formattedDate,
                relatedName: tx['yerAdi']?.toString() ?? '',
                relatedCode: tx['yerKodu']?.toString() ?? '',
                amount: amount,
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: hideActionsMenu
                          ? const SizedBox.shrink()
                          : _buildTransactionPopupMenu(id, kasa, transaction),
                    ),
                  ],
                ),
              ),
  ...
  String _getSourceSuffix(
    ...
    // If it's a direct entry (no destination), default to current page type (Kasa)
    if (locationType.isEmpty && locationName.isEmpty) return '(Kasa)';
    ...
    if (integrationRef != null) {
      ...
      if (lowRef.startsWith('retail-')) return '(Nakit)';
      ...
      if (integrationRef.startsWith('AUTO-TR-')) {
        ...
        return '(Kasa)'; // Default for Kasa page
      }
    ...
    // Default Cari for Cari Hesap type if it has a name
    if (locationType == 'Cari Hesap' && locationName.isNotEmpty) {
      return '(Cari)';
    }

    // Default for Kasa page entries
    return '(Kasa)';
  }
...
class _WarehouseFilterOverlay extends StatefulWidget {
  final KasaModel? selectedWarehouse;
  final ValueChanged<KasaModel?> onSelect;
...
class _WarehouseFilterOverlayState extends State<_WarehouseFilterOverlay> {
  ...
  Future<void> _fetchKasalar() async {
    if (!mounted) return;
    setState(() => _isLoading = true);

    try {
      final results = await KasalarVeritabaniServisi().kasaAra(
