...
import 'siparis_donustur_dialog.dart';
...
class SiparislerSayfasi extends StatefulWidget {
  final String tur;
  const SiparislerSayfasi({super.key, required this.tur});
...
class _SiparislerSayfasiState extends State<SiparislerSayfasi> {
  ...
  final Set<int> _mobileExpandedOrderIds = {};
  ...
  Future<void> _fetchSiparisler({
    ...
    try {
      final servis = SiparislerVeritabaniServisi();
      ...
      String? osEpochRange(
        ...
        if (s != null && e != null) return '$field:[$s TO $e}';
        if (s != null) return '$field:[$s TO *]';
        return '$field:[* TO $e}';
      }

      final String dir = _sortAscending ? 'asc' : 'desc';
      final String indexSortBy = switch (sortBy) {
        'tarih' => 'root_date:$dir',
        'tutar' => 'root_amount:$dir',
        'durum' => 'root_status:$dir',
        _ => 'root_id:$dir',
      };
      ...
      final statsFuture = servis.siparisFiltreIstatistikleriniGetir(
      ...
      List<SiparisModel> results = [];
      bool hasMore = false;
      String? nextCursor;

      final filterParts = <String>[];
      final tur = (_selectedType ?? '').trim();
      if (tur.isNotEmpty) {
        filterParts.add('root_type:"${osQuote(tur)}"');
      }
      final durum = (_selectedStatus ?? '').trim();
      if (durum.isNotEmpty) {
        filterParts.add('root_status:"${osQuote(durum)}"');
      }
      final account = (_selectedAccount ?? '').trim();
      if (account.isNotEmpty) {
        filterParts.add('root_account:"${osQuote(account)}"');
      }
      final user = (_selectedUser ?? '').trim();
      if (user.isNotEmpty) {
        filterParts.add('root_user:"${osQuote(user)}"');
      }
      final dateRange = osEpochRange('root_date', _startDate, _endDate);
      ...
      if (unit.isNotEmpty) {
        filterParts.add('unit:"${osQuote(unit)}"');
      }
      ...
      final bool needsItemTables =
          _searchQuery.trim().isNotEmpty ||
          _selectedWarehouse != null ||
          unit.isNotEmpty;

      final List<String> indexTables = needsItemTables
          ? const <String>['orders', 'order_items']
          : const <String>['orders'];
      final bool usePrimarySearch = _searchQuery.trim().length >= 2;

      if (usePrimarySearch) {
        final primary = await AramaPrimaryPath.fetchPageIndexFirst<SiparisModel>(
          query: _searchQuery,
          ...
          endDate: _endDate,
          dbFetchByIds: (ids) => servis.siparisleriGetir(
            sayfa: 1,
        ...
        results = await servis.siparisleriGetir(
          sayfa: _currentPage,
        ...
        hasMore = results.length > _rowsPerPage;
        if (hasMore) results = results.sublist(0, _rowsPerPage);
      }

      if (!mounted || sorguNo != _aktifSorguNo) return;

      final List<SiparisModel> display = results;

      if (mounted) {
        // Find indices that match in hidden fields (sub-table) and should be auto-expanded
        final newAutoExpandedIndices = display
            .asMap()
            .entries
      ...
      unawaited(
        statsFuture
            ...
            .catchError((e) {
              debugPrint('Sipariş filtre istatistikleri güncellenemedi: $e');
            }),
      );
    } catch (e) {
  ...
  void _onSearch(String query) {
    ...
    final effectiveQuery = trimmed.isNotEmpty && trimmed.length < 3 ? '' : raw;
    if (_mobileSearchController.text != query) {
      _mobileSearchController.text = query;
  ...
  void _onSelectAll(bool? value) {
    setState(() {
      ...
      if (value == true) {
        _selectedIds = _siparisler.map((s) => s.id).toSet();
      } else {
        _selectedIds.clear();
      }
    });
  ...
  void _onSelectAllDetails(int orderId, bool? value) {
    setState(() {
      if (value == true) {
        _selectedDetailIds[orderId] = (_visibleProductIds[orderId] ?? [])
            .toSet();
      } else {
        _selectedDetailIds[orderId]?.clear();
      }
    });
  }

  void _onSelectDetailRow(int orderId, int productId, bool? value) {
    setState(() {
      final selected = _selectedDetailIds[orderId] ?? {};
      if (value == true) {
        selected.add(productId);
      } else {
        selected.remove(productId);
      }
      _selectedDetailIds[orderId] = selected;
    });
  }

  Future<void> _siparisOlustur() async {
    final result = await Navigator.push(
      context,
      PageRouteBuilder(
        pageBuilder: (context, animation, secondaryAnimation) =>
            SiparisEkleSayfasi(tur: widget.tur),
  ...
  Future<void> _siparisSil(SiparisModel order) async {
    showDialog(
      context: context,
      builder: (dialogContext) => OnayDialog(
        ...
        onOnay: () async {
          ...
          final basarili = await SiparislerVeritabaniServisi().siparisSil(
            order.id,
          );
          if (mounted) {
            setState(() => _isLoading = false);
    ...
    return Scaffold(
      ...
      body: Focus(
        ...
        child: CallbackShortcuts(
          bindings: {
            const SingleActivator(LogicalKeyboardKey.f1): _siparisOlustur,
            const SingleActivator(LogicalKeyboardKey.f2): () {
              if (_selectedRowId != null) {
                final order = _siparisler.firstWhereOrNull(
                  (o) => o.id == _selectedRowId,
                );
                if (order != null) _siparisDuzenle(order);
              }
            },
            const SingleActivator(LogicalKeyboardKey.f3): () =>
                _searchFocusNode.requestFocus(),
            const SingleActivator(LogicalKeyboardKey.f4): _siparisleriAl,
            const SingleActivator(LogicalKeyboardKey.f5): () {
              if (_selectedDetailProductId != null &&
                  _selectedDetailOrder != null &&
                  _editingField == null) {
                _startDetailEdit();
              } else {
                _fetchSiparisler();
            ...
            const SingleActivator(LogicalKeyboardKey.f6): () {
              if (_selectedRowId != null) {
                ...
                if (order != null) _showQuickAddProductDrawer(order);
              } else {
                MesajYardimcisi.uyariGoster(
                  context,
                  tr('orders.quick_add.error.select_order'),
                );
              }
            },
            const SingleActivator(LogicalKeyboardKey.f7): _handlePrint,
            const SingleActivator(LogicalKeyboardKey.f8): () {
              if (_selectedIds.isNotEmpty) _deleteSelectedSiparisler();
            },
            ...
            const SingleActivator(LogicalKeyboardKey.f9): () {
              if (_selectedRowId != null) {
                ...
                if (order != null) _siparisAl(order);
              }
            },
            const SingleActivator(LogicalKeyboardKey.delete): () {
              if (_selectedDetailProductId != null &&
                  _selectedDetailOrder != null) {
                final item = _selectedDetailOrder!.urunler.firstWhereOrNull(
                  (u) => u.id == _selectedDetailProductId,
                );
                if (item != null) _deleteOrderItem(_selectedDetailOrder!, item);
              } else if (_selectedRowId != null) {
                final order = _siparisler.firstWhereOrNull(
                  (o) => o.id == _selectedRowId,
                );
                if (order != null) _siparisSil(order);
              }
            },
          },
          child: Stack(
  ...
  Widget _buildMobileView() {
    ...
    final int showingStart = _siparisler.isEmpty ? 0 : startRecordIndex + 1;
    final int endRecord = _siparisler.isEmpty
        ? 0
        : startRecordIndex + _siparisler.length;
    final String totalLabel = _siparisler.isEmpty
        ? '0'
                ...
                child: _siparisler.isEmpty
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) =>
                            _buildMobileOrderCard(_siparisler[index]),
                      ),
              ),
              if (!isKeyboardVisible)
                Padding(
  ...
  Widget _buildMobileTopActionRow() {
    ...
    return Row(
      children: [
        ...
        Expanded(
          child: _buildMobileActionButton(
            label: tr('orders.receive_order'),
            icon: Icons.bolt_rounded,
            color: const Color(0xFFF39C12),
            textColor: Colors.white,
            borderColor: Colors.transparent,
            onTap: _siparisleriAl,
            enabled: isReceiveEnabled,
          ),
        ),
        const SizedBox(width: 8),
  ...
  Widget _buildMobileActionButton({
    ...
    final bool effectiveEnabled = enabled && !printDisabled;
  ...
  Widget _buildMobileOrderCard(SiparisModel order) {
    final bool isSelected =
        _selectedRowId == order.id || _selectedIds.contains(order.id);
    final bool isExpanded = _mobileExpandedOrderIds.contains(order.id);
              ...
              Row(
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: () => _showQuickAddProductDrawer(order),
                    icon: const Icon(
                  ...
                  Expanded(
                    child: OutlinedButton.icon(
                      onPressed: () {
                        setState(() {
                          if (isExpanded) {
                            _mobileExpandedOrderIds.remove(order.id);
                          } else {
                            _mobileExpandedOrderIds.add(order.id);
                          }
                        });
                      },
              ...
              AnimatedSize(
                ...
                child: !isExpanded
                    ...
                    : Padding(
                        ...
                        child: _buildMobileOrderProducts(order),
                      ),
              ),
            ],
          ),
  ...
  Widget _buildMobileOrderProducts(SiparisModel order) {
  ...
  Widget _buildDesktopView() {
    final bool allSelected =
        _isSelectAllActive ||
        (_siparisler.isNotEmpty &&
            _siparisler.every((u) => _selectedIds.contains(u.id)));

    return GenisletilebilirTablo<SiparisModel>(
      title: tr('nav.orders_quotes.orders'),
  ...
  Future<void> _handlePrint() async {
    ...
    try {
      ...
      if (hasMainSelection) mainRowIdsToProcess.addAll(_selectedIds);
      ...
      for (var i = 0; i < dataToProcess.length; i++) {
        ...
        final isExpanded =
            ...
            _mobileExpandedOrderIds.contains(order.id);
        ...
        if (order.urunler.isNotEmpty) {
          ...
          if (itemsToShow.isNotEmpty) {
            subTable = DetailTable(
              title: tr('orders.products_in_order'),
              headers: [
        ...
        rows.add(
          ExpandableRowData(
            ...
            isExpanded: isExpanded || hasSpecificDetailSelection,
  ...
  Widget _buildPrintButton() {
    final bool printDisabled = YazdirmaErisimKontrolu.mobilBulutYazdirmaPasif;
    return MouseRegion(
      ...
      child: MouseRegion(
        ...
        hitTestBehavior: HitTestBehavior.deferToChild,
        child: GestureDetector(
          onTap: printDisabled ? null : _handlePrint,
          child: Container(
            height: 40,
            ...
            child: Row(
              ...
              children: [
                Icon(
                  ...
                  size: 18,
                  color: printDisabled ? Colors.grey.shade400 : Colors.black87,
                ),
                const SizedBox(width: 8),
                Text(
                  _selectedIds.isNotEmpty ||
  ...
  Widget _buildDetailView(SiparisModel order) {
        ...
        children: [
          Row(
            children: [
              ...
              _buildDetailHeaderRow(tr('orders.products_in_order')),
              const Spacer(),
  ...
  Widget _buildDetailHeaderRow(String title) {
    return Row(
      children: [
        Icon(Icons.shopping_cart_outlined, size: 20, color: Color(0xFF2C3E50)),
        const SizedBox(width: 8),
        Text(
          title,
          style: const TextStyle(
  ...
  Widget _buildProductsSubTable(SiparisModel order) {
  ...
  Widget _buildSubTableRow(
    ...
    final bool isFocused =
        ...
        (_selectedDetailProductId == u.id &&
            _selectedDetailOrder?.id == order.id);
  ...
  void _showQuickAddProductDrawer(SiparisModel order) {
    showGeneralDialog(
      context: context,
          ...
          child: Material(
            ...
            child: Container(
              ...
              child: _QuickProductAddPanel(
                ...
                onSuccess: () {
                  Navigator.pop(context);
                  _fetchSiparisler(force: true, silent: true);
                },
              ),
            ),
          ),
  ...
  Widget _buildPopupMenu(SiparisModel order) {
    return Theme(
      ...
      child: PopupMenuButton<String>(
        ...
        itemBuilder: (context) => [
          ...
          PopupMenuItem<String>(
            ...
            child: Row(
              children: [
                Icon(Icons.delete_outline, size: 20, color: Color(0xFFEA4335)),
                const SizedBox(width: 12),
                Text(
                  tr('common.delete'),
                  style: const TextStyle(
  ...
  Future<void> _siparisDuzenle(SiparisModel order) async {
    final result = await Navigator.push(
      context,
      PageRouteBuilder(
        pageBuilder: (context, animation, secondaryAnimation) =>
            SiparisEkleSayfasi(tur: widget.tur, initialOrder: order),
        transitionsBuilder: (context, animation, secondaryAnimation, child) {
          const begin = Offset(1.0, 0.0);
          const end = Offset.zero;
          const curve = Curves.easeInOut;
  ...
  void _updateOrderItem(
    ...
    setState(() {
      ...
      final orderIndex = _siparisler.indexWhere((o) => o.id == order.id);
      if (orderIndex != -1) {
        final List<SiparisUrunModel> newUrunler = updatedUrunlerJson.map((j) {
        ...
        final updatedOrder = _siparisler[orderIndex].copyWith(
          tutar: newOrderTotal,
          urunler: newUrunler,
        );
        _siparisler[orderIndex] = updatedOrder;
      }
    });
    ...
    try {
      ...
      // We already updated _siparisler optimistically.
      // Cursor pagination: COUNT(*) yok. UI footer/badge zaten end+ ile çalışır.
    } catch (e) {
      if (mounted) {
        MesajYardimcisi.hataGoster(
          context,
  ...
  void _deleteOrderItem(SiparisModel order, SiparisUrunModel item) {
    showDialog(
      context: context,
      builder: (dialogContext) => OnayDialog(
        baslik: tr('common.confirmation'),
        ...
        onOnay: () async {
          ...
          setState(() {
            final orderIndex = _siparisler.indexWhere((o) => o.id == order.id);
            if (orderIndex != -1) {
              final List<SiparisUrunModel> newUrunler = updatedUrunlerJson.map((
                j,
              ) {
                return SiparisUrunModel(
                  id: j['id'],
              ...
              _siparisler[orderIndex] = _siparisler[orderIndex].copyWith(
                tutar: newOrderTotal,
                urunler: newUrunler,
              );
  ...
  void _startDetailEdit() {
    if (_selectedDetailProductId == null || _selectedDetailOrder == null) {
      return;
    }
    setState(() {
      _editingIndex = _selectedDetailProductId;
      _editingOrderId = _selectedDetailOrder!.id;
  ...
  Future<void> _siparisAl(SiparisModel order) async {
    if (order.durum == tr('orders.status.converted')) {
      MesajYardimcisi.bilgiGoster(
        context,
        tr('orders.error.already_converted'),
      );
      return;
    }
    ...
    final result = await showDialog<Map<String, dynamic>>(
      context: context,
      builder: (context) => SiparisDonusturDialog(siparis: order),
    );
    ...
    if (result != null) {
      ...
      // Ürünleri TransactionItem'e dönüştür
      final List<TransactionItem> items = order.urunler.map((u) {
      ...
      // Türüne göre Alış veya Satış sayfasına yönlendir
      // Varsayılan olarak Alış (10), Eğer 'Satış' içeriyorsa Satış (11)
      final bool isSales =
          order.tur.toLowerCase().contains('satış') ||
          order.tur.toLowerCase().contains('sale');
      final int targetMenuIndex = isSales ? 11 : 10;
      ...
      // Eğer işlem tamamlandıysa siparişi "Dönüştürüldü" durumuna getir
      if (conversionResult == true) {
        final servis = SiparislerVeritabaniServisi();
        final success = await servis.siparisDurumGuncelle(
          order.id,
          tr('orders.status.converted'),
        );

        if (success) {
          _fetchSiparisler(force: true);
          if (mounted) {
            MesajYardimcisi.basariGoster(context, tr('orders.convert_success'));
          }
        }
      }
    }
  ...
  Future<void> _siparisleriAl() async {
    if (_selectedIds.isEmpty) {
      ...
      MesajYardimcisi.uyariGoster(
        context,
        tr('orders.error.select_order_for_action'),
      );
      return;
    }
    ...
    final bool? onay = await showDialog<bool>(
      ...
      builder: (context) => OnayDialog(
        ...
        onayButonMetni: tr('orders.receive_order'),
        onOnay: () {},
      ),
    );
    ...
    if (onay == true) {
      ...
      try {
        ...
        for (final id in _selectedIds) {
          final order = _siparisler.firstWhereOrNull((o) => o.id == id);
          if (order == null) continue;
          if (order.durum == 'Onaylandı') continue;

          bool basarili = false;
          if (order.tur.contains('Teklif')) {
            final yeniTur = order.tur.replaceAll('Teklif', 'Sipariş');
            basarili = await servis.siparisTurVeDurumGuncelle(
              order.id,
              yeniTur,
              'Onaylandı',
            );
          } else {
            basarili = await servis.siparisDurumGuncelle(order.id, 'Onaylandı');
          }
  ...
  Future<void> _deleteSelectedSiparisler() async {
    final bool? onay = await showDialog<bool>(
      context: context,
      builder: (context) => OnayDialog(
        baslik: tr('common.confirmation'),
