...
class _CariTxPageCursor {
  const _CariTxPageCursor({
    required this.lastDate,
    required this.lastSortPriority,
    required this.lastCreatedAt,
    required this.lastId,
    required this.nextStartBalance,
  });

  final DateTime lastDate;
  final int lastSortPriority;
  final DateTime lastCreatedAt;
  final int lastId;

  /// Backward-running balance hesaplamasında bir sonraki sayfanın başlangıç bakiyesi.
  final double nextStartBalance;
}
...
class _CariKartiSayfasiState extends State<CariKartiSayfasi> {
  ...
  final Map<int, Map<String, dynamic>> _selectedTransactionsById = {};
  final Map<int, Set<int>> _selectedInstallmentIds = {};
  int? _selectedInstallmentRowId;
  List<Map<String, dynamic>> _cachedTransactions = [];

  // [2026] Cursor-based "limitsiz" transactions loading (COUNT'suz, skip-based paging yok)
  static const int _transactionsPageSizeDefault =
      25; // GenisletilebilirTablo default
  int _transactionsPageSize = _transactionsPageSizeDefault;
  int _transactionsCurrentPage = 1;
  int _transactionsTableRevision = 0;
  final Map<int, _CariTxPageCursor> _transactionsPageCursors = {};
  bool _transactionsHasMore = false;

  bool _hasInstallments = false;
  bool _isInstallmentsMode = false;
  int _installmentsMobileCurrentPage = 1;
  ...
  Future<void> _loadTransactions() async {
    ...
    try {
      ...
      // Page 1'de cari detayını tazele (bakiye + görseller vs.)
      if (_transactionsCurrentPage == 1) {
        await _refreshCariDetails();
      }

      final prevCursor = (_transactionsCurrentPage > 1)
          ? _transactionsPageCursors[_transactionsCurrentPage - 1]
          : null;

      final int requestLimit = (_transactionsPageSize + 1);
      final List<Map<String, dynamic>> fetched =
          await CariHesaplarVeritabaniServisi().cariIslemleriniGetir(
            _currentCari.id,
            aramaTerimi: _searchQuery.trim().isNotEmpty
                ? _searchQuery.trim()
                : null,
            ...
            limit: requestLimit,
            lastDate: prevCursor?.lastDate,
            lastSortPriority: prevCursor?.lastSortPriority,
            lastCreatedAt: prevCursor?.lastCreatedAt,
            lastId: prevCursor?.lastId,
          );

      final bool hasMore = fetched.length > _transactionsPageSize;
      final List<Map<String, dynamic>> display = hasMore
          ? fetched.sublist(0, _transactionsPageSize)
          : fetched;

      // Running balance (geri sarma): sayfa 1 = güncel bakiye, diğer sayfalar = bir önceki sayfanın nextStartBalance değeri
      final double defaultStartBalance =
          _currentCari.bakiyeBorc - _currentCari.bakiyeAlacak;
      double currentTotalBalance = (_transactionsCurrentPage <= 1)
          ? defaultStartBalance
          : (prevCursor?.nextStartBalance ?? defaultStartBalance);

      for (int i = 0; i < display.length; i++) {
        final tx = display[i];
        final double tutar = parseDouble(tx['tutar']);
        ...
        // Geri sarma: Alacak hareketini "geri almak" bakiye'yi artırır; Borç'u geri almak azaltır.
        if (isIncoming) {
      ...
      // UI Labels Modification (arama açıkken ham değerleri bozma)
      ...
      // Auto-expand: arama detaya denk geliyorsa aç (mevcut davranış bozulmasın)
      _transactionAutoExpandedIndices.clear();
      if (_searchQuery.isNotEmpty) {
        for (int i = 0; i < display.length; i++) {
          if (_shouldExpandForSearch(display[i], _searchQuery)) {
            _transactionAutoExpandedIndices.add(i);
          }
        }
      }

      // Cursor cache (son satırın cursor'u → sonraki sayfa)
      if (display.isNotEmpty) {
        final last = display.last;
        final DateTime? cursorDate = parseDateTime(last['tarih']);
        final int? cursorSort = (last['sort_priority'] is int)
            ? last['sort_priority'] as int
            : int.tryParse(last['sort_priority']?.toString() ?? '');
        final DateTime? cursorCreatedAt = parseDateTime(last['created_at']);
        final int? cursorId = parseInt(last['id']);

        if (cursorDate != null &&
            cursorSort != null &&
            cursorCreatedAt != null &&
            cursorId != null) {
          _transactionsPageCursors[_transactionsCurrentPage] =
              _CariTxPageCursor(
                lastDate: cursorDate,
                lastSortPriority: cursorSort,
                lastCreatedAt: cursorCreatedAt,
                lastId: cursorId,
                nextStartBalance: currentTotalBalance,
              );
        }
      }

      // Filtre istatistiklerini arka planda güncelle (UI bloklamasın)
      ...
      if (mounted) {
        setState(() {
          _cachedTransactions = display;
          _visibleTransactionIds[_currentCari.id] = display
              .map((e) => parseInt(e['id']) ?? 0)
              .where((id) => id > 0)
              .toList();
          _isLoading = false;
          _transactionsHasMore = hasMore;
          ...
          if (hasFilter) {
            if (display.isNotEmpty && !_isManuallyClosedDuringFilter) {
      ...
      unawaited(
        statsFuture
            .then((stats) {
              if (!mounted) return;
              setState(() => _filterStats = stats);
            })
            .catchError((e) {
              debugPrint('Cari işlem filtre istatistikleri güncellenemedi: $e');
            }),
      );
    } catch (e) {
  ...
  void _toggleInstallmentsMode() {
    ...
    setState(() {
      ...
      _installmentsMobileCurrentPage = 1;
  ...
  void initState() {
    ...
    _loadTransactions();
    _refreshTransactionSummarySums();
  ...
  void _resetPagination() {
    _cachedTransactions = [];
    _visibleTransactionIds[_currentCari.id] = [];
    _selectedDetailIds[_currentCari.id]?.clear();
    _selectedTransactionsById.clear();
    _selectedInstallmentIds[_currentCari.id]?.clear();
    _selectedIds.clear();
    ...
    _singleViewRowId = null;
    _transactionAutoExpandedIndices.clear();
    _transactionsCurrentPage = 1;
    _transactionsPageCursors.clear();
    _transactionsHasMore = false;
    _installmentsMobileCurrentPage = 1;
    _transactionsTableRevision++;
  }
  ...
  Future<void> _refreshTransactionSummarySums() async {
    try {
      final sums = await CariHesaplarVeritabaniServisi()
          .cariIslemOzetTutarlariniGetir(_currentCari.id);
      if (!mounted) return;
      setState(() {
        _odemeAlindiSum = sums['odeme_alindi'] ?? 0;
        _odemeYapildiSum = sums['odeme_yapildi'] ?? 0;
        _alacakDekontuSum = sums['alacak_dekontu'] ?? 0;
        _borcDekontuSum = sums['borc_dekontu'] ?? 0;
      });
    } catch (e) {
      debugPrint('Cari özet tutarlar yüklenemedi: $e');
    }
  }
  ...
  Future<void> _handlePrint() async {
    ...
    try {
      ...
      final bool printingSelected = selectedIds.isNotEmpty;
      final List<Map<String, dynamic>> printTransactions = printingSelected
          ? selectedIds
                .map((id) => _selectedTransactionsById[id])
      ...
      if (printingSelected) {
        ...
        printTransactions.sort((a, b) {
          final da = dt(a['tarih']);
          final db = dt(b['tarih']);
          if (da != null && db != null) {
            final cmp = db.compareTo(da); // desc
            if (cmp != 0) return cmp;
          }

          final sa = i(a['sort_priority']);
          final sb = i(b['sort_priority']);
          if (sa != null && sb != null) {
            final cmp = sa.compareTo(sb); // asc
            if (cmp != 0) return cmp;
          }

          final ca = dt(a['created_at']);
          final cb = dt(b['created_at']);
          if (ca != null && cb != null) {
          ...
          final ia = i(a['id']);
          final ib = i(b['id']);
          if (ia != null && ib != null) return ib.compareTo(ia); // desc
          return 0;
        });
      }

      for (int i = 0; i < printTransactions.length; i++) {
        final tx = printTransactions[i];
        ...
        rows.add(
          ExpandableRowData(
            ...
            isExpanded: printingSelected
                ? _isTransactionExpandable(tx)
                : expandedIndices.contains(i),
            isSourceExpanded: expandedIndices.contains(i),
          ),
        );
      }
  ...
  Future<void> _deleteTransaction(int transactionId) async {
    ...
    if (onay == true) {
      try {
        ...
        final tx = _cachedTransactions.firstWhere(
        ...
        await _refreshCariDetails();
        await _refreshTransactionSummarySums();
        await _loadTransactions();
      } catch (e) {
        if (!mounted) return;
        MesajYardimcisi.hataGoster(context, '${tr('common.error')}: $e');
    ...
    if (onay == true) {
      try {
        ...
        for (final id in selectedIds) {
          final tx = _selectedTransactionsById[id];
          if (tx == null) continue;
          final String islemTuru = tx['islem_turu']?.toString() ?? '';
          final String integrationRef = tx['integration_ref']?.toString() ?? '';
  ...
  Widget build(BuildContext context) {
    ...
    const loadingIndicator = Positioned(
      top: 0,
      left: 0,
      right: 0,
        ...
        child: CallbackShortcuts(
          ...
          child: LayoutBuilder(
            builder: (context, constraints) {
              ...
              final bool isMobile = forceMobile || constraints.maxWidth < 800;

              if (isMobile) {
                return Stack(
                  children: [
                    _buildMobileView(theme, showAppBar: showAppBar),
  ...
  Widget _buildCariHesapInfoCard(
    ThemeData theme, {
    EdgeInsets? margin,
    bool compact = false,
  }) {
          ...
          AnimatedCrossFade(
            firstChild: Builder(
              builder: (context) {
                    ...
                    children: [
                      ...
                      Builder(
                        builder: (context) {
                          ...
                          final Widget balanceBox = Container(
                            margin: compact
                                ? EdgeInsets.zero
                                ...
                                Row(
                                  ...
                                  children: [
                                    SizedBox(
                                      ...
                                      child: Text(
                                        tr(
                                          'accounts.card.summary.payment_made',
                                        ),
                                        style: TextStyle(
                                          fontSize: 11,
                          ...
                          final Widget noteTextField = TextField(
                            ...
                            decoration: InputDecoration(
                              ...
                              hintText: tr('accounts.detail.note_placeholder'),
                              hintStyle: TextStyle(
                                fontSize: 11,
                                color: Colors.grey.shade400,
                              ),
                          ...
                          final Widget noteBox = Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                if (compact)
                                  SizedBox(height: 120, child: noteTextField)
                                else
                          ...
                          return IntrinsicHeight(
                            child: Row(
                              ...
                              children: [
                                Expanded(flex: 3, child: leftInfo),
                                balanceBox,
                                noteBox,
                              ],
                            ),
                      ...
                      Builder(
                        builder: (context) {
                          final Widget contact = _buildDetailColumn(
                          ...
                          final Widget invoice = _buildDetailColumn(
                            icon: Icons.receipt_long_outlined,
                            title: tr('accounts.form.section.invoice'),
                            items: [
                              (
                                'Fat. Ünvanı',
                                cari.fatUnvani.isNotEmpty
                                    ? cari.fatUnvani
                                    : '-',
                              ),
                              (
                          ...
                          final Widget shipping = _buildSevkAdresleriSection(
                            cari.sevkAdresleri,
                          );

                          final Widget commercial = _buildDetailColumn(
                          ...
                          final Widget specialInfo = Padding(
                ...
                if (!compact) return expandedContent;
  ...
  Widget _buildMobileView(ThemeData theme, {required bool showAppBar}) {
    ...
    final bool isInstallmentsMode = _isInstallmentsMode;
    final List<Map<String, dynamic>> source = isInstallmentsMode
        ? _cachedInstallments
        : _cachedTransactions;

    final int safeRowsPerPage = _transactionsPageSize <= 0
        ? 25
    ...
    int startIndex = 0;
    int endIndex = 0;
    List<Map<String, dynamic>> paginatedRows = const <Map<String, dynamic>>[];
    String totalLabel = totalRecords.toString();

    if (isInstallmentsMode) {
      totalPages = totalRecords == 0
          ? 1
          : (totalRecords / safeRowsPerPage).ceil();
      effectivePage = _installmentsMobileCurrentPage.clamp(1, totalPages);
      if (effectivePage != _installmentsMobileCurrentPage) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (!mounted) return;
          setState(() => _installmentsMobileCurrentPage = effectivePage);
        });
      }

      startIndex = (effectivePage - 1) * safeRowsPerPage;
      endIndex = (startIndex + safeRowsPerPage) > totalRecords
          ? totalRecords
      ...
      totalLabel = totalRecords.toString();
    } else {
      effectivePage = _transactionsCurrentPage < 1
          ? 1
          : _transactionsCurrentPage;
      startIndex = (effectivePage - 1) * safeRowsPerPage;
      paginatedRows = source; // cursor: source zaten mevcut sayfa verisi
      endIndex = startIndex + paginatedRows.length;
      totalLabel = paginatedRows.isEmpty
          ? '0'
          : (_transactionsHasMore ? '$endIndex+' : '$endIndex');
    }

    final bool hasAnyRows = paginatedRows.isNotEmpty;
    final int showingStart = hasAnyRows ? startIndex + 1 : 0;
    final int endRecord = hasAnyRows ? endIndex : 0;
    final int toolbarRecords = isInstallmentsMode ? totalRecords : endRecord;
      ...
      child: SafeArea(
        ...
        child: Column(
          children: [
            ...
            Expanded(
              child: (_isLoading && paginatedRows.isEmpty)
                  ? const Center(
                  ...
                  : (paginatedRows.isEmpty
                        ...
                        : ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return isInstallmentsMode
                                  ? _buildMobileInstallmentCard(
                                      row,
                                      rowNumber: rowNumber,
                                    )
            ...
            if (!isKeyboardVisible)
              Padding(
                ...
                child: Row(
                  children: [
                    IconButton(
                      onPressed: isInstallmentsMode
                          ? (effectivePage > 1
                                ? () => setState(
                                    () => _installmentsMobileCurrentPage =
                                        effectivePage - 1,
                                  )
                                : null)
                          : (effectivePage > 1 && !_isLoading
                                ? () {
                                    setState(
                                      () => _transactionsCurrentPage =
                                          effectivePage - 1,
                    ...
                    IconButton(
                      onPressed: isInstallmentsMode
                          ? (effectivePage < totalPages
                                ...
                                : null)
                          : (_transactionsHasMore && !_isLoading
                                ? () {
                                    setState(
                                      () => _transactionsCurrentPage =
                                          effectivePage + 1,
  ...
  Widget _buildMobileToolbarCard({
    ...
    final int selectedCount = _isInstallmentsMode
        ? (_selectedInstallmentIds[_currentCari.id]?.length ?? 0)
        : (_selectedDetailIds[_currentCari.id]?.length ?? 0);
    final bool hasSelection =
        selectedCount > 0 || (!_isInstallmentsMode && _isSelectAllActive);
  ...
  Widget _buildMobileFilterGrid() {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (_isInstallmentsMode) {
          return Column(
            children: [_buildDateRangeFilter(width: double.infinity)],
          );
        }
        ...
        if (singleColumn) {
          return Column(
            children: [
              ...
              const SizedBox(height: 12),
              _buildTransactionTypeFilter(width: double.infinity),
              const SizedBox(height: 12),
  ...
  Widget _buildMobileTopActionRow() {
    ...
    final bool hasSelection = isInstallmentsMode
        ? ((_selectedInstallmentIds[_currentCari.id] ?? <int>{}).isNotEmpty)
        : ((_selectedDetailIds[_currentCari.id] ?? <int>{}).isNotEmpty ||
              _isSelectAllActive);
    ...
    final bool canToggleInstallments = _hasInstallments;

    final Widget returnButton = Expanded(
      child: _buildMobileActionButton(
        label: tr('accounts.card.return_action'),
    ...
    final Widget cashInOutButton = Expanded(
      child: _buildMobileActionButton(
        label: tr('accounts.card.cash_in_out'),
    ...
    return Row(
      children: [
        ...
        const SizedBox(width: 8),
        cashInOutButton,
        if (canToggleInstallments) ...[
          const SizedBox(width: 8),
          _buildMobileSquareActionButton(
            icon: Icons.schedule_outlined,
            onTap: _toggleInstallmentsMode,
            color: isInstallmentsMode ? const Color(0xFF2C3E50) : Colors.white,
            iconColor: isInstallmentsMode
                ? Colors.white
                : const Color(0xFF2C3E50),
            borderColor: isInstallmentsMode
                ? Colors.transparent
                : Colors.grey.shade300,
            tooltip: isInstallmentsMode ? 'Hareketler' : 'Taksitler',
            size: 40,
          ),
        ],
        const SizedBox(width: 8),
  ...
  // ignore: unused_element
  Widget _buildMobileActionsMenuButton({
    double size = 48,
    required bool canToggleInstallments,
  }) {
    return Theme(
      ...
      child: PopupMenuButton<String>(
                ...
                child: Row(
                  children: [
                    Icon(
                      ...
                      size: 20,
                      color: isInstallmentsMode
                          ? const Color(0xFF2C3E50)
                          : const Color(0xFF4A4A4A),
                    ),
                    const SizedBox(width: 12),
                    Text(
                      isInstallmentsMode ? 'Hareketler' : 'Taksitler',
                      style: TextStyle(
                        color: isInstallmentsMode
                            ? const Color(0xFF2C3E50)
                            : const Color(0xFF4A4A4A),
              ...
              PopupMenuItem<String>(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 12),
                    Text(
                      tr('warehouses.keep_details_open'),
                      style: const TextStyle(
                        color: Color(0xFF2C3E50),
                        fontWeight: FontWeight.w600,
                        fontSize: 14,
        ...
        onSelected: (value) {
          if (value == 'toggle_installments') {
            _toggleInstallmentsMode();
            return;
          }
          if (value == 'toggle_keep_details') {
            unawaited(_toggleKeepDetailsOpen());
            return;
          }
        },
        child: MouseRegion(
  ...
  Widget _buildMobileTransactionCard(
    ...
    final int? id = parseInt(tx['id']);
    final selectedIds = _selectedDetailIds[_currentCari.id] ?? <int>{};
    final bool isChecked =
        id != null && (_isSelectAllActive || selectedIds.contains(id));
    final bool isFocused = id != null && _selectedRowId == id;
    final bool isSelected = isChecked || isFocused;

    final String rawIslemTuru = tx['islem_turu']?.toString() ?? '';
    String typeLabelRaw = rawIslemTuru.contains('Açılış')
        ? rawIslemTuru
    ...
    bool isIncoming =
        ...
        typeLabelRaw.toLowerCase().contains('tahsilat') ||
        typeLabelRaw.toLowerCase().contains('alındı') ||
        typeLabelRaw.toLowerCase().contains('alindi') ||
        typeLabelRaw.toLowerCase().contains('alış');

    if (rawIslemTuru == 'Cari İşlem' || typeLabelRaw == 'Cari İşlem') {
      if (yon == 'Alacak') {
        typeLabelRaw = 'Para Alındı';
        isIncoming = true;
      } else if (yon == 'Borç') {
        typeLabelRaw = 'Para Verildi';
        isIncoming = false;
      }
    }

    final String typeLabel = IslemCeviriYardimcisi.cevir(typeLabelRaw);
    final Color typeBg = IslemTuruRenkleri.arkaplanRengiGetir(
      typeLabelRaw,
      isIncoming,
    );
    final Color typeFg = IslemTuruRenkleri.ikonRengiGetir(
      typeLabelRaw,
      isIncoming,
    );
    final Color typeTextFg = IslemTuruRenkleri.metinRengiGetir(
      typeLabelRaw,
      isIncoming,
    );

    final String dateText =
        (tx['tarih'] != null && tx['tarih'].toString().trim().isNotEmpty)
        ? formatDate(tx['tarih'])
        : (tx['created_at'] != null ? formatDate(tx['created_at']) : '');

    final double tutar = parseDouble(tx['tutar']);
    final String amountText =
        '${FormatYardimcisi.sayiFormatlaOndalikli(tutar.abs(), binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.fiyatOndalik)} ${_currentCari.paraBirimi}';

    final bool hasRunningBalance = tx['running_balance'] != null;
    final double runningBalance = hasRunningBalance
        ? parseDouble(tx['running_balance'])
        : 0.0;
    final String runningText = hasRunningBalance
        ? '${FormatYardimcisi.sayiFormatlaOndalikli(runningBalance.abs(), binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.fiyatOndalik)} ${_currentCari.paraBirimi}'
        : '';
    final String runningLabel = hasRunningBalance
        ? (runningBalance >= 0
              ? tr('accounts.table.type_debit')
              : tr('accounts.table.type_credit'))
        : '';

    final String description = rawIslemTuru.contains('Açılış')
        ? ''
        : (tx['aciklama']?.toString() ?? '').trim();

    String locationName =
        tx['kaynak_adi']?.toString() ?? tx['yer']?.toString() ?? '';
    String locationCode = tx['kaynak_kodu']?.toString() ?? '';
    final int? sourceId = parseInt(tx['source_id']);

    if (locationName.isEmpty && description.isNotEmpty) {
    ...
    final String locationType = typeLabelRaw;
    final String locationTypeLabel = IslemCeviriYardimcisi.cevir(
      (locationType == 'Kasa' ||
              locationType == 'Banka' ||
    ...
    final bool canExpand = _isTransactionExpandable(tx);
    final Set<int> expandedIndices = _getEffectiveTransactionExpandedIndices();
    final bool isExpanded = canExpand && expandedIndices.contains(indexInPage);
                ...
                children: [
                  SizedBox(
                    ...
                    child: Checkbox(
                      ...
                      onChanged: (v) {
                        if (id == null) return;
                        _onSelectDetailRow(_currentCari.id, id, v);
                      },
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(4),
                      ),
                  ...
                  if (canExpand) ...[
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        ...
                        setState(() {
                          if (isExpanded) {
                            if (_manualExpandedIndex == indexInPage) {
                              _manualExpandedIndex = null;
                            }
                            _transactionAutoExpandedIndices.remove(indexInPage);
                          } else {
                            _manualExpandedIndex = indexInPage;
                            _transactionAutoExpandedIndices.add(indexInPage);
                          }
                        });
                      },
                    ),
                ...
                child: !isExpanded
                    ...
                    : Column(
                        children: [
                          ...
                          _buildMobileTransactionDetails(tx),
                        ],
                      ),
              ),
            ],
  ...
  Widget _buildMobileTransactionDetails(Map<String, dynamic> tx) {
    final String query = _searchQuery.trim();
    ...
    String formatDateOrDash(dynamic value, {bool includeTime = true}) {
      final txt = _formatPrintDate(value, includeTime: includeTime).trim();
      return txt.isEmpty ? '-' : txt;
    }

    final String belgeNo = (tx['belge_no'] ?? tx['belge'] ?? '')
        .toString()
        .trim();
    final String eBelgeNo = (tx['e_belge_no'] ?? tx['e_belge'] ?? '')
        .toString()
        .trim();
    final String irsaliyeNo = (tx['irsaliye_no'] ?? '').toString().trim();
    final String faturaNo = (tx['fatura_no'] ?? '').toString().trim();
    final String aciklama2 = (tx['aciklama2'] ?? '').toString().trim();

    final List<Map<String, dynamic>> details = _parseTxDetailRows(
      tx['hareket_detaylari'],
    );
    ...
    return Column(
      ...
      children: [
        ...
        if (eBelgeNo.isNotEmpty)
          _buildMobileKeyValue(
            tr('settings.general.option.documents.eDocument'),
            valueOrDash(eBelgeNo),
            maxLines: 2,
          ),
        if (irsaliyeNo.isNotEmpty)
          _buildMobileKeyValue(
            tr('purchase.complete.field.waybill_no'),
            valueOrDash(irsaliyeNo),
            maxLines: 2,
          ),
        if (faturaNo.isNotEmpty)
          _buildMobileKeyValue(
            tr('purchase.complete.field.invoice_no'),
            valueOrDash(faturaNo),
            maxLines: 2,
          ),
        if (aciklama2.isNotEmpty)
          _buildMobileKeyValue(
            tr('common.description2'),
            valueOrDash(aciklama2),
            maxLines: 3,
          ),
        if ((tx['vade_tarihi'] ?? '').toString().trim().isNotEmpty)
          _buildMobileKeyValue(
            tr('common.due_date_short'),
            formatDateOrDash(tx['vade_tarihi'], includeTime: false),
          ),
        if (details.isNotEmpty) ...[
          ...
          const SizedBox(height: 8),
          ...details.map((d) => _buildMobileTxDetailItemCard(d, query: query)),
        ] else ...[
          const SizedBox(height: 6),
          Text(
            tr('common.no_records_found'),
  ...
  List<Map<String, dynamic>> _parseTxDetailRows(dynamic rawDetaylar) {
    ...
    void push(dynamic value) {
      ...
      if (value is Map) {
        details.add(Map<String, dynamic>.from(value));
        return;
      }
      if (value is String) {
        try {
  ...
  Widget _buildMobileTxDetailItemCard(
    ...
    final double qty = parseDouble(detail['miktar'] ?? detail['quantity']);
    final String unit = (detail['birim'] ?? detail['unit'] ?? '')
        .toString()
        .trim();
    final double unitPrice = parseDouble(
      detail['birim_fiyat'] ??
          detail['price'] ??
          detail['unitCost'] ??
          detail['unit_price'],
    ...
    final double discountRate = parseDouble(
      detail['iskonto'] ?? detail['discount'] ?? detail['discountRate'],
    );

    final double computedTotal =
        qty *
        unitPrice *
        (1 - (discountRate.isFinite ? (discountRate / 100) : 0.0));
    final double total = parseDouble(detail['total']) > 0
        ? parseDouble(detail['total'])
        : (computedTotal.isFinite ? computedTotal : 0.0);
    ...
    final String unitPriceText =
        '${FormatYardimcisi.sayiFormatlaOndalikli(unitPrice, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.fiyatOndalik)} ${_currentCari.paraBirimi}';
    final String totalText =
        '${FormatYardimcisi.sayiFormatlaOndalikli(total, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.fiyatOndalik)} ${_currentCari.paraBirimi}';
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Text(
                '${qtyText.isNotEmpty ? qtyText : '-'}${unit.isNotEmpty ? ' $unit' : ''}',
                style: TextStyle(
                  fontSize: 13,
                  fontWeight: FontWeight.w700,
                  color: Colors.grey.shade800,
          ...
          Align(
            alignment: Alignment.centerRight,
            child: Text(
              '${tr('shipment.field.price')}: $unitPriceText${discountRate > 0 ? ' • ${tr('common.discount_percent_short')}: %${FormatYardimcisi.sayiFormatlaOndalikli(discountRate)}' : ''}',
              maxLines: 2,
              overflow: TextOverflow.ellipsis,
              textAlign: TextAlign.right,
              style: TextStyle(
  ...
  Widget _buildMobileInstallmentCard(
    ...
    final int? id = parseInt(row['id']);
    final selectedIds = _selectedInstallmentIds[_currentCari.id] ?? <int>{};
    final bool isChecked = id != null && selectedIds.contains(id);
    final bool isFocused = id != null && _selectedInstallmentRowId == id;
    final bool isSelected = isChecked || isFocused;
    ...
    final DateTime? satisTarihiDt = parseDate(row['satis_tarihi']);
    final String satisTarihStr = satisTarihiDt != null
        ? DateFormat('dd.MM.yyyy HH:mm').format(satisTarihiDt)
        : '-';

    final DateTime? vadeDt = parseDate(row['vade_tarihi']);
    final String vadeStr = vadeDt != null
    ...
    Future<void> openInstallmentDialog() async {
      final ref = row['integration_ref']?.toString() ?? '';
                ...
                children: [
                  SizedBox(
                    ...
                    child: Checkbox(
                      ...
                      onChanged: (v) {
                        if (id == null) return;
                        _onSelectInstallmentRow(_currentCari.id, id, v);
                      },
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(4),
                      ),
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            Expanded(
                              child: Text(
                                '${tr('accounts.card.installments.col.sale_date')}: $satisTarihStr',
                                maxLines: 1,
                                overflow: TextOverflow.ellipsis,
                                style: TextStyle(
                                  fontSize: 12,
                        ...
                        Row(
                          children: [
                            Expanded(
                              child: Text(
                                '${tr('common.due_date_short')}: $vadeStr',
                                maxLines: 1,
                                overflow: TextOverflow.ellipsis,
                                style: TextStyle(
                                  fontSize: 12,
                        ...
                        if (faturaNo.trim().isNotEmpty) ...[
                          const SizedBox(height: 6),
                          HighlightText(
                            text:
                                '${tr('purchase.complete.field.invoice_no')}: $faturaNo',
                            query: query,
                            style: TextStyle(
                              fontSize: 12,
                              color: Colors.grey.shade700,
  ...
  Widget _buildMobileKeyValue(String label, String value, {int maxLines = 1}) {
    return Padding(
  ...
  Widget _buildDesktopView(BoxConstraints constraints) {
    ...
    return GenisletilebilirTablo<Map<String, dynamic>>(
      key: ValueKey('cari_tx_table_${_currentCari.id}'),
      title: '',
      totalRecords: 0,
      cursorPagination: true,
      hasNextPage: _transactionsHasMore,
      paginationResetKey: _transactionsTableRevision,
      ...
      onPageChanged: (page, rowsPerPage) {
        setState(() {
          final bool rowsChanged = rowsPerPage != _transactionsPageSize;
          _transactionsPageSize = rowsPerPage;
          if (rowsChanged) {
            _resetPagination();
          } else {
  ...
  Widget _buildTransactionPopupMenu(
    ...
    final tabScope = TabAciciScope.of(context);
    if (targetIndex != null && tabScope != null) {
  ...
  Widget _buildTransactionsSection(CariHesapModel cari) {
    ...
    return FutureBuilder<List<Map<String, dynamic>>>(
      ...
      future: Future.value(_cachedTransactions),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
    ...
    setState(() {
      ...
      if (value == true) {
        ...
        for (final tx in _cachedTransactions) {
          final dynamic idValue = tx['id'];
          final int? id = idValue is int
              ? idValue
              : int.tryParse(idValue?.toString() ?? '');
          if (id == null || id <= 0) continue;
          if (!visibleIds.contains(id)) continue;
          _selectedTransactionsById[id] = Map<String, dynamic>.from(tx);
        }
      } else {
  ...
  void _onSelectDetailRow(int cariId, int txId, bool? value) {
    setState(() {
      ...
      if (value == true) {
        ...
        final tx = _cachedTransactions.firstWhere((t) {
        ...
        if (tx.isNotEmpty) {
          _selectedTransactionsById[txId] = Map<String, dynamic>.from(tx);
        }
      } else {
        _selectedDetailIds[cariId]!.remove(txId);
        _selectedTransactionsById.remove(txId);
      }
    });
  }
