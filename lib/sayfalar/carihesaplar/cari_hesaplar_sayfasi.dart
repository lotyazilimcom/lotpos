...
class _CariHesaplarSayfasiState extends State<CariHesaplarSayfasi> {
  ...
  int? _lastMobileTappedCariId;
  int _lastMobileTapAtMs = 0;
  int? _selectedRowId;
  bool _hasNextPage =
      false; // [2026] COUNT(*) yok: cursor next/prev (pagination)
  ...
  // Mobile cards: separate cache to avoid desktop/detail future collisions
  final Map<int, Future<List<Map<String, dynamic>>>> _mobileDetailFutures = {};

  // [2026] Expanded/detail cursor pagination (limitsiz geçmiş gezinme)
  static const int _detailTransactionsPageSize = 500;
  ...
  bool _consumeMobileDoubleTap(int cariId) {
    final int now = DateTime.now().millisecondsSinceEpoch;
    final bool isDoubleTap =
        _lastMobileTappedCariId == cariId && (now - _lastMobileTapAtMs) <= 350;

    _lastMobileTappedCariId = cariId;
    _lastMobileTapAtMs = now;

    if (isDoubleTap) {
      _lastMobileTappedCariId = null;
  ...
  CariHesapModel? _getSelectedMobileCari() {
    final id = _selectedMobileCardId;
    if (id == null) return null;
    for (final cari in _cachedCariHesaplar) {
      if (cari.id == id) return cari;
    }
    return null;
  }

  Future<void> _openCariKarti(CariHesapModel cari) async {
    ...
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => CariKartiSayfasi(cariHesap: cari),
      ),
    );
  }
  ...
  Future<void> _fetchCariHesaplar({bool showLoading = true}) async {
    ...
    try {
      ...
      final String indexSortBy = switch (_sortBy) {
        'kod_no' => 'root_code.keyword:$dir',
        'adi' => 'root_name.keyword:$dir',
        'hesap_turu' => 'root_account_type:$dir',
        'bakiye_borc' => 'root_balance_debit:$dir',
        'bakiye_alacak' => 'root_balance_credit:$dir',
      ...
      final List<String> indexTables = needsTxTables
          ? const <String>['current_accounts', 'current_account_transactions']
          : const <String>['current_accounts'];

      final filterParts = <String>[];
      if (aktifMi != null) {
        filterParts.add('root_aktif_mi:${aktifMi ? 'true' : 'false'}');
      }
      final hesapTuru = (_selectedAccountType ?? '').trim();
      if (hesapTuru.isNotEmpty) {
        filterParts.add('root_account_type:"${osQuote(hesapTuru)}"');
      }
      final sehir = (_selectedCity ?? '').trim();
      if (sehir.isNotEmpty) {
        filterParts.add('root_city:"${osQuote(sehir)}"');
      }
      ...
      if (primary.indexEnabled) {
        ...
        final fallback = await CariHesaplarVeritabaniServisi()
            .cariHesaplariGetir(
              sayfa: _currentPage,
        ...
        cariHesaplar = hasMore
            ? fallback.sublist(0, _rowsPerPage)
            : List<CariHesapModel>.from(fallback);
        if (cariHesaplar.isNotEmpty) {
          nextCursor = 'id:${cariHesaplar.last.id}';
        }
      }

      if (!mounted || sorguNo != _aktifSorguNo) return;
      final List<CariHesapModel> display = cariHesaplar;
  ...
  Future<void> _loadMoreDetailTransactions(
    ...
    if (_detailTransactionsLoading[cari.id] == true) return;

    final existing =
        _detailTransactionsCache[cari.id] ??
        List<Map<String, dynamic>>.from(initialTransactions);
    if (existing.isEmpty) {
      setState(() => _detailTransactionsHasMore[cari.id] = false);
      return;
    }
    ...
    final dynamic lastDateRaw = existing.last['tarih'];
    final DateTime? lastDate = lastDateRaw is DateTime
        ? lastDateRaw
        : DateTime.tryParse(lastDateRaw?.toString() ?? '');

    final dynamic lastSortRaw = existing.last['sort_priority'];
    final int? lastSortPriority = lastSortRaw is int
        ? lastSortRaw
        : int.tryParse(lastSortRaw?.toString() ?? '');

    final dynamic lastCreatedRaw = existing.last['created_at'];
    final DateTime? lastCreatedAt = lastCreatedRaw is DateTime
        ? lastCreatedRaw
        : DateTime.tryParse(lastCreatedRaw?.toString() ?? '');
    ...
    setState(() {
      _detailTransactionsLoading[cari.id] = true;
      _detailTransactionsCache[cari.id] = existing;
      _detailTransactionsHasMore[cari.id] =
          _detailTransactionsHasMore[cari.id] ??
          (initialTransactions.length == _detailTransactionsPageSize);
    });

    try {
      final more = await CariHesaplarVeritabaniServisi().cariIslemleriniGetir(
        cari.id,
        ...
        lastDate: lastDate,
        lastSortPriority: lastSortPriority,
      ...
      setState(() {
        _detailTransactionsCache[cari.id] = [...existing, ...more];
        _detailTransactionsHasMore[cari.id] =
            more.length == _detailTransactionsPageSize;
        _detailTransactionsLoading[cari.id] = false;
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _detailTransactionsLoading[cari.id] = false;
        _detailTransactionsHasMore[cari.id] = false;
      });
      debugPrint('Cari detay yükleme hatası: $e');
    }
  }
  ...
  Widget _buildMobileView(List<CariHesapModel> cariHesaplar) {
    ...
    final int showingStart = cariHesaplar.isEmpty ? 0 : startRecordIndex + 1;
    final int endRecord = cariHesaplar.isEmpty
        ? 0
        : startRecordIndex + cariHesaplar.length;
    final String totalLabel = cariHesaplar.isEmpty
        ? '0'
                  ...
                  : (cariHesaplar.isEmpty
                        ...
                        : ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              return _buildMobileCariCard(cariHesaplar[index]);
                            },
                          )),
            ),
            if (!isKeyboardVisible)
  ...
  int _getActiveMobileFilterCount() {
    ...
    if (_selectedAccountType != null) count++;
  ...
  Future<void> _handleMobileActionMenuSelection(String value) async {
    final cari = _getSelectedMobileCari();
    if (cari == null) {
      MesajYardimcisi.bilgiGoster(context, 'Önce bir cari seçin.');
      return;
    }

    if (value == 'open_card') {
      await _openCariKarti(cari);
      return;
    }
  ...
  Widget _buildMobileActionsMenuButton({double size = 48}) {
    final bool hasSelection = _selectedMobileCardId != null;
    return Theme(
          ...
          PopupMenuItem<String>(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 12),
                Text(
                  '${tr('common.activate')} / ${tr('common.deactivate')}',
                  style: TextStyle(
                    color: hasSelection
                        ? const Color(0xFF2C3E50)
                        : Colors.grey.shade400,
  ...
  Widget _buildMobileTopActionRow() {
    ...
    final String openCardLabel = isNarrow ? 'Kart' : tr('common.open_card');
    final String addLabel = isNarrow ? 'Ekle' : tr('accounts.add');
    final String printTooltip = _selectedIds.isNotEmpty || _isSelectAllActive
        ...
        if (singleColumn) {
          return Column(
            children: [
              ...
              const SizedBox(height: 12),
              _buildAccountTypeFilter(width: double.infinity),
              const SizedBox(height: 12),
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 12),
                Expanded(
                  child: _buildAccountTypeFilter(width: double.infinity),
                ),
              ],
            ),
            const SizedBox(height: 12),
  ...
  Widget _buildMobileToolbarCard({
    ...
    final Set<int> optionsSet = {25, 50, 100, 200, _rowsPerPage};
    final List<int> rowsOptions = optionsSet.toList()..sort();
                                  ...
                                  Container(
                                    ...
                                    child: DropdownButtonHideUnderline(
                                      child: DropdownButton<int>(
                                        value:
                                            rowsOptions.contains(_rowsPerPage)
                                            ? _rowsPerPage
                                            : 25,
                                        icon: const Icon(
                                          Icons.keyboard_arrow_down,
  ...
  Widget _buildMobileCariCard(CariHesapModel cari) {
    final bool isExpanded = _expandedMobileIds.contains(cari.id);
    final bool isSelectedForActions = _selectedMobileCardId == cari.id;

    final double netBalance = cari.bakiyeBorc - cari.bakiyeAlacak;
    final bool riskExceeded =
        cari.riskLimiti > 0 && netBalance > cari.riskLimiti;
    ...
    final Color balanceColor = riskExceeded
        ? Colors.deepOrange
        : (netBalance > 0
              ? Colors.red.shade700
              : (netBalance < 0
    ...
    final String accountTypeLabel = IslemCeviriYardimcisi.cevir(
      IslemTuruRenkleri.getProfessionalLabel(cari.hesapTuru, context: 'cari'),
    );

    Color? markedBgColor;
    Color? markedBorderColor;
    Color primaryTextColor = Colors.black87;
    Color secondaryTextColor = Colors.grey.shade600;
    Color surfaceIconColor = const Color(0xFF2C3E50);
    Color checkboxSideColor = const Color(0xFFD1D1D1);

    if (cari.renk == 'blue') {
      markedBgColor = Colors.blue.shade700;
      markedBorderColor = Colors.blue.shade900;
      primaryTextColor = Colors.white;
      secondaryTextColor = Colors.white70;
      surfaceIconColor = Colors.white70;
      checkboxSideColor = Colors.white70;
    } else if (cari.renk == 'red') {
      markedBgColor = Colors.red.shade700;
      markedBorderColor = Colors.red.shade900;
      primaryTextColor = Colors.white;
      secondaryTextColor = Colors.white70;
      surfaceIconColor = Colors.white70;
      checkboxSideColor = Colors.white70;
    } else if (cari.renk == 'black') {
      markedBgColor = Colors.grey.shade900;
      markedBorderColor = Colors.black;
      primaryTextColor = Colors.white;
      secondaryTextColor = Colors.white70;
      surfaceIconColor = Colors.white70;
      checkboxSideColor = Colors.white70;
    ...
    final Color effectiveBorderColor = isSelectedForActions
        ? (markedBorderColor ?? const Color(0xFF2C3E50).withValues(alpha: 0.3))
        : (markedBorderColor ?? Colors.grey.shade200);
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        onTap: () {
          if (_consumeMobileDoubleTap(cari.id)) {
            unawaited(_openCariKarti(cari));
            return;
          }
          setState(() {
            if (_selectedMobileCardId == cari.id) {
              _selectedMobileCardId = null;
            } else {
              _selectedMobileCardId = cari.id;
            }
        ...
        child: Container(
          decoration: BoxDecoration(
            ...
            border: Border.all(
              ...
              width: markedBorderColor != null || isSelectedForActions
                  ? 1.5
                  : 1,
            ),
            boxShadow: [
              BoxShadow(
                ...
                blurRadius: isSelectedForActions ? 12 : 10,
              ...
              Row(
                ...
                children: [
                  SizedBox(
                    ...
                    child: Checkbox(
                      value:
                          _isSelectAllActive || _selectedIds.contains(cari.id),
                      onChanged: (v) => _onSelectRow(v, cari.id),
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(4),
                      ),
                      side: BorderSide(color: checkboxSideColor),
                    ),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              child: Text(
                                netBalance == 0 ? '-' : cari.bakiyeDurumu,
                                style: const TextStyle(
                                  fontWeight: FontWeight.w600,
                                  fontSize: 13,
                                ),
  ...
  Widget _buildMobileCariDetails(CariHesapModel cari) {
    const int mobileTxLimit = 50;
    ...
    return Column(
      ...
      children: [
        ...
        FutureBuilder<List<Map<String, dynamic>>>(
          future: _mobileDetailFutures.putIfAbsent(
            cari.id,
          ...
          builder: (context, snapshot) {
            ...
            final txs = snapshot.data ?? const <Map<String, dynamic>>[];
            if (txs.isEmpty) {
              return Text(
                tr('common.no_data'),
            ...
            return Column(
              children: txs
                  .map(
                    (tx) => _buildMobileCariTransactionRow(
                      tx,
  ...
  Widget _buildMobileCariTransactionRow(
    Map<String, dynamic> tx, {
    required String currency,
  }) {
    String formatDate(dynamic raw) {
      if (raw == null) return '';
      if (raw is DateTime) return DateFormat('dd.MM.yyyy HH:mm').format(raw);
    ...
    final String rawType = (tx['islem_turu'] ?? '').toString().trim();
    final String yon = (tx['yon'] ?? '').toString().toLowerCase();
    final bool isIncoming =
        yon.contains('alacak') ||
        rawType.toLowerCase().contains('tahsilat') ||
        rawType.toLowerCase().contains('alış') ||
        rawType.toLowerCase().contains('alis') ||
        rawType.toLowerCase().contains('girdi') ||
        rawType.toLowerCase().contains('alındı') ||
        rawType.toLowerCase().contains('alindi');

    final String typeLabel = IslemCeviriYardimcisi.cevir(
      IslemTuruRenkleri.getProfessionalLabel(rawType, context: 'cari'),
    );
    ...
    final Color typeFg = IslemTuruRenkleri.ikonRengiGetir(rawType, isIncoming);
    final Color typeTextFg = IslemTuruRenkleri.metinRengiGetir(
      rawType,
      isIncoming,
    );

    final String dateText = formatDate(tx['tarih']);
    final String sourceName = (tx['kaynak_adi'] ?? '').toString().trim();
    final String description = (tx['aciklama'] ?? '').toString().trim();
    final String user = (tx['kullanici'] ?? '').toString().trim();

    final double amount = parseDouble(tx['tutar']);
    final String amountText =
        '${FormatYardimcisi.sayiFormatlaOndalikli(amount.abs(), binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.fiyatOndalik)} $currency';

    final String sourceSingleLine = sourceName.replaceAll('\n', ' • ').trim();
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (sourceSingleLine.isNotEmpty) ...[
  ...
  Widget _buildTransactionsSection(CariHesapModel cari) {
    ...
    return FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final cached = _detailTransactionsCache[cari.id];
        final transactions = cached ?? initialTransactions;
        final bool isLoadingMore = _detailTransactionsLoading[cari.id] == true;
        final bool hasMore =
            _detailTransactionsHasMore[cari.id] ??
            (initialTransactions.length == _detailTransactionsPageSize);
                                  ...
                                  Flexible(
                                    child: HighlightText(
                                      ...
                                      style: TextStyle(
                                        ...
                                        fontWeight: FontWeight.w600,
                                        color:
                                            IslemTuruRenkleri.metinRengiGetir(
                                              customTypeLabel ?? name,
                                              isIncoming,
                                            ),
                                      ),
