...
class _KrediKartlariSayfasiState extends State<KrediKartlariSayfasi> {
  ...
  Future<void> _fetchKrediKartlari({bool showLoading = true}) async {
    ...
    try {
      ...
      final List<String> indexTables = needsTxTables
          ? const <String>['credit_cards', 'credit_card_transactions']
          : const <String>['credit_cards'];
      ...
      if (usePrimarySearch) {
        ...
        final fallback = await KrediKartlariVeritabaniServisi()
        ...
        krediKartlari = hasMore
            ? fallback.sublist(0, _rowsPerPage)
            : List<KrediKartiModel>.from(fallback);
        if (krediKartlari.isNotEmpty) {
          nextCursor = 'id:${krediKartlari.last.id}';
        }
      }

      if (!mounted || sorguNo != _aktifSorguNo) return;

      final List<KrediKartiModel> display = krediKartlari;
  ...
  Future<void> _loadMoreDetailTransactions(
    ...
    if (_detailTransactionsLoading[krediKarti.id] == true) return;

    final existing =
        _detailTransactionsCache[krediKarti.id] ??
        List<Map<String, dynamic>>.from(initialTransactions);
    if (existing.isEmpty) {
      setState(() => _detailTransactionsHasMore[krediKarti.id] = false);
      return;
    }
    ...
    setState(() {
      _detailTransactionsLoading[krediKarti.id] = true;
      _detailTransactionsCache[krediKarti.id] = existing;
      _detailTransactionsHasMore[krediKarti.id] =
          _detailTransactionsHasMore[krediKarti.id] ??
          (initialTransactions.length == _detailTransactionsPageSize);
    });

    try {
      final more = await KrediKartlariVeritabaniServisi()
      ...
      setState(() {
        _detailTransactionsCache[krediKarti.id] = [...existing, ...more];
        _detailTransactionsHasMore[krediKarti.id] =
            more.length == _detailTransactionsPageSize;
        _detailTransactionsLoading[krediKarti.id] = false;
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _detailTransactionsLoading[krediKarti.id] = false;
        _detailTransactionsHasMore[krediKarti.id] = false;
      });
      debugPrint('Kredi kartı detay yükleme hatası: $e');
    }
  }
    ...
    try {
      ...
      for (var i = 0; i < dataToProcess.length; i++) {
        ...
        final isExpanded =
            ...
            _expandedMobileIds.contains(krediKarti.id) ||
            ...
            children: [
              ...
              FutureBuilder<List<Map<String, dynamic>>>(
                ...
                builder: (context, snapshot) {
                  ...
                  final cached = _detailTransactionsCache[krediKarti.id];
  ...
  Widget _buildMobileView(List<KrediKartiModel> filteredKrediKartlari) {
    final int safeRowsPerPage = _rowsPerPage <= 0 ? 25 : _rowsPerPage;
    final int startRecordIndex = (_currentPage - 1) * safeRowsPerPage;
    final int showingStart = filteredKrediKartlari.isEmpty
        ? 0
        : startRecordIndex + 1;
    final int endRecord = filteredKrediKartlari.isEmpty
        ? 0
        : startRecordIndex + filteredKrediKartlari.length;
    final String totalLabel = filteredKrediKartlari.isEmpty
        ? '0'
  ...
  Widget _buildMobileDetails(KrediKartiModel krediKarti) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      key: ValueKey(krediKarti.id),
      future: _detailFutures.putIfAbsent(
        krediKarti.id,
        () => KrediKartlariVeritabaniServisi().krediKartiIslemleriniGetir(
          krediKarti.id,
          aramaTerimi: _searchQuery,
          kullanici: _selectedUser,
          baslangicTarihi: _startDate,
