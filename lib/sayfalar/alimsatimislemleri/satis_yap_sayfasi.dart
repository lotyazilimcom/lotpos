...
class SatisYapSayfasi extends StatefulWidget {
  ...
  @override
  State<SatisYapSayfasi> createState() => _SatisYapSayfasiState();
}

class _SatisYapSayfasiState extends State<SatisYapSayfasi> {
  ...
  int _selectedSalePriceGroup = 1; // 1/2/3 satış fiyatı
  ...
  String? _selectedSerialNumber; // Seçili IMEI/Seri No
  List<CihazModel> _availableDevices = []; // Ürüne ait satılabilir cihazlar
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          String currency =
              widget.initialCurrency ?? settings.varsayilanParaBirimi;
          ...
          // Eğer initialCari varsa müşteri alanlarını doldur
          if (widget.initialCari != null) {
          ...
          // Eğer initialItems varsa ürünleri yükle (teklif dönüştürme için)
          ...
          } else if (_selectedCurrency != settings.varsayilanParaBirimi) {
            _updateCurrentRate(_selectedCurrency);
          }

          _updateInvoiceRate(_invoiceCurrency);

          // Eğer initialDescription varsa açıklamayı yükle
  ...
  Future<void> _findProduct({String? query, bool isAuto = false}) async {
    ...
    // Eğer manuel arama (buton tıklaması) ise ve bir arama terimi varsa, doğrudan diyaloğu aç
    // Bu sayede kullanıcı seçili ürünü değiştirebilir.
    if (!isAuto) {
    ...
    try {
      ...
      if (mounted) {
        if (urunResults.isNotEmpty) {
          ...
          // 2. IMEI/Seri No Araması (Doğrudan cihaz okutulmuş olabilir)
          try {
            final q = searchTerm;
            final deviceResult = await UrunlerVeritabaniServisi()
                .getPool()
                ?.execute(
                  Sql.named(
                    'SELECT * FROM product_devices WHERE identity_value = @q AND is_sold = 0 LIMIT 1',
                  ),
                  parameters: {'q': q},
                );

            if (deviceResult != null && deviceResult.isNotEmpty) {
              final device = CihazModel.fromMap(
                deviceResult.first.toColumnMap(),
              );
              final product = await UrunlerVeritabaniServisi().urunGetirById(
                device.productId,
              );
              if (product != null) {
                _fillProductFields(product, updateCodeField: !isAuto);
                setState(() {
                  _selectedSerialNumber = device.identityValue;
                  _quantityController.text = '1'; // IMEI ürünleri 1 adettir
                });
                return;
              }
            }
          } catch (e) {
            debugPrint('Device search error: $e');
          }
          ...
          if (uretimResults.isNotEmpty) {
            ...
            // Üretimi UrunModel'e dönüştür
            final urunAsModel = UrunModel(
              id: u.id,
              kod: u.kod,
              ad: '${u.ad} (Üretim)',
              birim: u.birim,
            ...
            _fillProductFields(urunAsModel, updateCodeField: !isAuto);
          } else {
            if (isAuto) {
              _clearProductFields(shouldClearCode: false);
            } else {
  ...
  double _selectedSalePrice(UrunModel product) {
    switch (_selectedSalePriceGroup) {
      case 2:
        return product.satisFiyati2;
      case 3:
        return product.satisFiyati3;
      case 1:
      default:
  ...
  void _setUnitPriceFromProduct(UrunModel product) {
    _unitPriceController.text = FormatYardimcisi.sayiFormatlaOndalikli(
      _selectedSalePrice(product),
  ...
  void _selectSalePriceGroup(int group) {
    setState(() => _selectedSalePriceGroup = group);
    final currentProduct = _currentProduct;
    if (currentProduct != null) {
      _setUnitPriceFromProduct(currentProduct);
    }
  }

  String _salePriceGroupTooltip(int group) {
    final product = _currentProduct;
    if (product == null) return '${tr('sale.price_group')} $group';

    final double price = switch (group) {
      2 => product.satisFiyati2,
      3 => product.satisFiyati3,
      _ => product.satisFiyati1,
    };
    ...
    return '${tr('sale.price_group')} $group: $formatted';
  }

  Widget _buildSalePriceGroupSelector({bool isDense = false}) {
    final double size = isDense ? 28 : 30;
    ...
    Widget buildButton(int group) {
      final bool selected = _selectedSalePriceGroup == group;
      final Color borderColor = selected ? _primaryColor : Colors.grey.shade300;
      final Color backgroundColor = selected
          ? _primaryColor
          : Colors.grey.shade50;
      final Color textColor = selected ? Colors.white : Colors.grey.shade700;
    ...
    return Row(
      ...
      children: List.generate(groups.length * 2 - 1, (index) {
        ...
        return buildButton(groups[index ~/ 2]);
      }),
    );
  }
  ...
  void _fillProductFields(UrunModel p, {bool updateCodeField = true}) {
    setState(() {
      _currentProduct = p;
      _selectedSalePriceGroup = _resolveSalePriceGroupForProduct(
        p,
      ...
      // Cihazları getir ve eğer varsa seçim diyaloğunu göster
      _loadDevicesAndShowSelection(p);
    });
  }

  Future<void> _loadDevicesAndShowSelection(UrunModel p) async {
    final devices = await UrunlerVeritabaniServisi().cihazlariGetir(p.id);
    setState(() {
      _availableDevices = devices;
    });

    if (devices.isNotEmpty && _selectedSerialNumber == null) {
      _showIMEISelectionDialog();
    }
  }

  void _showIMEISelectionDialog() {
                            ...
                            children: [
                              ...
                              Text(
                                tr(
                                  'common.items_count',
                                  args: {
                                    'count': _availableDevices.length
                                        .toString(),
                                  },
                                ),
                                style: const TextStyle(
  ...
  Future<void> _addItem() async {
    ...
    final newItem = SaleItem(
      ...
      serialNumber: _selectedSerialNumber, // IMEI bilgisini ekle
    );

    setState(() {
      final existingIndex = _items.indexWhere(
        (e) => e.code == newItem.code && e.warehouseId == newItem.warehouseId,
      );
      if (existingIndex != -1) {
  ...
  Future<void> _completeSale() async {
    ...
    // SatisTamamlaSayfasi'na yönlendir
    final result = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (context) => SatisTamamlaSayfasi(
          ...
          orderRef: widget.initialOrderRef != null
    ...
    if (result == true && mounted) {
      // Satış başarıyla tamamlandı, sayfayı sıfırla
      _resetPage();
  ...
  Widget _buildTableActions() {
    return Row(
      ...
      children: [
        ...
        const SizedBox(width: 8),
        Tooltip(
          message: '${tr('sale.button.invoice_discount')} (F5)',
          child: MouseRegion(
            cursor: _items.isEmpty
                ? SystemMouseCursors.basic
                : SystemMouseCursors.click,
                          ...
                          child: Form(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                _buildSection(
                                  title:
                                      '${tr('sale.title')} Listesi (${_items.length})',
  ...
  Widget _buildProductEntryFields({bool isDense = false}) {
    final product = _currentProduct;
    final bool showSalePriceSelector =
        product != null &&
        (product.satisFiyati1 > 0 ||
            product.satisFiyati2 > 0 ||
            product.satisFiyati3 > 0);
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final bool veryNarrow = constraints.maxWidth < 460;
        ...
        final codeField = _buildProductAutocompleteField(
          ...
          searchHint: 'Kod, İsim, Barkod, IMEI / Seri No',
          isRequired: true,
          isCodeField: true,
          isDense: denseMode,
          suffixIcon: IconButton(
        ...
        final nameField = _buildProductAutocompleteField(
          ...
          label: tr('sale.field.product_name'),
          searchHint: 'İsim, Kod, IMEI / Seri No',
          isRequired: true,
          isCodeField: false,
          isDense: denseMode,
          suffixIcon: IconButton(
        ...
        final Widget priceField = Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (showSalePriceSelector && veryNarrow) ...[
              _buildInputField(
                controller: _unitPriceController,
                label: tr('sale.field.unit_price'),
                isRequired: true,
                isNumeric: true,
                isDense: denseMode,
                maxDecimalDigits: _genelAyarlar.fiyatOndalik,
              ...
              Align(
                ...
                child: _buildSalePriceGroupSelector(isDense: true),
              ),
            ] else
              Row(
                ...
                children: [
                  ...
                  if (showSalePriceSelector) ...[
                    const SizedBox(width: 8),
                    _buildSalePriceGroupSelector(isDense: denseMode),
                  ],
                ],
              ),
        ...
        final vatStatusField = _buildDropdownField<String>(
          ...
          items: [
            DropdownMenuItem(
              value: 'excluded',
              child: Text(tr('sale.field.vat_excluded')),
            ),
            DropdownMenuItem(
              value: 'included',
              child: Text(tr('sale.field.vat_included')),
            ),
          ],
        ...
        final quantityField = _buildInputField(
          ...
          label: tr('sale.field.quantity'),
          isRequired: true,
          isNumeric: true,
          isDense: denseMode,
          maxDecimalDigits: _genelAyarlar.miktarOndalik,
          focusNode: _quantityFocusNode,
          onFieldSubmitted: (_) => _completeSale(),
          focusOrder: 16,
        );
            ...
            child: SingleChildScrollView(
              ...
              child: ConstrainedBox(
                ...
                child: DataTable(
                  ...
                  columns: [
                    DataColumn(label: Text(tr('sale.grid.warehouse'))),
                    DataColumn(label: Text(tr('sale.grid.code'))),
                    DataColumn(label: Text(tr('sale.grid.barcode'))),
                    DataColumn(label: Text(tr('sale.grid.name'))),
                    DataColumn(
                      label: Text(
                        tr('sale.grid.vat'),
                    ...
                    DataColumn(
                      label: Text(
                        tr('sale.grid.quantity'),
                        textAlign: TextAlign.right,
                      ),
                      numeric: true,
                    ),
                    DataColumn(label: Text(tr('sale.grid.unit'))),
                    DataColumn(
                      label: Text(
                        tr('sale.grid.total'),
                        textAlign: TextAlign.right,
      ...
      child: Wrap(
        ...
        children: [
          ...
          _buildMobileActionButton(
            ...
            label: tr('sale.button.invoice_discount'),
            color: _primaryColor,
            onTap: _items.isEmpty ? null : _showInvoiceDiscountDialog,
          ),
        ],
  ...
  Widget _buildMobileItemCard(int index, SaleItem item) {
          ...
          children: [
            ...
            _buildResponsiveRow(
              ...
              children: [
                ...
                _buildMobileEditableCellBlock(
                  index: index,
                  label: '${tr('sale.grid.quantity')} (${item.unit})',
                  field: 'quantity',
                  value: item.quantity,
                  onSubmitted: (val) {
                    setState(() {
              ...
              child: Wrap(
                ...
                children: [
                  _buildMobileMetaChip(
                    ...
                    value:
                        '%${FormatYardimcisi.sayiFormatlaOran(item.vatRate, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci)} • ${item.vatIncluded ? tr('sale.field.vat_included') : tr('sale.field.vat_excluded')}',
                  ),
                  if (_genelAyarlar.otvKullanimi || item.otvRate > 0)
                    _buildMobileMetaChip(
                      label: tr('sale.grid.otv'),
                      value:
                          '%${FormatYardimcisi.sayiFormatlaOran(item.otvRate, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci)} • ${item.otvIncluded ? tr('sale.field.vat_included') : tr('sale.field.vat_excluded')}',
                    ),
                  if (_genelAyarlar.oivKullanimi || item.oivRate > 0)
                    _buildMobileMetaChip(
                      label: tr('sale.grid.oiv'),
                      value:
                          '%${FormatYardimcisi.sayiFormatlaOran(item.oivRate, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci)} • ${item.oivIncluded ? tr('sale.field.vat_included') : tr('sale.field.vat_excluded')}',
                    ),
            ...
            if (item.barcode.isNotEmpty) ...[
              ...
              Align(
                alignment: Alignment.centerLeft,
                child: Text(
                  '${tr('sale.grid.barcode')}: ${item.barcode}',
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                  style: TextStyle(
                    fontSize: 11,
                        ...
                        children: [
                          Padding(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                color: IslemTuruRenkleri.satisArkaplan,
                                borderRadius: BorderRadius.circular(8),
                              ),
                              child: Align(
                                alignment: Alignment.centerRight,
                                child: Text(
                                  tr('sale.footer.grand_total'),
                                  style: const TextStyle(
                                    fontSize: 14,
                                    fontWeight: FontWeight.bold,
                                    color: IslemTuruRenkleri.satisMetin,
                                  ),
                                ),
                              ),
                            ),
            ...
            return RawAutocomplete<UrunModel>(
              ...
              optionsBuilder: (TextEditingValue textEditingValue) async {
                ...
                _searchDebounce = Timer(
                  ...
                  () async {
                    try {
                      // Hem ürünleri hem üretimleri paralel olarak ara
                      ...
                      // Üretimleri UrunModel'e dönüştür
  ...
  String _fmt(double val) => NumberFormat("#,##0.00", "tr_TR").format(val);
}
