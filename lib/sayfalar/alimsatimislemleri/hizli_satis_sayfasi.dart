...
class _HizliSatisSayfasiState extends State<HizliSatisSayfasi> {
  ...
  Future<void> _openProductSearchDialog({String initialQuery = ''}) async {
    ...
    if (selected is UrunModel) {
      product = selected;
    } else if (selected is UretimModel) {
      product = _productionToProduct(selected);
    } else {
      return;
    }

    await _appendProductRow(product);
    if (!mounted) return;
...
class _HizliSatisUrunSearchDialog extends StatefulWidget {
  final String initialQuery;

  const _HizliSatisUrunSearchDialog({this.initialQuery = ''});

  @override
  State<_HizliSatisUrunSearchDialog> createState() =>
