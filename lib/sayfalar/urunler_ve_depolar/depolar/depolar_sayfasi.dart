...
import '../urunler/urun_karti_sayfasi.dart';
...
class _DepolarSayfasiState extends State<DepolarSayfasi> {
  ...
  Future<void> _fetchDepolar({bool showLoading = true}) async {
    ...
    try {
      ...
      if (hasTxFilters) {
        filterParts.add('table:shipments');
      }
      ...
      if (primary.indexEnabled) {
        ...
        depolar = await DepolarVeritabaniServisi().depolariGetir(
          sayfa: _currentPage,
        ...
        if (hasMore) depolar = depolar.sublist(0, _rowsPerPage);
      }
  ...
  Future<void> _loadMoreDetailTransactions(
    ...
    if (_detailTransactionsLoading[depo.id] == true) return;

    final existing =
        _detailTransactionsCache[depo.id] ??
        List<Map<String, dynamic>>.from(initialTransactions);
    if (existing.isEmpty) {
      setState(() => _detailTransactionsHasMore[depo.id] = false);
      return;
    }
    ...
    final dynamic lastDateRaw =
        existing.last['date_raw'] ?? existing.last['date'];
    final DateTime? lastDate = lastDateRaw is DateTime
        ? lastDateRaw
        : DateTime.tryParse(lastDateRaw?.toString() ?? '');

    if (lastId == null || lastId <= 0 || lastDate == null) {
      setState(() => _detailTransactionsHasMore[depo.id] = false);
      return;
    }

    setState(() {
      _detailTransactionsLoading[depo.id] = true;
      _detailTransactionsCache[depo.id] = existing;
    ...
    try {
      final more = await DepolarVeritabaniServisi().depoIslemleriniGetir(
        depo.id,
        aramaTerimi: _searchQuery,
        baslangicTarihi: _startDate,
        bitisTarihi: _endDate,
      ...
      setState(() {
        _detailTransactionsCache[depo.id] = [...existing, ...more];
        _detailTransactionsHasMore[depo.id] =
            more.length == _detailTransactionsPageSize;
        _detailTransactionsLoading[depo.id] = false;
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _detailTransactionsLoading[depo.id] = false;
        _detailTransactionsHasMore[depo.id] = false;
      });
      debugPrint('Depo detay yükleme hatası: $e');
    }
  }
  ...
  Future<void> _openUrunKartiByProductCode(String productCode) async {
    final code = productCode.trim();
    if (code.isEmpty) return;

    try {
      final urun = await UrunlerVeritabaniServisi().urunGetir(kod: code);
      if (!mounted) return;
  ...
  Widget _buildTransactionRow({
    ...
    return Builder(
      builder: (ctx) {
        ...
        final String relatedAccountLower = (relatedAccount ?? '')
            .toString()
            .toLowerCase();
        final bool isRetailSale =
            relatedAccountLower.contains('perakende satış') ||
            relatedAccountLower.contains('perakende satis');
        final String productCodeTrimmed = (productCode ?? '').trim();
        final bool canGoToProductCard =
            isRetailSale && productCodeTrimmed.isNotEmpty;

        Widget actionWidget;
        if (hideActionMenuForOpeningStock) {
          actionWidget = const SizedBox.shrink();
        } else if (canGoToProductCard) {
          actionWidget = Center(
            child: Tooltip(
              ...
              child: InkWell(
                mouseCursor: WidgetStateMouseCursor.clickable,
                onTap: () async {
                  await _openUrunKartiByProductCode(productCodeTrimmed);
                },
                borderRadius: BorderRadius.circular(6),
                child: Container(
          ...
          actionWidget = _buildTransactionPopupMenu(
            id,
            customTypeLabel,
            productCode,
            productName,
  ...
  Widget _buildMobileView(List<DepoModel> filteredDepolar) {
    ...
    final int showingStart = filteredDepolar.isEmpty ? 0 : startRecordIndex + 1;
    final int endRecord = filteredDepolar.isEmpty
        ? 0
        : startRecordIndex + filteredDepolar.length;
    final String totalLabel = filteredDepolar.isEmpty
        ? '0'
        ...
        FutureBuilder<List<dynamic>>(
          ...
          builder: (context, snapshot) {
            ...
            final initialTransactions =
                (snapshot.data?[0] as List<Map<String, dynamic>>?) ?? [];
            final cached = _detailTransactionsCache[depo.id];
            final depotTransactions = cached ?? initialTransactions;
            final bool isLoadingMore =
                _detailTransactionsLoading[depo.id] == true;
            final bool hasMore =
                _detailTransactionsHasMore[depo.id] ??
            ...
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (!mounted) return;
              _visibleTransactionIds[depo.id] = depotTransactions
                  .map((t) => t['id'])
                  .whereType<int>()
                  .toList(growable: false);
            });
