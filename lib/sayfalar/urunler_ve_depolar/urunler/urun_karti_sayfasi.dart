...
class _UrunKartiSayfasiState extends State<UrunKartiSayfasi> {
  ...
  static const int _transactionsPageSize = 250;
  List<Map<String, dynamic>> _allTransactionsRaw = [];
  bool _transactionsHasMore = false;
  bool _transactionsLoadingMore = false;
  int _totalRecords = 0;
  Map<String, Map<String, int>> _filterStats = {};
  int _transactionsRequestToken = 0;
  ...
  int _serialTotalRecords = 0; // Cursor modunda 0 (COUNT yok)
  bool _serialHasNextPage = false;
  int _serialCurrentPage = 1;
  int _serialRowsPerPage = 25;
  final Map<int, int> _serialPageCursors = {}; // page -> lastDeviceId
  Object _serialPaginationResetKey = Object();
  int _serialRequestToken = 0;
  bool _serialIncludeSold = false; // stokta cihaz yoksa true (satılanlar dahil)
  ...
  Map<String, dynamic> _mapProductMovementToRow(Map<String, dynamic> t) {
    return {
      'id': t['id'],
  ...
  Future<void> _loadTransactions() async {
    try {
      final int requestToken = ++_transactionsRequestToken;
      ...
      if (!mounted || requestToken != _transactionsRequestToken) return;

      final mappedTransactions = transactions
          .map(_mapProductMovementToRow)
          .toList();
      ...
      if (mounted) {
        setState(() {
          ...
          _allTransactionsRaw = mappedTransactions;
  ...
  Future<void> _loadMoreTransactions() async {
    ...
    try {
      ...
      final more = await depoServis.urunHareketleriniGetir(
        _currentUrun.kod,
        kdvOrani: _currentUrun.kdvOrani,
        limit: _transactionsPageSize,
        aramaTerimi: _searchQuery.trim().isNotEmpty
      ...
      final mappedMore = more.map(_mapProductMovementToRow).toList();

      if (!mounted) return;
      if (requestToken != _transactionsRequestToken) return;

      final newRaw = [...existingRaw, ...mappedMore];

      final Map<String, int> unitCounts = {};
      for (final tx in newRaw) {
        final String u = (tx['birim'] ?? '').toString().trim();
        if (u.isEmpty) continue;
        unitCounts[u] = (unitCounts[u] ?? 0) + 1;
      }
      ...
      setState(() {
        ...
        _allTransactionsRaw = newRaw;
        _cachedTransactions = searchResult.filtered;
        _totalRecords = searchResult.filtered.length;
        _transactionsHasMore = more.length == _transactionsPageSize;
        _transactionsLoadingMore = false;
  ...
  Future<void> _toggleSerialListMode() async {
    ...
    // [2026] İlk geçişte cihaz "son işlem" kolonlarını best-effort doldur (tam geçmiş).
    // Bu sayede arama + tarih filtresi limitsiz çalışır.
    try {
      await UrunlerVeritabaniServisi().urunCihazSonIslemleriniBackfillEt(
        productId: _currentUrun.id,
        urunKodu: _currentUrun.kod,
        kdvOrani: _currentUrun.kdvOrani,
      );
    } catch (e) {
      debugPrint('Cihaz son işlem backfill uyarısı: $e');
    }

    await _loadSerialListRows(page: 1, rowsPerPage: _serialRowsPerPage);
  }

  bool _serialQueryMatchesGlobalFields(String query) {
    ...
    // Bu alanlar satır bazında değişmiyor (tüm cihazlar için aynı).
    // İn-memory aramada hepsi eşleşeceği için backend'e filtre göndermeyiz (sonuç: tüm satırlar).
  ...
  Future<void> _loadSerialListRows({int? page, int? rowsPerPage}) async {
    final int requestToken = ++_serialRequestToken;

    final int targetPage = (page ?? _serialCurrentPage).clamp(1, 1 << 30);
    final int requestedRpp = rowsPerPage ?? _serialRowsPerPage;
    final int effectiveRpp = (requestedRpp <= 0 ? 25 : requestedRpp).clamp(
      10,
      250,
    );
    ...
    try {
      ...
      // Stokta cihaz yoksa: satılanlar dahil listele (mevcut davranışla uyumlu).
      bool includeSold = _serialIncludeSold;
      if (targetPage == 1) {
        final hasUnsold = await urunlerServis.urunStoktaCihazVarMi(
          _currentUrun.id,
        );
        includeSold = !hasUnsold;
      }

      final int? lastId = targetPage > 1
          ? _serialPageCursors[targetPage - 1]
          : null;

      if (targetPage > 1 && lastId == null) {
        // Cursor yoksa (random jump vs): güvenli şekilde ilk sayfaya dön.
        if (!mounted || requestToken != _serialRequestToken) return;
        setState(() {
          _serialPageCursors.clear();
          _serialCurrentPage = 1;
          _serialHasNextPage = false;
        ...
        unawaited(_loadSerialListRows(page: 1, rowsPerPage: effectiveRpp));
        return;
      }
      ...
      final String? dbQuery =
          trimmedQuery.isNotEmpty &&
              !_serialQueryMatchesGlobalFields(trimmedQuery)
          ? trimmedQuery
          : null;

      final devices = await urunlerServis.urunCihazlariniSayfalaGetir(
        productId: _currentUrun.id,
        limit: effectiveRpp + 1,
        lastId: lastId,
      ...
      final bool hasMore = devices.length > effectiveRpp;
      final display = hasMore ? devices.sublist(0, effectiveRpp) : devices;
      ...
      for (final d in display) {
        final int? deviceId = asInt(d['id']);
        if (deviceId == null) continue;

        final String identity = (d['identity_value'] ?? '').toString().trim();
        if (identity.isEmpty) continue;

        final DateTime? dt = asDateTime(d['last_tx_at']);
        final String rawType = (d['last_tx_type'] ?? '').toString().trim();

        final String islemText = rawType.isEmpty
            ? '-'
            : _formatStockTransactionTypeLabel(rawType);

        final String tarihText = dt == null
            ? '-'
      ...
      setState(() {
        _serialIncludeSold = includeSold;
        _serialCurrentPage = targetPage;
        _serialRowsPerPage = effectiveRpp;
        _serialHasNextPage = hasMore;
        _serialTotalRecords = 0;
        _cachedSerialRows = rows;
        _isSerialListLoading = false;
        if (rows.isNotEmpty) {
          _serialPageCursors[targetPage] = rows.last['id'] as int;
        }
        _isSerialNumberedProduct = _isSerialNumberedProduct || rows.isNotEmpty;
      });
    } catch (e) {
      debugPrint('Seri liste yükleme hatası: $e');
      if (!mounted || requestToken != _serialRequestToken) return;
      setState(() {
        _cachedSerialRows = const <Map<String, dynamic>>[];
        _serialTotalRecords = 0;
        _serialHasNextPage = false;
        _isSerialListLoading = false;
      });
  ...
  _filterTransactionsForSearch(List<Map<String, dynamic>> base, String query) {
    ...
    // Arama sonuçları DB tarafından filtrelenir. UI tarafı sadece "expanded cihaz" eşleşmeleri
    // için auto-expand ID seti üretir (in-memory filtre YOK).
    return (filtered: base, expandedIds: expandedIds);
  }
  ...
  void _applySearchQuery(String query) {
    if (_isSerialListMode) {
      ...
      unawaited(_loadSerialListRows(page: 1, rowsPerPage: _serialRowsPerPage));
      return;
    }

    // [2026] Arama, in-memory filtre yerine DB (search_tags + cursor) üzerinden yapılır.
    // Böylece 5 yıllık geçmişte de "şak" arama bozulmadan çalışır.
    if (mounted) {
      setState(() {
        _searchQuery = query;
        _cachedTransactions = const <Map<String, dynamic>>[];
        _allTransactionsRaw = const <Map<String, dynamic>>[];
        _totalRecords = 0;
        _transactionsHasMore = false;
        _transactionsLoadingMore = false;
        _searchAutoExpandedTransactionIds.clear();
  ...
  Future<void> _handlePrint() async {
    ...
    final Set<int> selectedIds = Set<int>.from(_selectedTransactionIds);

    List<Map<String, dynamic>> dataToProcess;
    int serialOrderStart = 1;
    ...
    if (isSerialMode) {
      if (selectedIds.isNotEmpty) {
        ...
        final devices = await UrunlerVeritabaniServisi().cihazlariIdIleGetir(
          ids: selectedIds.toList(growable: false),
        );
        ...
        serialOrderStart = ((_serialCurrentPage - 1) * _serialRowsPerPage) + 1;
        dataToProcess = _cachedSerialRows;
      }
    } else {
      final source = _cachedTransactions;
      dataToProcess = selectedIds.isNotEmpty
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              tr('print.error.during_print', args: {'error': e.toString()}),
            ),
            backgroundColor: Colors.red,
          ),
        );
          ...
          child: LayoutBuilder(
            builder: (context, constraints) {
              ...
              if (forceMobile || constraints.maxWidth < 1000) {
                return _buildMobileView(theme, showAppBar: showAppBar);
              }
  ...
  Widget _buildMobileView(ThemeData theme, {required bool showAppBar}) {
    ...
    final bool isSerialMode = _isSerialListMode;
    final List<Map<String, dynamic>> source = isSerialMode
        ? _cachedSerialRows
        : _cachedTransactions;

    final int safeRowsPerPage = isSerialMode
        ? (_serialRowsPerPage <= 0 ? 25 : _serialRowsPerPage)
        : (_mobileRowsPerPage <= 0 ? 25 : _mobileRowsPerPage);
    ...
    if (isSerialMode) {
      effectivePage = _serialCurrentPage < 1 ? 1 : _serialCurrentPage;
      ...
      totalLabel = paginatedRows.isEmpty
          ? '0'
          : (_serialHasNextPage ? '$endIndex+' : '$endIndex');
    } else {
      totalPages = totalRecords == 0
          ? 1
          : (totalRecords / safeRowsPerPage).ceil();
      effectivePage = _mobileCurrentPage.clamp(1, totalPages);
    ...
    final int toolbarRecords = isSerialMode ? endRecord : totalRecords;
            ...
            if (!isKeyboardVisible)
              Padding(
                ...
                child: Row(
                  children: [
                    IconButton(
                      onPressed: isSerialMode
                          ? (effectivePage > 1 && !_isSerialListLoading
                                ? () => _loadSerialListRows(
                    ...
                    IconButton(
                      onPressed: isSerialMode
                          ? (_serialHasNextPage && !_isSerialListLoading
                                ? () => _loadSerialListRows(
                                    page: effectivePage + 1,
                                    rowsPerPage: safeRowsPerPage,
                                  )
  ...
  Widget _buildMobileTopActionRow() {
    ...
    final String selectAllTooltip = _isSelectAllActive
        ? tr('common.clear')
        ...
        if (canToggleSerialList) ...[
          ...
          Expanded(
            child: _buildMobileActionButton(
              ...
              textColor: isSerialMode ? Colors.white : const Color(0xFF2C3E50),
                                    ...
                                    child: DropdownButtonHideUnderline(
                                      child: DropdownButton<int>(
                                        ...
                                        onChanged: (val) {
                                          ...
                                          setState(() {
                                            if (_isSerialListMode) {
                                              _serialRowsPerPage = val;
                                              _serialCurrentPage = 1;
                                              _serialHasNextPage = false;
                                              _serialPageCursors.clear();
                                              _serialPaginationResetKey =
                                                  Object();
                                            } else {
  ...
  Widget _buildMobileTransactionCard(
    ...
    final String relatedParty = (tx['relatedPartyName'] ?? '')
        .toString()
        .trim();
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              if (hasBox)
                _buildMobileMetaChip(tr('common.gadget.box'), tr('common.yes')),
              if (hasInvoice)
      ...
      child: Column(
        ...
        children: [
          ...
          AnimatedCrossFade(
            firstChild: Builder(
              builder: (context) {
                final EdgeInsetsGeometry sectionPadding = compact
                    ? EdgeInsets.zero
                    : const EdgeInsets.symmetric(horizontal: 16);

                final Widget stockTax = _buildDetailColumn(
                ...
                final Widget pricing = _buildDetailColumn(
                ...
                final double maxHeight =
                    (MediaQuery.sizeOf(context).height * 0.38).clamp(
                      240.0,
                      420.0,
                    );
  ...
  Widget _buildSerialListDesktopView(BoxConstraints constraints) {
    final colBarkodWidth = _calculateHeaderWidth(
      tr('common.barcode'),
      minWidth: 140.0,
    );
    ...
    final colKdvWidth = _calculateHeaderWidth(
      tr('common.vat_short'),
      minWidth: 150.0,
    );
    final colKdvOranWidth = _calculateHeaderWidth(
      tr('common.vat_rate'),
      minWidth: 90.0,
    );
    ...
    return GenisletilebilirTablo<Map<String, dynamic>>(
      ...
      hasNextPage: _serialHasNextPage,
      paginationResetKey: _serialPaginationResetKey,
      searchFocusNode: _searchFocusNode,
      onClearSelection: _clearAllTableSelections,
      onFocusedRowChanged: (item, index) {
        if (item != null) {
      ...
      onPageChanged: (page, rowsPerPage) =>
          _loadSerialListRows(page: page, rowsPerPage: rowsPerPage),
      onSearch: (query) {
        if (_debounce?.isActive ?? false) _debounce!.cancel();
        _debounce = Timer(const Duration(milliseconds: 200), () {
          if (!mounted) return;
    ...
    return Row(
      children: [
        ...
        Container(
          ...
          child: HighlightText(
            text:
                '${((_serialCurrentPage - 1) * _serialRowsPerPage) + index + 1}',
            query: query,
