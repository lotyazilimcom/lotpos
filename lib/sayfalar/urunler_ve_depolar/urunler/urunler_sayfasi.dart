...
class _UrunlerSayfasiState extends State<UrunlerSayfasi> {
  ...
  Future<void> _fetchUrunler({bool showLoading = true}) async {
    ...
    try {
      ...
      final urunler = await UrunlerVeritabaniServisi().urunleriGetir(
        sayfa: _currentPage,
        ...
        // If we have the cursor for the previous page, use it (Fast Keyset).
        // If not (e.g. random jump), pass null (Fallback to Offset).
        lastId: _currentPage > 1 ? _pageCursors[_currentPage - 1] : null,
      );

      if (!mounted || sorguNo != _aktifSorguNo) return;

      // Update Cursor Map for the current page
      if (urunler.isNotEmpty) {
        // We map the *current* page number to the last Item ID of this page.
        // So when we go to page+1, we can retrieve _pageCursors[page].
        _pageCursors[_currentPage] = urunler.last.id;
      } else {
        // If empty, maybe clear subsequent?
      }
  ...
  Future<void> _sayimVeIstatistikleriArkaPlandaGetir({
    ...
    final totalFuture = UrunlerVeritabaniServisi().urunSayisiGetir(
    ...
    try {
      ...
      debugPrint('Ürün toplam sayısı güncellenemedi: $e');
    }
  ...
  Widget _buildDesktopView(
    ...
    return GenisletilebilirTablo<UrunModel>(
      ...
      onRowDoubleTap: (urun) {
        TabAciciScope.of(
          context,
        )?.tabAc(menuIndex: TabAciciScope.urunKartiIndex, initialUrun: urun);
      },
                          ...
                          _buildTransactionRow(
                            ...
                            onTap: () {
                              ...
                              setState(() {
                                ...
                                _selectedDetailWarehouseName = tx['warehouse'];
                              });
                            },
  ...
  Widget _buildMobileView(List<UrunModel> filteredUrunler) {
    final int totalRecords = filteredUrunler.length;
    ...
    final int startRecord = (effectivePage - 1) * safeRowsPerPage;
    final int endRecord = (startRecord + safeRowsPerPage).clamp(
      0,
      totalRecords,
    );
    final int showingStart = totalRecords == 0 ? 0 : startRecord + 1;

    final paginatedUrunler = filteredUrunler
        .skip(startRecord)
          ...
          children: [
            ...
            Expanded(
              child: ListView.separated(
                ...
                keyboardDismissBehavior:
                    ScrollViewKeyboardDismissBehavior.onDrag,
                itemCount: paginatedUrunler.length,
                separatorBuilder: (context, index) =>
                    const SizedBox(height: 16),
                itemBuilder: (context, index) {
                  return _buildUrunCard(paginatedUrunler[index]);
                },
              ),
            ),
            if (!isKeyboardVisible)
