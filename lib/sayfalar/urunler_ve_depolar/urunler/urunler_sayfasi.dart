...
import '../../../servisler/arama/arama_primary_path.dart';
...
class _UrunlerSayfasiState extends State<UrunlerSayfasi> {
  ...
  bool _consumeMobileDoubleTap(int urunId) {
    final int now = DateTime.now().millisecondsSinceEpoch;
    final bool isDoubleTap =
        _lastMobileTappedUrunId == urunId && (now - _lastMobileTapAtMs) <= 350;

    _lastMobileTappedUrunId = urunId;
    _lastMobileTapAtMs = now;

    if (isDoubleTap) {
      _lastMobileTappedUrunId = null;
      _lastMobileTapAtMs = 0;
    }

    return isDoubleTap;
  }

  Future<void> _openUrunKarti(UrunModel urun) async {
    final tabScope = TabAciciScope.of(context);
    if (tabScope != null) {
      tabScope.tabAc(
        menuIndex: TabAciciScope.urunKartiIndex,
        initialUrun: urun,
      );
      return;
    }
    ...
    await Navigator.of(context).push(
      MaterialPageRoute(builder: (context) => UrunKartiSayfasi(urun: urun)),
    );
  }
  ...
  void _resetPagination() {
    ...
    _currentPage = 1;
    _hasNextPage = false;
    _totalRecords = 0; // COUNT(*) kullanmıyoruz (cursor footer)
  }

  int _rowsPerPage = 25;

  int _currentPage = 1;
  // Keyset Pagination: Map<PageIndex, LastItemID>
  // Stores the last ID of previous pages to use as cursors.
  ...
  static const int _detailTransactionsPageSize = 250;
  final Map<int, List<Map<String, dynamic>>> _detailTransactionsCache = {};
  final Map<int, bool> _detailTransactionsHasMore = {};
  final Map<int, bool> _detailTransactionsLoading = {};
  ...
  void initState() {
    ...
    unawaited(_ilkAcilisYuklemeAkisi());

    _searchController.addListener(() {
      if (_debounce?.isActive ?? false) _debounce!.cancel();
      _debounce = Timer(const Duration(milliseconds: 500), () {
        final raw = _searchController.text.toLowerCase();
        final trimmed = raw.trim();
        final effectiveQuery = trimmed.isNotEmpty && trimmed.length < 3
            ? ''
            : raw;
        if (effectiveQuery != _searchQuery) {
          setState(() {
            _searchQuery = effectiveQuery;
            _resetPagination(); // Reset cursors on search
          });
          _fetchUrunler();
        }
      });
  ...
  void _onGlobalSync() {
    // Veri değiştiyse facet sayımları da değişebilir; cache'i boz.
    _lastStatsKey = '';
    _fetchUrunler(showLoading: false);
  }

  Future<void> _ilkAcilisYuklemeAkisi() async {
    await _fetchUrunler();
    unawaited(
      _loadSettings().catchError((e) {
        debugPrint('Ayar yükleme hatası: $e');
      }),
    );
    unawaited(
      _loadAvailableFilters().catchError((e) {
        debugPrint('Filtre metadata yükleme hatası: $e');
      }),
    );
    unawaited(
      _loadAvailableUsers().catchError((e) {
        debugPrint('Kullanıcı metadata yükleme hatası: $e');
      }),
    );
    unawaited(
      _fetchWarehouses().catchError((e) {
        debugPrint('Depo metadata yükleme hatası: $e');
      }),
    );
  }

  Future<void> _fetchUrunler({bool showLoading = true}) async {
    // Clear detail cache when refreshing main list
    _detailFutures.clear();
    _detailTransactionsCache.clear();
    _detailTransactionsHasMore.clear();
    _detailTransactionsLoading.clear();
    _imageCache.clear();
    ...
    try {
      ...
      final List<int>? depoIds = _selectedWarehouse != null
          ? [_selectedWarehouse!.id]
          : null;

      // [2026] Cursor sayfalama: asla OFFSET'e düşme.
      // Eğer gerekli cursor yoksa güvenli şekilde 1. sayfaya dön.
      if (_currentPage > 1) {
        if (_pageCursors[_currentPage - 1] == null) {
          _resetPagination();
        }
      }
      ...
      String osQuote(String input) {
        final s = input.trim();
        if (s.isEmpty) return '';
        return s.replaceAll(r'\', r'\\').replaceAll('"', r'\"');
      }

      int toEpochSecond(DateTime dt) =>
          dt.toUtc().millisecondsSinceEpoch ~/ 1000;
      String? osDateRange(DateTime? start, DateTime? endInclusive) {
        final s = start == null
            ? null
            : toEpochSecond(DateTime(start.year, start.month, start.day));
        final e = endInclusive == null
            ? null
            : toEpochSecond(
                DateTime(
                  endInclusive.year,
        ...
        if (s != null && e != null) return 'date:[$s TO $e}';
        if (s != null) return 'date:[$s TO *]';
        return 'date:[* TO $e}';
      }

      final String dir = _sortAscending ? 'asc' : 'desc';
      final String indexSortBy = switch (_sortBy) {
        'kod' => 'root_code.keyword:$dir',
        'ad' => 'root_name.keyword:$dir',
        'stok' => 'root_stock:$dir',
        'birim' => 'root_unit:$dir',
        'fiyat' => 'root_price:$dir',
        'satis_fiyati_1' => 'root_sale_price_1:$dir',
        'aktif_mi' => 'root_aktif_mi:$dir',
        _ => 'root_id:$dir',
      };
      ...
      final List<String> indexTables =
          ...
          ? const <String>['products', 'stock_movements']
          : const <String>['products'];

      final filterParts = <String>[];
      if (aktifMi != null) {
        filterParts.add('root_aktif_mi:${aktifMi ? 'true' : 'false'}');
      }
      final group = (_selectedGroup ?? '').trim();
      if (group.isNotEmpty) {
        filterParts.add('root_group:"${osQuote(group)}"');
      }
      final unit = (_selectedUnit ?? '').trim();
      if (unit.isNotEmpty) {
        filterParts.add('root_unit:"${osQuote(unit)}"');
      }
      if (_selectedVat != null) {
        filterParts.add('root_vat:${_selectedVat!}');
      }
      if (_selectedWarehouse != null) {
        filterParts.add('warehouse_id:${_selectedWarehouse!.id}');
      }
      final user = (_selectedUser ?? '').trim();
      if (user.isNotEmpty) {
        filterParts.add('user_name:"${osQuote(user)}"');
      }
      final islemTuru = (_selectedTransactionType ?? '').trim();
      if (islemTuru.isNotEmpty) {
        filterParts.add('source_type:"${osQuote(islemTuru)}"');
      }
      final dateRange = osDateRange(_startDate, _endDate);
      if (dateRange != null) filterParts.add(dateRange);

      final String? indexExtraFilter = filterParts.isEmpty
          ? null
          : filterParts.join(' AND ');

      final String? prevCursorRaw = _currentPage > 1
          ? _pageCursors[_currentPage - 1]
          : null;
      final String? indexCursor =
          (prevCursorRaw == null ||
              prevCursorRaw.trim().isEmpty ||
              prevCursorRaw.contains(':'))
          ? null
          : prevCursorRaw.trim();
      ...
      if (!primary.indexEnabled) {
        ...
        if (prev.startsWith('id:')) {
          lastId = int.tryParse(prev.substring(3).trim());
        }

        urunler = await UrunlerVeritabaniServisi().urunleriGetir(
          sayfa: _currentPage,
          // [2026] COUNT(*) olmadan "hasNext" tespiti için +1 satır çek.
          sayfaBasinaKayit: _rowsPerPage + 1,
          aramaTerimi: _searchQuery,
          sortBy: _sortBy,
        ...
        hasMore = urunler.length > _rowsPerPage;
      }

      if (!mounted || sorguNo != _aktifSorguNo) return;

      final List<UrunModel> display = hasMore
          ? urunler.sublist(0, _rowsPerPage)
          : urunler;
      ...
      if (mounted) {
        ...
        if (_selectedWarehouse != null ||
          ...
          for (int i = 0; i < display.length; i++) {
            if (display[i].matchedInHidden) {
              indices.add(i);
              _expandedMobileIds.add(display[i].id);
            }
          }
        }
        ...
        setState(() {
          ...
          if (primary.indexEnabled) {
            ...
            if (display.isNotEmpty) {
              _pageCursors[_currentPage] = 'id:${display.last.id}';
            }
          }
        });
      }
      ...
      unawaited(
        _sayimVeIstatistikleriArkaPlandaGetir(
          sorguNo: sorguNo,
          aramaTerimi: _searchQuery,
          aktifMi: aktifMi,
          grup: _selectedGroup,
  ...
  Future<void> _sayimVeIstatistikleriArkaPlandaGetir({
    required int sorguNo,
    ...
    required List<int>? depoIds,
    required String? islemTuru,
    required String? kullanici,
  }) async {
    final String key =
        '${aramaTerimi ?? ''}|${aktifMi ?? ''}|${grup ?? ''}|${birim ?? ''}|${kdvOrani ?? ''}|${baslangicTarihi?.toIso8601String() ?? ''}|${bitisTarihi?.toIso8601String() ?? ''}|${(depoIds ?? const <int>[]).join(',')}|${islemTuru ?? ''}|${kullanici ?? ''}';
    if (key == _lastStatsKey) return;
    _lastStatsKey = key;

    final statsFuture = UrunlerVeritabaniServisi()
        .urunFiltreIstatistikleriniGetir(
    ...
    try {
      ...
      debugPrint('Ürün filtre istatistikleri güncellenemedi: $e');
      if (_lastStatsKey == key) _lastStatsKey = '';
    }
  }
  ...
  Future<void> _loadSettings() async {
    ...
    if (mounted) {
      setState(() {
        _keepDetailsOpen = prefs.getBool('urunler_keep_details_open') ?? false;
  ...
  Future<void> _toggleKeepDetailsOpen() async {
    ...
    await prefs.setBool('urunler_keep_details_open', _keepDetailsOpen);
  }
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          _buildConfigCheckbox(
                            ...
                            localVisibility,
                            'dt_quantity',
                            tr('products.transaction.quantity'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
                          ...
                          _buildConfigCheckbox(
                            ...
                            localVisibility,
                            'dt_unit_price_vd',
                            tr('products.transaction.unit_price_vd'),
                          ),
                          _buildConfigCheckbox(
                            setDialogState,
                            localVisibility,
  ...
  Future<void> _loadAvailableFilters() async {
    try {
      final groups = await UrunlerVeritabaniServisi().urunGruplariniGetir();
      final units = await UrunlerVeritabaniServisi().urunBirimleriniGetir();
  ...
  void _showVatOverlay() {
    ...
    final vats = _availableVats.isNotEmpty
        ? _availableVats
        : UrunModel.ornekVeriler().map((u) => u.kdvOrani).toSet().toList();
    vats.sort();
  ...
  Future<void> _showQuickAddDialog() async {
    ...
    if (_isMobilePlatform) {
      ...
      result = await showDialog<bool>(
        context: context,
        builder: (context) => const HizliUrunEkleDialog(),
      );
    }
    if (result == true) {
      _fetchUrunler();
  ...
  Future<void> _showEditDialog(
    ...
    final result = await Navigator.of(context).push<bool>(
      PageRouteBuilder(
        pageBuilder: (context, animation, secondaryAnimation) =>
            UrunEkleSayfasi(urun: urun, focusOnStock: focusOnStock),
        transitionsBuilder: (context, animation, secondaryAnimation, child) {
          const begin = Offset(1.0, 0.0);
          const end = Offset.zero;
          const curve = Curves.easeInOut;
  ...
  Future<void> _deleteSelectedUrunler() async {
    ...
    final String hedefEtiketi = _isSelectAllActive
        ? (_totalRecords > 0
              ? '$_totalRecords kayıt (${tr('common.all')})'
              : tr('common.all'))
        : '${_selectedIds.length} kayıt';
    ...
    if (onay == true) {
      if (_isSelectAllActive) {
        await UrunlerVeritabaniServisi().topluUrunSilByFilter(
          aramaTerimi: _searchQuery,
        ...
        await UrunlerVeritabaniServisi().topluUrunSil(_selectedIds.toList());
      }
  ...
  void _deleteUrun(UrunModel urun) async {
    final bool? onay = await showDialog<bool>(
      context: context,
      builder: (context) => OnayDialog(
        baslik: tr('common.confirmation'),
    ...
    if (onay == true) {
      try {
        await UrunlerVeritabaniServisi().urunSil(urun.id);
        if (!mounted) return;
        SayfaSenkronizasyonServisi().veriDegisti('urun');
        ...
        // Exception: prefix'ini temizleyelim daha şık görünsün
        if (errorMessage.startsWith('Exception: ')) {
          errorMessage = errorMessage.substring(11);
        }
        MesajYardimcisi.hataGoster(context, errorMessage);
      }
    }
  }
  ...
  void _handleDetailEdit(
    ...
    if (customTypeLabel == 'Açılış Stoğu (Girdi)') {
      ...
      DepolarVeritabaniServisi().sevkiyatGetir(id).then((data) {
    ...
    setState(() {
      ...
      _selectedDetailUrun = null;
      _selectedDetailWarehouseName = null;
    });
  }
  ...
  Future<void> _loadMoreDetailTransactions(
    UrunModel urun,
    List<Map<String, dynamic>> initialTransactions,
  ) async {
    if (_detailTransactionsLoading[urun.id] == true) return;

    final existing = _detailTransactionsCache[urun.id] ?? initialTransactions;
    if (existing.isEmpty) return;

    final last = existing.last;
    final DateTime? lastDate = last['date_raw'] is DateTime
        ? last['date_raw'] as DateTime
        : null;
    final int? lastShipmentId = last['id'] as int?;

    if (lastDate == null || lastShipmentId == null) {
      setState(() {
        _detailTransactionsHasMore[urun.id] = false;
      });
      return;
    }

    setState(() {
      _detailTransactionsLoading[urun.id] = true;
      _detailTransactionsCache[urun.id] = existing;
      // If never set before, infer "has more" from page size.
      _detailTransactionsHasMore[urun.id] =
          _detailTransactionsHasMore[urun.id] ??
          (existing.length == _detailTransactionsPageSize);
    });

    try {
      final more = await DepolarVeritabaniServisi().urunHareketleriniGetir(
        urun.kod,
        kdvOrani: urun.kdvOrani,
        limit: _detailTransactionsPageSize,
        baslangicTarihi: _startDate,
        bitisTarihi: _endDate,
      ...
      setState(() {
        _detailTransactionsCache[urun.id] = [...existing, ...more];
        _detailTransactionsHasMore[urun.id] =
            more.length == _detailTransactionsPageSize;
        _detailTransactionsLoading[urun.id] = false;
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
      ...
      debugPrint('Detail transactions load more error: $e');
    }
  }
    ...
    try {
      ...
      for (var i = 0; i < dataToProcess.length; i++) {
        ...
        final isExpanded =
            ...
            _expandedMobileIds.contains(urun.id) ||
        ...
        final mainRow = [
          ...
          tr('common.vat_short'), // KDV Label for Main Row
          tr('common.vat_excluded'), // "(KDV Hariç)" - Matches UI
        ...
        // 3. Details (Key-Value) - Product Card Info for Print
        // Calculate Girdi/Çıktı totals from transactions
        double toplamGirdi = 0;
        double toplamCikti = 0;
        for (final tx in transactions) {
          final qty = (tx['quantity'] is num)
              ? (tx['quantity'] as num).toDouble()
              : double.tryParse(tx['quantity'].toString()) ?? 0.0;
        ...
        Map<String, String> details = {
          ...
          tr(
            'products.table.sales_price_1',
          ): '${FormatYardimcisi.sayiFormatlaOndalikli(urun.satisFiyati1, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.fiyatOndalik)} ₺ (${tr('common.vat_excluded')})',
          tr(
            'products.table.sales_price_2',
          ): '${FormatYardimcisi.sayiFormatlaOndalikli(urun.satisFiyati2, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.fiyatOndalik)} ₺ (${tr('common.vat_excluded')})',
          tr(
            'products.table.sales_price_3',
          ): '${FormatYardimcisi.sayiFormatlaOndalikli(urun.satisFiyati3, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.fiyatOndalik)} ₺ (${tr('common.vat_excluded')})',
          tr('products.table.features'): urun.ozellikler.isNotEmpty
              ? urun.ozellikler
              : '-',
          tr('products.transaction.type.input'): FormatYardimcisi.sayiFormatla(
            toplamGirdi,
            binlik: _genelAyarlar.binlikAyiraci,
            ondalik: _genelAyarlar.ondalikAyiraci,
            decimalDigits: _genelAyarlar.miktarOndalik,
          ),
          tr('products.transaction.type.output'): FormatYardimcisi.sayiFormatla(
            toplamCikti,
            binlik: _genelAyarlar.binlikAyiraci,
            ondalik: _genelAyarlar.ondalikAyiraci,
            decimalDigits: _genelAyarlar.miktarOndalik,
        ...
        // 4. Transaction Table - Only if has data
        DetailTable? txTable;
        if (transactions.isNotEmpty) {
          txTable = DetailTable(
            title: tr('products.detail.transactions'),
            headers: [
              tr('products.transaction.type'),
            ...
            data: transactions.map((t) {
              ...
              if (t['unitPrice'] is num && (t['unitPrice'] as num) > 0) {
              ...
              double unitPriceVatVal = 0.0;
              if (t['unitPriceVat'] is num && (t['unitPriceVat'] as num) > 0) {
                unitPriceVatVal = (t['unitPriceVat'] as num).toDouble();
              ...
              double qtyVal = (t['quantity'] is num)
                  ? (t['quantity'] as num).toDouble()
                  : double.tryParse(t['quantity'].toString()) ?? 0.0;

              double unitPriceVal = 0.0;
              if (t['unitPrice'] is num) {
                unitPriceVal = (t['unitPrice'] as num).toDouble();
              }

              double totalVal = qtyVal * unitPriceVatVal;
              double totalValExcluded = qtyVal * unitPriceVal;

              if (totalVal > 0) {
                String valVat = FormatYardimcisi.sayiFormatlaOndalikli(
                  totalVal,
                  binlik: _genelAyarlar.binlikAyiraci,
                  ondalik: _genelAyarlar.ondalikAyiraci,
                  decimalDigits: _genelAyarlar.fiyatOndalik,
                ...
                String valExcluded = FormatYardimcisi.sayiFormatlaOndalikli(
                  totalValExcluded,
                  binlik: _genelAyarlar.binlikAyiraci,
                  ondalik: _genelAyarlar.ondalikAyiraci,
                  decimalDigits: _genelAyarlar.fiyatOndalik,
                ...
                if (_genelAyarlar.sembolGoster) {
                  valVat += ' ${_genelAyarlar.varsayilanParaBirimi}';
                  valExcluded += ' ${_genelAyarlar.varsayilanParaBirimi}';
                }

                formattedTotalPrice = '$valVat\n$valExcluded';
              }

              // Correct Transaction Label Logic to match UI
              final String transactionSourceSuffix =
                  t['sourceSuffix']?.toString() ?? '';
              final String transactionSourceText =
                  transactionSourceSuffix.isNotEmpty
                  ? ' ${IslemCeviriYardimcisi.parantezliKaynakKisaltma(transactionSourceSuffix)}'
                  : '';

              String transactionTypeLabel = IslemCeviriYardimcisi.cevir(
              ...
              return <String>[
                '$transactionTypeLabel$transactionSourceText',
                ...
                formattedUnitPrice,
                formattedUnitPriceVat,
                formattedTotalPrice, // Total Price Data
        ...
        rows.add(
          ExpandableRowData(
            ...
            isExpanded: isExpanded || hasPrintSelectedDetails,
          ...
          builder: (context) => GenisletilebilirPrintPreviewScreen(
            ...
            extraDetailToggles: [
              CustomContentToggle(
                key: 'total_vat',
                label:
                    '${tr('products.transaction.total_price')} (${tr('products.vat.included')})',
              ),
              CustomContentToggle(
                key: 'total_base',
                label:
                    '${tr('products.transaction.total_price')} (${tr('products.vat.excluded')})',
              ),
            ],
            onProcessDetailCell:
                ({
                  ...
                  // Only process if it is the Total Price column (which contains 'Toplam Fiyat' or 'Total Price' etc)
                  // and if the content has a newline (meaning it has two values)
                  final isTotalPrice =
                      header == tr('products.transaction.total_price');

                  if (isTotalPrice && content.contains('\n')) {
                    final parts = content.split('\n');
                    if (parts.length == 2) {
                      final showVat = toggles['total_vat'] ?? true;
                      final showBase = toggles['total_base'] ?? true;
            ...
            headers: [
              ...
              tr('products.table.unit'),
              tr('products.table.purchase_price'),
              tr('common.vat_short'), // KDV Short Label
              tr('common.vat_excluded'), // "(KDV Hariç)"
              tr('products.table.vat'), // KDV Ratio
              tr('products.table.status'),
            ],
            data: rows,
  ...
  Future<void> _urunDurumDegistir(UrunModel urun, bool aktifMi) async {
    try {
      final yeniUrun = urun.copyWith(aktifMi: aktifMi);
      await UrunlerVeritabaniServisi().urunGuncelle(yeniUrun);
      await _fetchUrunler(showLoading: false);
  ...
  Widget build(BuildContext context) {
    List<UrunModel> filteredUrunler = _cachedUrunler;
        ...
        child: CallbackShortcuts(
          bindings: {
            ...
            const SingleActivator(LogicalKeyboardKey.f2): () {
              ...
              final urun = _cachedUrunler.firstWhere((u) => u.id == selectedId);
            ...
            const SingleActivator(LogicalKeyboardKey.f5): () => _fetchUrunler(),
            const SingleActivator(LogicalKeyboardKey.f6): () {
              // F6: Aktif/Pasif Toggle - seçili satır varsa
              if (_selectedRowId == null) return;
              final selectedId = _selectedRowId!;
              final urun = _cachedUrunler.firstWhere((u) => u.id == selectedId);
              _urunDurumDegistir(urun, !urun.aktifMi).then((_) {
                _detailFutures.clear();
                _detailTransactionsCache.clear();
                _detailTransactionsHasMore.clear();
                _detailTransactionsLoading.clear();
            ...
            const SingleActivator(LogicalKeyboardKey.f10): () async {
              ...
              final result = await Navigator.push<bool>(
                context,
                MaterialPageRoute(
                  builder: (context) => DevirYapSayfasi(urun: urun),
                ),
              );
              if (result == true) _fetchUrunler();
            },
  ...
  void _onSelectAll(bool? value) {
    setState(() {
      ...
      if (value == true) {
        _selectedIds
            .clear(); // Clear manual selection as Global selection takes precedence
      } else {
        _selectedIds.clear();
      }
    });
  ...
  // so I'll put the class definition at the bottom of the file outside _UrunlerSayfasiState)
  ...
  Widget _buildDesktopView(
    ...
    final bool allSelected =
        _isSelectAllActive ||
        (urunler.isNotEmpty &&
            urunler.every((u) => _selectedIds.contains(u.id)));
    ...
    return GenisletilebilirTablo<UrunModel>(
      ...
      actionButton: Row(
        children: [
          MouseRegion(
            cursor: YazdirmaErisimKontrolu.mobilBulutYazdirmaPasif
            ...
            child: MouseRegion(
              ...
              hitTestBehavior: HitTestBehavior.deferToChild,
              child: GestureDetector(
                onTap: YazdirmaErisimKontrolu.mobilBulutYazdirmaPasif
                    ? null
                    : _handlePrint,
                child: Container(
                  height: 40,
            ...
            child: PopupMenuButton<String>(
              ...
              onSelected: (value) async {
                if (value == 'transfer') {
                  if (_selectedRowId == null) return;
                  final selectedId = _selectedRowId!;
                  final urun = _cachedUrunler.firstWhere(
                    (u) => u.id == selectedId,
                  );
      ...
      columns: [
        ...
        if (_columnVisibility['unit'] == true)
          GenisletilebilirTabloKolon(
            label: tr('products.table.unit'),
            width: 50,
            alignment: Alignment.centerLeft,
            allowSorting: true,
            flex: useFlex ? 1 : null,
          ),
        if (_columnVisibility['purchase_price'] == true)
          GenisletilebilirTabloKolon(
        ...
        if (_columnVisibility['vat_status'] == true)
          GenisletilebilirTabloKolon(
        ...
        if (_columnVisibility['vat_rate'] == true)
          GenisletilebilirTabloKolon(
        ...
        if (_columnVisibility['status'] == true)
          GenisletilebilirTabloKolon(
            label: tr('products.table.status'),
            width: statusColumnWidth,
            alignment: Alignment.centerLeft,
            allowSorting: true,
          ),
        GenisletilebilirTabloKolon(
          label: tr('products.table.actions'),
          width: 110,
          alignment: Alignment.centerLeft,
        ),
      ],
          ...
          children: [
            _buildCell(
              ...
              child: SizedBox(
                ...
                height: 20,
                child: Checkbox(
                  value: _isSelectAllActive || _selectedIds.contains(urun.id),
      ...
      detailBuilder: (context, urun) {
        ...
        // Ürün ilk harfi (depolar gibi avatar için)
            ...
            children: [
                ...
                child: Row(
                  children: [
                    ...
                    if (_columnVisibility['dt_date'] == true) ...[
                      const SizedBox(
                        width: 32,
                      ), // Spacing between Type/Related and Date
                      Expanded(
                        flex: 7, // Date column width
                        child: _buildDetailHeader(
                          tr('products.transaction.date'),
                        ),
                      ),
                    ...
                    if (_columnVisibility['dt_quantity'] == true) ...[
                      const SizedBox(
                        width: 32,
                      ), // Spacing between Warehouse and Quantity
                      Expanded(
                    ...
                    if (_columnVisibility['dt_unit'] == true) ...[
                      ...
                      Expanded(
                        flex: 2, // Unit column width
                        child: _buildDetailHeader(tr('products.table.unit')),
                      ),
                    ],
                    if (_columnVisibility['dt_unit_price'] == true)
                      Expanded(
                        flex: 7, // Unit Price column width
                        child: _buildDetailHeader(
                          tr('products.transaction.unit_price'),
                          alignRight: true,
                        ),
                    ...
                    if (_columnVisibility['dt_unit_price_vd'] == true) ...[
                      const SizedBox(width: 12),
                      Expanded(
                        flex: 6, // Unit Price VD column width
                        child: _buildDetailHeader(
                          tr('products.transaction.unit_price_vd'),
                          alignRight: true,
                        ),
                    ...
                    if (_columnVisibility['dt_total_price'] == true) ...[
                      const SizedBox(width: 12),
                      Expanded(
                        flex: 7, // Total Price column width
                        child: _buildDetailHeader(
                          tr('products.transaction.total_price'),
                          alignRight: true,
                        ),
                    ...
                    if (_columnVisibility['dt_description'] == true) ...[
                      const SizedBox(
                        width: 48,
                      ), // Spacing between VAT and Description
                      Expanded(
                        flex: 8, // Description
              ...
              FutureBuilder<List<Map<String, dynamic>>>(
                ...
                builder: (context, snapshot) {
                  ...
                  final initialTransactions = snapshot.data!;
                  final cached = _detailTransactionsCache[urun.id];
                  final transactions = cached ?? initialTransactions;
                      ...
                      if (isLoadingMore)
                        ...
                        Padding(
                          ...
                          child: OutlinedButton.icon(
                            onPressed: () => _loadMoreDetailTransactions(
                              urun,
                              initialTransactions,
                            ),
                            icon: const Icon(
  ...
  Widget _buildMobileActionButton({
    ...
    final bool printDisabled =
        ...
        YazdirmaErisimKontrolu.isPrintIcon(icon);
    final bool effectiveEnabled = !printDisabled;
    final Color effectiveColor = effectiveEnabled
        ? color
        : color.withValues(alpha: 0.45);
    final Color effectiveTextColor = effectiveEnabled
        ? textColor
  ...
  Widget _buildMobileSquareActionButton({
    ...
    final bool printDisabled =
        YazdirmaErisimKontrolu.mobilBulutYazdirmaPasif &&
        YazdirmaErisimKontrolu.isPrintIcon(icon);
    final Color effectiveColor = printDisabled
        ? color.withValues(alpha: 0.45)
        : color;
    final Color effectiveIconColor = printDisabled
        ? iconColor.withValues(alpha: 0.45)
        : iconColor;
    final Color effectiveBorderColor = printDisabled
    ...
    Widget child = Material(
      ...
      child: InkWell(
        mouseCursor: WidgetStateMouseCursor.clickable,
        onTap: printDisabled ? null : onTap,
        borderRadius: BorderRadius.circular(8),
        child: Container(
          width: size,
          height: size,
    ...
    final String? effectiveTooltip = printDisabled
        ? YazdirmaErisimKontrolu.tooltip(tooltip)
        : tooltip;
  ...
  Future<void> _handleMobileActionMenuSelection(String value) async {
    if (value == 'transfer') {
      final selectedId = _selectedMobileCardId;
      if (selectedId == null) return;
      final urun = _cachedUrunler.firstWhere((u) => u.id == selectedId);

      final result = await Navigator.push<bool>(
        context,
        MaterialPageRoute(builder: (context) => DevirYapSayfasi(urun: urun)),
      );
  ...
  Widget _buildMobileActionsMenuButton({double size = 48}) {
    return Theme(
      ...
      child: PopupMenuButton<String>(
        ...
        itemBuilder: (context) => [
          PopupMenuItem<String>(
            enabled: _selectedMobileCardId != null,
            value: 'transfer',
            height: 44,
            ...
            child: Row(
              children: [
                Icon(
                  Icons.swap_horiz_rounded,
                  size: 20,
                  color: _selectedMobileCardId != null
                      ? const Color(0xFF4A4A4A)
                      : Colors.grey.shade400,
                ),
                const SizedBox(width: 12),
  ...
  Widget _buildMobileTopActionRow() {
    ...
    final String quickAddLabel = isNarrow ? 'Hızlı' : 'Hızlı Ekle';
  ...
  Widget _buildMobileView(List<UrunModel> filteredUrunler) {
    final int safeRowsPerPage = _rowsPerPage <= 0 ? 25 : _rowsPerPage;
    final int startRecordIndex = (_currentPage - 1) * safeRowsPerPage;
    final int showingStart = filteredUrunler.isEmpty ? 0 : startRecordIndex + 1;
    final int endRecord = filteredUrunler.isEmpty
        ? 0
        : startRecordIndex + filteredUrunler.length;
    final String totalLabel = filteredUrunler.isEmpty
        ? '0'
        : (_hasNextPage ? '$endRecord+' : '$endRecord');
    final int totalRecords = endRecord; // cursor modunda yaklaşık
    ...
    final double footerBottomPadding = mediaQuery.padding.bottom > 0
        ? mediaQuery.padding.bottom.clamp(4.0, 8.0).toDouble()
        : 4.0;
            ...
            Expanded(
              child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  return _buildUrunCard(filteredUrunler[index]);
                },
              ),
            ),
            if (!isKeyboardVisible)
              Padding(
                padding: EdgeInsets.fromLTRB(10, 0, 10, footerBottomPadding),
                child: Row(
  ...
  Widget _buildUrunCard(UrunModel urun) {
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        onTap: () {
          if (_consumeMobileDoubleTap(urun.id)) {
            _openUrunKarti(urun);
            return;
          }
          setState(() {
            if (_selectedMobileCardId == urun.id) {
              ...
              _selectedMobileCardId = urun.id;
            }
          });
        },
        child: Container(
          decoration: BoxDecoration(
            color: isSelectedForTransfer
            ...
            boxShadow: [
              BoxShadow(
                ...
                blurRadius: isSelectedForTransfer ? 12 : 10,
              ...
              Row(
                ...
                children: [
                  SizedBox(
                    ...
                    child: Checkbox(
                      value:
                          _isSelectAllActive || _selectedIds.contains(urun.id),
                      onChanged: (v) => _onSelectRow(v, urun.id),
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(4),
                      ),
  ...
  Widget _buildMobileDetails(UrunModel urun) {
    return Column(
      ...
      children: [
        ...
        _buildMobileProperty(tr('products.table.user'), urun.kullanici),
            ...
            return Column(
              children: snapshot.data!.map((tx) {
                return _buildMobileTransactionRow(
                  ...
                  sourceSuffix: tx['sourceSuffix']?.toString(),
                  devices: _normalizeDeviceMaps(tx['devices']),
  ...
  List<Map<String, dynamic>> _normalizeDeviceMaps(dynamic rawDevices) {
    if (rawDevices is! List) return const <Map<String, dynamic>>[];
    return rawDevices
        .whereType<Map>()
  ...
  String _formatDeviceIdentity(Map<String, dynamic> device) {
    final String identityType =
        (device['identity_type'] ?? device['identityType'] ?? '')
            .toString()
            .trim();
    ...
    if (identityValue.isEmpty) return '';
    if (identityType.isEmpty) return identityValue;
    return '$identityType: $identityValue';
  }

  String _buildDeviceExtraInfo(Map<String, dynamic> device) {
    ...
    if (condition.isNotEmpty) extras.add(condition);
    if (color.isNotEmpty) extras.add(color);
    if (capacity.isNotEmpty) extras.add(capacity);
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(
                      child: Text(
                        '${FormatYardimcisi.sayiFormatla(quantity, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.miktarOndalik)} $unit',
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                        style: const TextStyle(
                          fontWeight: FontWeight.bold,
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: devices.map((device) {
                          final String identity = _formatDeviceIdentity(device);
                          if (identity.isEmpty) return const SizedBox.shrink();

                          final String extra = _buildDeviceExtraInfo(device);
                          return Container(
                            padding: const EdgeInsets.symmetric(
                              horizontal: 8,
                              vertical: 4,
                ...
                child: Row(
                  ...
                  children: [
                                    ...
                                    text: IslemCeviriYardimcisi.cevir(
                                      IslemTuruRenkleri.getProfessionalLabel(
                                        customTypeLabel ??
                                            (isIncoming
                                                ? tr(
                                                    'warehouses.detail.type_in',
                                                  )
                                                : tr(
                                                    'warehouses.detail.type_out',
                                                  )),
                                        context: 'stock',
                                      ),
                                    ),
                        ...
                        child: Align(
                          ...
                          child: Builder(
                            builder: (context) {
                              ...
                              return HighlightText(
                                text:
                                    '${FormatYardimcisi.sayiFormatlaOndalikli(0.0, binlik: _genelAyarlar.binlikAyiraci, ondalik: _genelAyarlar.ondalikAyiraci, decimalDigits: _genelAyarlar.fiyatOndalik)} ${_genelAyarlar.varsayilanParaBirimi}',
                                query: _searchQuery,
                                textAlign: TextAlign.right,
                                style: const TextStyle(
                                  fontSize: 11,
                            ...
                            builder: (context) {
                              ...
                              if (quantity is num && unitPrice is num) {
                                ...
                                final q =
                                    double.tryParse(quantity.toString()) ?? 0.0;
                                final p =
                                    double.tryParse(unitPrice.toString()) ??
                                    0.0;
                    ...
                    if (_columnVisibility['dt_description'] == true) ...[
                      ...
                      Expanded(
                        flex: 8, // Increased for better width
                        child: HighlightText(
                          text: description,
                          query: _searchQuery,
                          style: const TextStyle(
    ...
    return Theme(
      ...
      child: PopupMenuButton<String>(
        ...
        onSelected: (value) {
          if (value == 'edit') {
            if (urun != null) {
              _handleDetailEdit(id, customTypeLabel, urun, warehouseName);
            }
  ...
  Widget _buildPopupMenu(UrunModel urun) {
    return Theme(
      data: Theme.of(context).copyWith(
        dividerTheme: const DividerThemeData(
          color: Color(0xFFEEEEEE),
      ...
      child: PopupMenuButton<String>(
        ...
        onSelected: (value) {
          if (value == 'open_card') {
            ...
            _urunDurumDegistir(urun, false).then((_) {
              _detailFutures.clear();
              _detailTransactionsCache.clear();
              _detailTransactionsHasMore.clear();
              _detailTransactionsLoading.clear();
            ...
            _urunDurumDegistir(urun, true).then((_) {
              _detailFutures.clear();
              _detailTransactionsCache.clear();
              _detailTransactionsHasMore.clear();
              _detailTransactionsLoading.clear();
  ...
  Future<void> _deleteTransaction(int id) async {
    final result = await showDialog<bool>(
      ...
      builder: (context) => OnayDialog(
        ...
        onOnay: () {
          // Do nothing inside the dialog callback. We handle the logic after await.
        },
        isDestructive: true,
        onayButonMetni: tr('common.delete'),
      ),
    ...
    if (result == true) {
      ...
      // Perform deletion here, awaiting completion
      try {
        ...
        if (mounted) {
          setState(() {
            _detailFutures.clear(); // Clear cache
            _detailTransactionsCache.clear();
            _detailTransactionsHasMore.clear();
            _detailTransactionsLoading.clear();
