...
class _UretimlerSayfasiState extends State<UretimlerSayfasi> {
  ...
  Future<void> _fetchUretimler({bool showLoading = true}) async {
    ...
    try {
      ...
      final bool needsMovementTables =
          _searchQuery.trim().isNotEmpty || hasMovementFilters;

      final List<String> indexTables = needsMovementTables
          ? const <String>['productions', 'production_stock_movements']
      ...
      // Movement-level filters should not accidentally match root docs.
      if (hasMovementFilters) {
        filterParts.add('table:production_stock_movements');
      }
      ...
      if (usePrimarySearch) {
        ...
        urunler = await UretimlerVeritabaniServisi().uretimleriGetir(
          sayfa: _currentPage,
        ...
        if (hasMore) urunler = urunler.sublist(0, _rowsPerPage);
      }

      if (!mounted || sorguNo != _aktifSorguNo) return;
      final List<UretimModel> display = urunler;
  ...
  Future<void> _handlePrint() async {
    ...
    try {
      ...
      for (var i = 0; i < dataToProcess.length; i++) {
        ...
        final int originIndex = _cachedUretimler.indexWhere(
          (u) => u.id == urun.id,
        );
        final int orderNo =
        ...
        if (isExpanded || hasPrintSelectedDetails) {
          ...
          if (cached != null && cached.isNotEmpty) {
            transactions = List<Map<String, dynamic>>.from(cached);
          } else {
