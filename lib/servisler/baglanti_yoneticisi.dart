...
import 'bulut_sema_bootstrap_servisi.dart';
import 'buyuk_olcek_arama_bootstrap_servisi.dart';
import 'yerel_ag_yazdirma_servisi.dart';
...
class BaglantiYoneticisi extends ChangeNotifier {
  ...
  Future<void> _mobilAkilliBaslatma() async {
    ...
    if (mode == 'cloud' && !VeritabaniYapilandirma.cloudAccessReady) {
  ...
  Future<bool> _bulutKimlikleriniHazirlaBestEffort() async {
    if (VeritabaniYapilandirma.cloudAccessReady) return true;
  ...
  Future<void> _standartBaslatma() async {
    ...
    if (VeritabaniYapilandirma.connectionMode == 'cloud') {
      if (VeritabaniYapilandirma.cloudApiCredentialsReady) {
        final ok =
            await VeritabaniYapilandirma.testSavedCloudDatabaseConnection(
    ...
    // [2026 GOOGLE-SEARCH] Bulut (Supabase/Neon) şemasını proaktif hazırla.
    // Amaç: pg_trgm + normalize_text + trigger'lar + search_tags GIN indeksleri hazır olsun.
    // Best-effort: başarısız olursa mevcut akışı/özellikleri bozmaz.
    if (VeritabaniYapilandirma.connectionMode == 'cloud') {
  ...
  Future<bool> _clusterUyumlulukKontroluBestEffort() async {
    try {
      ...
      final prefs = await SharedPreferences.getInstance();
      final expected = (prefs.getString(_prefExpectedClusterIdKey) ?? '')
          .trim();
      if (expected.isEmpty) {
