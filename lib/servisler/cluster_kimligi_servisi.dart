...
class ClusterKimligiServisi {
  ...
  static String _sqlLiteral(String value) {
    // Simple Query Protocol modunda parametre kullanamayacağımız için
    // minimal ve güvenli literal escape.
  ...
  Future<Connection> _open(
    ...
    return Connection.open(
      ...
      settings: ConnectionSettings(
        ...
        // PgBouncer / pooler uyumluluğu için güvenli varsayılan.
  ...
  Future<String?> oku({
    ...
    try {
      ...
      final k = _sqlLiteral(clusterIdKey);
  ...
  Future<String?> okuVeyaOlusturWithExecutor({
    required SessionExecutor executor,
    bool allowCreate = true,
  }) async {
    try {
      return await executor.run((session) async {
        final res = await session.execute(
          Sql.named(
            'SELECT value FROM public.general_settings WHERE key = @k LIMIT 1',
          ),
          parameters: <String, Object?>{'k': clusterIdKey},
        );
        if (res.isNotEmpty) {
          final v = (res.first[0] as String?)?.trim();
          if (v != null && v.isNotEmpty) return v;
