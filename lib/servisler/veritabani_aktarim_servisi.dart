...
class VeritabaniAktarimServisi {
  ...
  Future<VeritabaniAktarimHazirlik?> hazirlikYap({
    ...
    if ((from == 'cloud' || to == 'cloud') &&
        !VeritabaniYapilandirma.cloudAccessReady) {
      return null;
    }
  ...
  Future<bool> _isPartitionedParent(Connection conn, String table) async {
    final t = table.trim();
    ...
    try {
      ...
      final kind = (res.first[0]?.toString() ?? '').toLowerCase();
      return kind == 'p';
    } catch (_) {
      return false;
    }
  ...
  Future<({DateTime? min, DateTime? max})> _minMaxAcrossSources({
    ...
    for (final src in sources) {
      try {
        final res = await src.conn.execute('''
          SELECT MIN(${_qi(partitionKeyColumn)}), MAX(${_qi(partitionKeyColumn)})
          FROM ${_qt(table)}
          WHERE ${_qi(partitionKeyColumn)} IS NOT NULL
        ''');
        if (res.isEmpty) continue;
        final a = res.first[0] as DateTime?;
        final b = res.first[1] as DateTime?;
        if (a != null) {
          min = (min == null || a.isBefore(min)) ? a : min;
        }
        if (b != null) {
          max = (max == null || b.isAfter(max)) ? b : max;
        }
      } catch (_) {
        // Kaynakta tablo yoksa veya kolon uyumsuzsa atla.
        continue;
      }
    }
  ...
  Future<void> _ensureMonthlyPartitionsForRange({
    required Connection target,
    required _PartitionedTableInfo info,
    ...
    // DEFAULT partition garanti (import sırasında insert patlamasın).
    try {
      await target.execute(
        'CREATE TABLE IF NOT EXISTS ${_qi(info.defaultTable)} PARTITION OF ${_qi(info.parentTable)} DEFAULT',
      );
    } catch (_) {}
    ...
    for (var i = 0; i < 600; i++) {
      ...
      final String partitionName =
          '${info.partitionPrefix}_y${year}_m$monthStr';
      ...
      try {
        await target.execute('''
          CREATE TABLE IF NOT EXISTS ${_qi(partitionName)}
          PARTITION OF ${_qi(info.parentTable)}
        ...
        if (e is ServerException && e.code == '42P17') {
          ...
          // Fail-open: import'u tamamen kırmamak için sessizce geç.
        }
      }
  ...
  Future<void> _backfillPartitionedDefault({
    required Connection target,
    required _PartitionedTableInfo info,
  }) async {
    // DEFAULT içindeki aralığı öğren, eksik partitionları hazırla, sonra taşı.
    final range = await target.execute('''
      SELECT MIN(${_qi(info.partitionKeyColumn)}), MAX(${_qi(info.partitionKeyColumn)})
      FROM ${_qt(info.defaultTable)}
      WHERE ${_qi(info.partitionKeyColumn)} IS NOT NULL
    ''');
    ...
    await _ensureMonthlyPartitionsForRange(
      target: target,
      info: info,
      start: minDt,
      end: maxDt,
    ...
    await PgEklentiler.moveRowsFromDefaultPartition(
      ...
      defaultTable: info.defaultTable,
      partitionKeyColumn: info.partitionKeyColumn,
    );
  }
  ...
  Future<int> _copyTableFromMultipleSources({
    ...
    final partInfo = _partitionedTables[normalizedTable];
    if (deltaSince == null && partInfo != null) {
      try {
        final isPart = await _isPartitionedParent(target, partInfo.parentTable);
        if (isPart) {
          final range = await _minMaxAcrossSources(
            sources: sources,
            table: partInfo.parentTable,
            partitionKeyColumn: partInfo.partitionKeyColumn,
          );
        ...
        _log('Partition prep skipped: table=$table error=$e');
      }
    }
    ...
    if (deltaSince == null && partInfo != null) {
      try {
        final isPart = await _isPartitionedParent(target, partInfo.parentTable);
        if (isPart) {
          await _backfillPartitionedDefault(target: target, info: partInfo);
        }
      } catch (e) {
        _log('Default backfill skipped: table=$table error=$e');
      }
    }
        ...
        BEGIN
          ...
          ON CONFLICT (table_name, pk_hash) DO UPDATE
            SET deleted_at = GREATEST(public.$_tombstonesTable.deleted_at, EXCLUDED.deleted_at),
                public.$_tombstonesTable.pk = EXCLUDED.pk;
  ...
  Future<void> _ensureDeltaOutboxInfraBestEffort(
    ...
    try {
      ...
      // Upgrade-safe: older installations may already have the table without
      // the search consumer columns.
      await conn.execute(
        'ALTER TABLE public.$_deltaOutboxTable ADD COLUMN IF NOT EXISTS search_acked_at TIMESTAMPTZ',
      );
      await conn.execute(
        'ALTER TABLE public.$_deltaOutboxTable ADD COLUMN IF NOT EXISTS search_retry_count INTEGER NOT NULL DEFAULT 0',
      );
      await conn.execute(
        'ALTER TABLE public.$_deltaOutboxTable ADD COLUMN IF NOT EXISTS search_last_error TEXT',
      );
      await conn.execute(
        'ALTER TABLE public.$_deltaOutboxTable ADD COLUMN IF NOT EXISTS search_dead BOOLEAN NOT NULL DEFAULT false',
      );
      await conn.execute(
      ...
      await conn.execute(
        'CREATE INDEX IF NOT EXISTS idx_sync_delta_outbox_search_acked_at ON public.$_deltaOutboxTable (search_acked_at)',
      );
      ...
      await conn.execute('''
        ...
        BEGIN
          ...
          ON CONFLICT (table_name, pk_hash) DO UPDATE
            SET public.$_deltaOutboxTable.pk = EXCLUDED.pk,
  ...
  _DbConn? _buildCloudConnection() {
    if (VeritabaniYapilandirma.cloudApiCredentialsReady) {
      final baseUrl = VeritabaniYapilandirma.cloudApiBaseUrl;
      final token = VeritabaniYapilandirma.cloudApiToken;
      final db = VeritabaniYapilandirma.cloudDatabase;
      if (baseUrl == null || token == null || db == null) return null;
      return _DbConn(
        host: baseUrl,
        port: 0,
        database: db.trim(),
  ...
  Future<Connection> _open(_DbConn info) async {
    if (info.useApi) {
      final raw = (info.apiBaseUrl ?? info.host).trim();
      final token = (info.apiToken ?? '').trim();
      final uri = Uri.tryParse(raw);
      if (uri == null || token.isEmpty) {
        throw StateError('Cloud API config eksik/geçersiz (URL/token).');
      }
      return ApiConnection(
        client: VeritabaniApiClient(
          baseUrl: uri,
          token: token,
          defaultTimeout: info.isCloud
    ...
    // Bu servis çok sayıda parametreli sorgu (`Sql.named(..., parameters: ...)`) kullanır.
    // Dart `postgres` paketinde QueryMode.simple parametreli sorguları desteklemez.
    // Bu yüzden burada her zaman EXTENDED kullan.
    const QueryMode qm = QueryMode.extended;
...
class _PartitionedTableInfo {
  final String parentTable;
  final String defaultTable;
  final String partitionKeyColumn;
  final String partitionPrefix;

  const _PartitionedTableInfo({
    required this.parentTable,
    required this.defaultTable,
    required this.partitionKeyColumn,
    required this.partitionPrefix,
  });
}
