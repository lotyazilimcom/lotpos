...
class UrunlerVeritabaniServisi {
    ...
    try {
      ...
      if (depsOk.isNotEmpty && shipsOk.isNotEmpty) {
                ...
                SELECT 
                  LOWER(
                    (CASE 
                      ...
                      WHEN s.integration_ref = 'production_output' OR s.description ILIKE '%Üretim (Çıktı)%' THEN 'üretim çıktısı'
                      WHEN s.description ILIKE '%Üretim (Girdi)%' OR s.description ILIKE '%Üretim (Giriş)%' THEN 'üretim girdisi'
                      WHEN s.description ILIKE 'Satış%' OR s.description ILIKE 'Satis%' THEN 'satış faturası'
                      WHEN s.description ILIKE 'Alış%' OR s.description ILIKE 'Alis%' THEN 'alış faturası'
                      WHEN s.source_warehouse_id IS NULL AND s.dest_warehouse_id IS NOT NULL THEN 'devir girdi'
                      WHEN s.source_warehouse_id IS NOT NULL AND s.dest_warehouse_id IS NULL THEN 'devir çıktı'
       ...
       ''');
        ...
        debugPrint(
          'Sevkiyat geçmişi indeksleme atlandı (depots/shipments tabloları hazır değil).',
        );
      }
  ...
  Future<void> _tablolariOlustur() async {
    ...
    await _pool!.execute('''
      CREATE TABLE IF NOT EXISTS products (
        ...
        search_tags TEXT, -- 1 Milyar Kayıt İçin Performans Sütunu
        created_by TEXT,
  ...
  Future<List<UrunModel>> urunleriGetir({
    ...
    if (depoIds != null && depoIds.isNotEmpty) {
      // Depo görünümünde stok, seçili depoların toplamıdır (HIZLI SUBQUERY)
      // Parametreli ANY kullanımı (plan cache dostu)
      selectClause += ''', (
    ...
    if (lastId != null) {
      if (sortColumn == 'id' || sortColumn == 'products.id') {
        ...
        // Subquery alias olduğu için keyset zor, fallback offset veya basit id
        // Şimdilik performans için ID bazlı ilerletelim (UI'da titreme olabilir ama hızlıdır)
        // Veya depo_stogu hesaplanan bir alan olduğu için performanslı bir şekilde where'e koyamayız.
        // Mecburen OFFSET fallback.
      } else {
        // Karmaşık Sıralama (Adı, ID)
        String op = direction == 'ASC' ? '>' : '<';

        // Tablo alias'ı (products.) varsa kaldır veya kullan
        // sortColumn 'ad' ise -> products.ad
        // where içinde 'products.ad' demek gerekebilir
        String dbSortCol = sortColumn.startsWith('products.')
            ? sortColumn
            : 'products.$sortColumn';
        // 'depo_stogu' gibi aliaslar hariç
    ...
    // Keyset kullanılabiliyorsa (lastId var ve depo_stogu değilse) OFFSET KULLANMA
    bool useKeyset = lastId != null && sortColumn != 'depo_stogu';

    if (useKeyset) {
      // Keyset ile oluşturulan where conditionlar zaten eklendi.
      ...
      // Keyset mümkün değilse (depo_stogu veya ilk sayfa offsetli)
      // İlk sayfa için lastId null gelir -> OFFSET 0.

      // DEPO STOGU gibi computed column'larda deferred join şart.
  ...
  Future<int> urunSayisiGetir({
    ...
    // Eğer Estimate çalışmazsa veya sayı küçükse (ama 100B veride aslında büyükse)
    // COUNT(*) patlamasın diye LIMIT'li subquery kullan.
    // "100.001 kayıt varsa 100001 dön"
    if (!query.contains('LIMIT')) {
      // Mevcut query: SELECT COUNT(*) FROM products ...
      // Bunu şu şekle çevir: SELECT COUNT(*) FROM (SELECT 1 FROM products ... LIMIT 100001)

      // Önce orijinal query'deki COUNT(*) kısmını atıp SELECT 1 yapalım
  ...
  Future<void> topluFiyatGuncelle({
    ...
    while (islenenKayit < toplamKayit) {
      // ID tabanlı keyset pagination: OFFSET yerine son id'den sonrasını al
