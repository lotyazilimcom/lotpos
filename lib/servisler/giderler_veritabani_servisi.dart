...
class GiderlerVeritabaniServisi {
  ...
  bool _expenseItemsHasSearchTags = false;
  ...
  Future<void> baslat() async {
    ...
    try {
      if (_pool != null) {
        ...
        await _ensureExpenseItemSearchTagsBestEffort();
  ...
  Future<void> _tablolariOlustur() async {
    ...
    final alterQueries = [
      ...
      "ALTER TABLE expenses ADD COLUMN IF NOT EXISTS search_tags TEXT NOT NULL DEFAULT ''",
      ...
      "ALTER TABLE expense_items ADD COLUMN IF NOT EXISTS search_tags TEXT NOT NULL DEFAULT ''",
    ];
  ...
  Future<void> _ensureExpenseItemSearchTagsBestEffort() async {
    final pool = _pool;
    if (pool == null) return;

    try {
      await PgEklentiler.ensureSearchTagsNotNullDefault(pool, 'expense_items');
    } catch (_) {}
    ...
    if (_expenseItemsHasSearchTags &&
      ...
      unawaited(_backfillExpenseItemSearchTags());
    }
  }
  ...
  Future<void> _backfillExpenseItemSearchTags({
    int batchSize = 2000,
    int maxBatches = 50,
  }) async {
    if (_pool == null) return;
    if (!_expenseItemsHasSearchTags) return;
      ...
      final updated = await _pool!.execute(
        Sql.named('''
          ...
          SET search_tags = LOWER(
            COALESCE(ei.aciklama, '') || ' ' ||
            COALESCE(ei.not_metni, '') || ' ' ||
            COALESCE(ei.tutar::text, '') || ' ' ||
            COALESCE(TO_CHAR(ei.created_at, 'DD.MM.YYYY HH24:MI'), '') || ' ' ||
            COALESCE(ei.id::text, '')
          )
          FROM todo
          WHERE ei.id = todo.id
      ...
      await ctx.execute(
        Sql.named('''
          ...
          SET 
            ...
            search_tags = LOWER(TRIM(CONCAT_WS(' ', e.search_tags, NULLIF(@append_tags::text, '')))),
  ...
  Future<List<GiderModel>> giderleriGetir({
    ...
    switch (sortBy) {
      case 'kod':
        sortColumn = 'kod';
        sortExpr = "COALESCE(kod, '')";
        break;
      case 'baslik':
        sortColumn = 'baslik';
        sortExpr = "COALESCE(baslik, '')";
        break;
      case 'tutar':
        sortColumn = 'tutar';
        sortExpr = 'tutar';
    ...
    final bool isIdSort = sortColumn == 'id';
    ...
    if (lastId != null && lastId > 0 && !isIdSort) {
      try {
        ...
        if (cursor.isNotEmpty) {
          lastIsNull = cursor.first[0] as bool?;
          lastSortValue = cursor.first[1];
        }
      } catch (_) {
    ...
    if (lastId != null && lastId > 0) {
      ...
      if (isIdSort) {
        whereConditions.add('id $op @lastId');
        params['lastId'] = lastId;
      } else if (lastIsNull != null && lastSortValue != null) {
        whereConditions.add(
          '(($sortColumn IS NULL), $sortExpr, id) $op (@lastIsNull, @lastSort, @lastId)',
        );
        ...
        whereConditions.add(
          '($sortExpr $op @lastSort OR ($sortExpr = @lastSort AND id $op @lastId))',
        );
        params['lastSort'] = lastSortValue;
        params['lastId'] = lastId;
      } else {
        // Fallback: id cursor (still deep-pagination safe without skip-based paging).
        whereConditions.add('id $op @lastId');
        params['lastId'] = lastId;
      }
    }
    ...
    final query =
      ...
      FROM expenses
      $whereClause
      ${isIdSort ? 'ORDER BY id $direction' : 'ORDER BY ($sortColumn IS NULL) ${sortAscending ? 'ASC' : 'DESC'}, $sortExpr $direction, id $idDirection'}
      LIMIT @limit
    ''';
  ...
  Future<Map<String, Map<String, int>>> giderFiltreIstatistikleriniGetir({
    ...
    final categoryQuery =
        '''
      SELECT COALESCE(kategori, '') as kategori, COUNT(*)
      FROM expenses
    ...
    final paymentQuery =
        '''
      SELECT COALESCE(odeme_durumu, '') as odeme_durumu, COUNT(*)
      FROM expenses
    ...
    final userQuery =
        '''
      SELECT COALESCE(kullanici, '') as kullanici, COUNT(*)
      FROM expenses
