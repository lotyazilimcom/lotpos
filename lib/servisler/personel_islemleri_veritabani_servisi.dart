...
class PersonelIslemleriVeritabaniServisi {
  ...
  Future<void> _tablolariOlustur() async {
    ...
    try {
      ...
      if (isPartitioned) {
        ...
        await _createUserTransactionPartitions(now);
        ...
        if (_config.allowBackgroundDbMaintenance) {
          unawaited(() async {
            try {
              for (int i = -24; i <= 60; i++) {
                if (i == 0) continue;
                await _createUserTransactionPartitions(
                  DateTime(now.year, now.month + i, 1),
                );
              }
              // [100B/20Y] DEFAULT partition'a yığılan eski verileri doğru aylık partitionlara taşı (best-effort).
              try {
                await _backfillUserTransactionsDefault();
              } catch (e) {
                debugPrint('Personel default backfill uyarısı: $e');
              }
            } catch (e) {
        ...
        try {
          ...
          try {
            final rangeRows = await _pool!.execute('''
              SELECT MIN($dateExpr), MAX($dateExpr)
            ...
            if (rangeRows.isNotEmpty) {
              ...
              if (minDt != null && maxDt != null) {
                await _ensureUserTransactionPartitionsForRange(minDt, maxDt);
              }
            }
          } catch (_) {
            // Partition hazırlığı başarısız olsa bile DEFAULT partition sayesinde migration devam edebilir.
          }
  ...
  Future<void> _ensurePartitionExists(DateTime date, {TxSession? session}) async {
    ...
    try {
      await _createUserTransactionPartitions(date, session: session);
      _cachePartitionReady(key);
    } catch (e) {
      debugPrint('Personel partition check failed for $key: $e');
    }
  }
  ...
  Future<void> _createUserTransactionPartitions(
    ...
    final yearTable = 'user_transactions_y${year}_m$monthStr';
    final legacyYearTable = 'user_transactions_$year';
  ...
  Future<void> _ensureUserTransactionPartitionsForRange(
    DateTime start,
    DateTime end, {
    TxSession? session,
  }) async {
    ...
    for (var i = 0; i < 600; i++) {
      await _createUserTransactionPartitions(cursor, session: session);
      if (cursor.year == endMonth.year && cursor.month == endMonth.month) break;
      cursor = DateTime(cursor.year, cursor.month + 1, 1);
    }
  }

  Future<void> _backfillUserTransactionsDefault({TxSession? session}) async {
    if (_pool == null && session == null) return;
    final executor = session ?? _pool!;

    final range = await executor.execute('''
      SELECT MIN(date), MAX(date)
      FROM user_transactions_default
  ...
  Future<void> processOutbox() async {
    ...
    Pool<void>? settingsPool;
    try {
      settingsPool = await VeritabaniHavuzu().havuzAl(
      ...
      // settingsPool: VeritabaniHavuzu tarafından yönetilir.
    }
  }
  ...
  Future<void> alacaklandir({
    ...
    Future<void> operation(TxSession s) async {
      await _ensurePartitionExists(tarih, session: s);
